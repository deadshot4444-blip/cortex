...
async function loadMcatRepairs() {
  ...
  repairLoading = (async () => {
    try {
      ...
      if (data.version !== 1 || !Array.isArray(data.concepts) || !data.concepts.length)
        throw new Error('Invalid repair lessons');
      repairData = data;
...
function setRepairView(root) {
  ...
  footer.querySelector('.sf-legal').textContent = footer
    .querySelector('.sf-legal')
    .textContent.replace(
      'Original study content with guided self-review.',
      'Original educational pilot. Sources included with each lesson.'
    );
  root.appendChild(footer);
  setView(root);
}
function repairDate(at) {
  return new Date(at).toLocaleString(undefined, { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' });
}
function repairConcept(id) {
  return repairData?.concepts.find(c => c.id === id);
}
...
function mountRepairDashboard(host, showEvidence = true) {
  ...
  if (!repairData) {
    host.innerHTML =
      '<div class="repair-priority"><h2>Five-minute repair sessions</h2><p>The lessons could not load. Your other study tools are ready.</p><button class="btn" data-repair-retry>Retry lessons</button></div>';
    host.querySelector('[data-repair-retry]').onclick = async () => {
      await loadMcatRepairs();
      mountRepairDashboard(host, showEvidence);
    };
    return;
  }
  ...
  host
    .querySelector('[data-repair-start]')
    ?.addEventListener('click', () => launchRepairRecommendation(recommendation));
...
async function renderRepairHub() {
  ...
  if (repairData) {
    ...
    main.querySelector('.repair-library').innerHTML = repairData.concepts
      .map(c => {
        ...
        const action = isActive
          ...
          : due
            ? 'Start later check'
            : !(r?.attempts || []).some(a => a.mode !== 'diagnose') || gap
              ? 'Start session'
              : 'Review & practice';
      ...
      .join('');
    main.querySelectorAll('[data-repair-concept]').forEach(
      b =>
        (b.onclick = () => {
          const c = repairConcept(b.dataset.repairConcept),
            r = repairState.records[c.id];
          if (repairState.active) return renderRepairSession();
          const kind =
            r?.dueAt && r.dueAt <= Date.now() && McatRepairCore.unseenChecks(c, repairState).length
              ? 'later'
              : !(r?.attempts || []).some(a => a.mode !== 'diagnose') || gaps.some(g => g.concept.id === c.id)
                ? 'repair'
                : 'practice';
          McatRepairCore.begin(c, repairState, kind, Date.now());
          saveMcatRepair();
          renderRepairSession();
        })
    );
    if (repairState.active)
      main
        .querySelector('.repair-library')
        .prepend(el('<p class="repair-fine">Finish your saved session before opening a different concept.</p>'));
  }
  ...
  if (c.id === 'enzyme-inhibition') {
    const curve = km =>
      Array.from({ length: 81 }, (_, i) => {
        const x = i / 4;
        return `${i ? 'L' : 'M'}${40 + 15 * x},${175 - (130 * x) / (km + x)}`;
      }).join(' ');
...
function renderRepairSession() {
  const a = repairState?.active,
    c = repairConcept(a?.conceptId);
  if (!a || !c) return renderRepairHub();
  if (location.pathname !== '/mcat') {
    const url = new URL(location.href);
    url.pathname = '/mcat';
    history.pushState({ sec: 'mcat' }, '', url.pathname + url.search);
  }
  const root = el(`<div>${mcatTaskHeader(['Repair'], '<span class="topstat">5 min</span>', 'Exit')}</div>`);
  const step =
    ['diagnose', 'feedback'].includes(a.phase) && a.questionId === c.diagnostic.id
      ? 0
      : a.phase === 'lesson'
        ? 1
        : a.phase === 'done'
  ...
  if (a.phase === 'lesson') {
    ...
    const label =
      a.phase === 'diagnose' || (answered && a.result.mode === 'diagnose')
        ? 'Find the gap'
        : a.kind === 'later'
          ? 'Later check · answer before review'
          : a.kind === 'practice' || (answered ? a.result.mode === 'practice' : !first)
            ? 'Practice · previously seen question'
            : 'Apply the idea · new question';
    ...
    stage.querySelectorAll('[name="repair-confidence"]').forEach(
      radio =>
        (radio.onchange = () => {
          a.confidence = radio.value;
          saveMcatRepair();
        })
    );
    stage.querySelectorAll('[data-repair-answer]').forEach(
      b =>
        (b.onclick = () => {
          McatRepairCore.answer(c, repairState, Number(b.dataset.repairAnswer), a.confidence, Date.now());
          saveMcatRepair();
          renderRepairSession();
        })
    );
