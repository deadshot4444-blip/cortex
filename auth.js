...
function downloadProgress() {
  let copies;
  try {
    copies = progress?.recovery() || { recoveryError: 'Account storage is unavailable.' };
  } catch {
    copies = { recoveryError: 'Account copies could not be read. Keep this tab open.' };
  }
  ...
  link.href = url;
  link.download = 'cortex-progress-recovery.json';
  link.click();
...
function accountBlocked(message, kind) {
  ...
  dialog.style.cssText =
    'max-width:480px;width:calc(100% - 32px);box-sizing:border-box;padding:24px;border-radius:16px;line-height:1.6';
  dialog.innerHTML =
    '<h2 id="account-paused-title">Saving is paused</h2><p></p><div class="fbmodal-btns"><button class="btn" data-download>Download recovery copy</button><button class="btn btn-solid" data-reload>Reload</button></div><p role="status"></p>';
  ...
  dialog.querySelector('[data-download]').onclick = () => {
    try {
      ...
      dialog.querySelector('[role="status"]').textContent = 'Could not prepare a download. Keep this tab open.';
    }
  };
...
async function sendMagicLink(email) {
  try {
    return await sb.auth.signInWithOtp({ email, options: { emailRedirectTo: location.origin + '/' } });
  } catch (error) {
    return { error };
  }
}
...
function openAuth() {
  if (authFailure) {
    accountBlocked('Account storage is unavailable. Saving cannot be confirmed. Free browser storage and reload.');
    return;
  }
  ...
  if (signedIn) {
    ...
    back.querySelector('[data-signout]').addEventListener('click', async e => {
      ...
      if (error) {
        status.textContent = 'Sign-out could not finish. Try again; this account is still active.';
        back.querySelector('[data-signout]').disabled = false;
      } else close();
    });
    back.querySelector('[data-download]').onclick = () => {
      try {
        downloadProgress();
        status.textContent = 'Recovery download prepared.';
      } catch {
        status.textContent = 'Could not prepare the download. Keep this tab open.';
      }
    };
    back.querySelector('[data-retry]')?.addEventListener('click', async () => {
      await progress.sync();
      close();
      openAuth();
    });
    for (const choice of ['cloud', 'device'])
      back.querySelector('[data-' + choice + ']')?.addEventListener('click', () => {
        try {
          progress.resolve(choice);
          close();
        } catch {
          status.textContent = 'Could not save a recovery copy. Your active progress has not been replaced.';
        }
      });
    back.querySelector('[data-guest]')?.addEventListener('click', () => {
      try {
        if (!progress.useGuest()) status.textContent = 'Finish syncing or resolve the saved-copy conflict first.';
      } catch {
        status.textContent = 'Could not save a recovery copy. Your active progress has not been replaced.';
      }
    });
    ...
    send.addEventListener('click', async () => {
      ...
      if (error) {
        ...
        status.textContent = 'Check your email for a one-tap sign-in link. 📧';
...
window.CortexAccount = Object.freeze({
  get available() {
    return !!progress && !authFailure;
  },
  get label() {
    return progress?.owner === 'guest'
      ? 'Guest workspace on this device'
      : currentUser?.email || 'Saved account workspace';
  },
  get state() {
    return progress?.state || 'unavailable';
  },
  snapshot() {
    if (!progress) throw Error('Saved-work storage is unavailable. Reload to retry.');
    return progress.portableSnapshot();
  },
  prepareRestore(data) {
    if (!progress) throw Error('Saved-work storage is unavailable.');
    return progress.prepareRestore(data);
  },
  restore(preview) {
    return progress.restore(preview);
  },
...
function initAuth() {
  try {
    ...
    const raw = {
      ...
      setItem: (k, v) => rawSet.call(localStorage, k, v),
      removeItem: k => rawRemove.call(localStorage, k),
    };
    ...
    progress = CortexProgress.create({
      storage: raw,
      client: sb,
    ...
    refreshAuthUI();
    accountBlocked(
      'Browser storage could not be opened safely. Keep this tab open and free some browser storage before reloading.'
    );
    return;
  }
  if (!AUTH_ENABLED) {
    ...
    if (progress.owner !== 'guest') {
      progress.stop();
      accountBlocked(
        'Sign-in support could not load. Your saved account workspace is retained. Reload with a connection before changing it.'
      );
    }
    refreshAuthUI();
    return;
  ...
  document.addEventListener('visibilitychange', () => {
    if (document.visibilityState === 'visible') progress.sync();
  });
