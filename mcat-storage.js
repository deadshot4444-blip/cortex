/* Shared MCAT persistence: retain drafts, refuse stale writes, and expose recovery. */
const McatStorage=(()=>{
  ...
  function recovery(){
    ...
    return {exportedAt:new Date().toISOString(),purpose:'MCAT save recovery',accountChanged:currentOwner===null?null:!currentOwner,records};
  }
  function download(){
  ...
  function showProblem(kind){
    ...
    if(first){window.dispatchEvent(new Event('mcat-storage-paused'));if(kind==='conflict')window.dispatchEvent(new Event('mcat-storage-conflict'));}
    if(!document.body)return;
    if(!dialog){
      dialog=document.createElement('dialog');dialog.id='mcat-save-conflict';
      dialog.setAttribute('aria-labelledby','mcat-conflict-title');dialog.setAttribute('aria-describedby','mcat-conflict-description');
      ...
      dialog.innerHTML='<h2 id="mcat-conflict-title"></h2><p id="mcat-conflict-description"></p><p>Keep this tab open. Download the available copies before reloading if you have work to keep.</p><div style="display:flex;gap:12px;flex-wrap:wrap"><button class="btn" id="mcat-conflict-export">Download recovery copies</button><button class="btn" id="mcat-save-retry">Retry saving</button><button class="btn" id="mcat-conflict-reload">Reload saved work</button></div><p id="mcat-conflict-status" role="status"></p>';
      ...
      dialog.querySelector('#mcat-conflict-export').onclick=()=>{try{download();dialog.querySelector('#mcat-conflict-status').textContent='Recovery download prepared. It includes drafts still held by this tab.';}catch{dialog.querySelector('#mcat-conflict-status').textContent='Download could not be prepared. Keep this tab open and copy important notes.';}};
      dialog.querySelector('#mcat-conflict-reload').onclick=()=>{allowReload=true;location.reload();};
      dialog.querySelector('#mcat-save-retry').onclick=retry;
      document.body.appendChild(dialog);dialog.showModal();
    }
    dialog.querySelector('#mcat-conflict-title').textContent=kind==='conflict'?'Newer MCAT work was saved.':kind==='account'?'Your active account changed.':kind==='read'?'Saved work could not be read.':kind==='session'?'Your saved session could not open.':'Your latest work could not be saved.';
    dialog.querySelector('#mcat-conflict-description').textContent={
      conflict:'Another tab or account sync changed your progress. Saving is paused here so it cannot replace the newer copy.',
      account:'This tab cannot save into the newly active account. Its recovery download includes only copies already held by this tab.',
    ...
    const retryButton=dialog.querySelector('#mcat-save-retry');
  ...
  function retry(){
    ...
    try{
      ...
      window.dispatchEvent(new Event('mcat-storage-recovered'));return true;
    }catch{showProblem('write');if(dialog)dialog.querySelector('#mcat-conflict-status').textContent='Saving still failed. The pending copies remain in this tab.';return false;}
  }
  ...
  return {read,write,writeRaw,remove:key=>writeRaw(key,null),watch,retry,recovery,sessionFailed:()=>showProblem('session'),get conflicted(){return problem==='conflict'||problem==='account';},get paused(){return !!problem;}};
})();
