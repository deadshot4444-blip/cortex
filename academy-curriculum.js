...
(function (root, factory) {
  ...
  'use strict';
  // Control characters are rejected on purpose: return targets are re-rendered as links.
  ...
  function safeReturn(value) {
    ...
    try {
      ...
      if (
        url.origin !== 'https://cortex.invalid' ||
      ...
      if (
        ...
        [...url.searchParams.values()].some(v => v.length > 600 || UNSAFE_QUERY_VALUE.test(v))
      )
        return null;
  ...
  function validCard(card) {
    return (
      ...
      text(card.reviewStatus) &&
      Array.isArray(card.links) &&
      card.links.length >= 2 &&
      card.links.every(validEntry) &&
      Array.isArray(card.sources) &&
      card.sources.length > 0 &&
      card.sources.every(
        source => text(source.title) && typeof source.url === 'string' && /^https:\/\/[^\s<>"'`]+$/.test(source.url)
      )
    );
  }
  function validate(data) {
    if (
      ...
      new Set(data.objectives.map(o => o.id)).size !== data.objectives.length
    )
      throw Error('The curriculum index is incomplete.');
    const entries = new Map(data.entries.map(e => [e.id, e])),
      objectives = new Set(data.objectives.map(o => o.id));
    for (const entry of data.entries)
      if (entry.prerequisites.some(p => !entries.has(p)) || entry.objectives.some(o => !objectives.has(o)))
        throw Error('A curriculum reference is missing.');
    ...
    function walk(key) {
      ...
      visiting.add(key);
      entries.get(key).prerequisites.forEach(walk);
      visiting.delete(key);
      visited.add(key);
    }
    ...
    if (
      ...
      !data.connections.every(
        c =>
          ...
          entries.has(c.to) &&
          c.from !== c.to &&
          ['application', 'comparison', 'foundation'].includes(c.relation) &&
          text(c.why) &&
          text(c.limits)
      )
    )
      throw Error('A course connection is incomplete.');
    if (
      !Array.isArray(data.cards) ||
      !data.cards.every(c => validCard(c) && c.links.every(e => entries.has(e.id))) ||
      new Set(data.cards.map(c => c.id)).size !== data.cards.length
    )
      throw Error('A retrieval prompt is incomplete.');
    return data;
  }
    ...
    return data.entries
      .filter(
        e =>
          ...
          (filters.drafts || e.status === 'revised')
      )
      .map(entry => {
        const title = entry.title.toLowerCase(),
          summary = (entry.summary + ' ' + entry.objectives.map(o => names[o]).join(' ')).toLowerCase();
        return {
          entry,
          score: tokens.every(t => title.includes(t) || summary.includes(t))
            ? tokens.reduce((sum, t) => sum + (title.includes(t) ? 3 : 1), 0)
            : -1,
        };
      })
  ...
  function contextKey(value) {
    ...
    if (!safe) return '';
    const url = new URL(safe, 'https://cortex.invalid'),
      p = url.searchParams;
    if (url.pathname === '/practice' && p.get('view') === 'longitudinal' && p.get('case') && !p.get('run'))
    ...
    if (url.pathname === '/cogpsych' && p.get('view') === 'lesson' && p.get('lesson'))
      return 'cogpsych:' + p.get('lesson');
    if (['/anatomy', '/medicine'].includes(url.pathname) && p.get('lesson'))
      return (url.pathname === '/anatomy' ? 'anatomy:' : 'reference:') + p.get('lesson');
    if (url.pathname === '/medicine' && p.get('tool') === 'ecg' && p.get('mode') === 'library' && p.get('focus'))
      return 'ecg:' + p.get('focus');
    if (url.pathname === '/neuro')
      for (const key of ['unit', 'code', 'sim']) if (p.get(key)) return 'neuro-' + key + ':' + p.get(key);
    if (url.pathname === '/practice' && p.get('view') === 'content' && url.hash.startsWith('#case-'))
      return 'practice:' + url.hash.slice(6);
    return '';
  }
  function destination(entry, current, returnTo) {
    const safe = safeReturn(entry.url);
    if (!safe) return null;
    const url = new URL(safe, 'https://cortex.invalid'),
      origin = new URL(current, 'https://cortex.invalid');
    for (const key of ['gates', 'offline'])
    ...
    return (
      ...
      state.records.every(
        r =>
          ...
          validCard(r.content) &&
          time(r.queuedAt) &&
          ['pending', 'deferred', 'removed'].includes(r.status) &&
          typeof r.draft === 'string' &&
          r.draft.length <= 6000 &&
          typeof r.comparison === 'string' &&
          r.comparison.length <= 6000 &&
          (r.revealedAt == null ||
            (time(r.revealedAt) && text(r.firstDraft) && r.firstDraft.trim().length > 0 && r.draft === r.firstDraft)) &&
          (r.revealedAt != null || (r.firstDraft == null && !r.comparison && r.completedAt == null)) &&
          (r.completedAt == null ||
            (time(r.completedAt) && r.revealedAt != null && r.comparison.trim().length > 0 && r.status === 'pending'))
      )
    );
  }
  ...
  function edit(state, key, field, value) {
    ...
    if (
      ...
      record.completedAt ||
      !['draft', 'comparison'].includes(field) ||
      typeof value !== 'string' ||
      value.length > 6000 ||
      (field === 'draft' ? record.revealedAt != null : record.revealedAt == null)
    )
      return false;
    record[field] = value;
    return true;
  ...
  function reveal(state, key, now) {
    ...
    if (
      ...
      return false;
    record.firstDraft = record.draft;
  ...
  function move(state, key, status) {
    ...
    if (
      status === 'pending' &&
      state.records.some(
        r => r.id !== key && r.content.id === record.content.id && !r.completedAt && r.status === 'pending'
      )
    )
      return false;
