...
window.AcademyPortfolio = (() => {
  ...
  function load() {
    ...
    try { Core.validate(saved); state = saved; StudyStorage.watch(Core.KEY, () => state); }
    catch (error) { StudyStorage.sessionFailed(); throw error; }
  }
  function access() {
    if (StudyStorage.paused) throw Error('Resolve the saved-work problem before changing or exporting the portfolio. Your copies remain available in recovery.');
    if (!window.CortexAccount?.available) throw Error('The active workspace is not ready. Reload after signing in or continuing as a guest.');
    const data = CortexAccount.snapshot().data;
    const actual = data[Core.KEY] == null ? Core.empty() : JSON.parse(data[Core.KEY]);
    if (state && JSON.stringify(actual) !== JSON.stringify(state)) throw Error('Your portfolio changed in another tab. Reload to inspect the current copy.');
    return data;
  }
  function save() { Core.validate(state); return StudyStorage.write(Core.KEY, state); }
  function render(track = 'all') {
    const current = ++generation, root = el('<div></div>'); root.appendChild(topbar('academy'));
    const main = el(`<main class="panel academy-shell academy-portfolio"><div class="academy-intro"><span class="label">Your private portfolio</span><h1>Keep work you want to revisit.</h1>
    ...
    const status = message => { if (active()) main.querySelector('#portfolio-status').textContent = message; };
    const action = fn => { if (!active()) return; try { access(); fn(); } catch (error) { status(error.message); } };
    let candidates;
    try { load(); candidates = Core.candidates(access()); }
    catch (error) { status(error.message); return; }
    const selected = new Set(), host = main.querySelector('#portfolio-body');
    ...
    host.innerHTML = `<section aria-labelledby="portfolio-add-title"><h2 id="portfolio-add-title">Choose from saved work</h2>
      ...
      <label for="portfolio-track">Course</label><select id="portfolio-track"><option value="all">All courses</option>${Core.TRACKS.filter(id => CortexAcademy.tracks.some(track => track.id === id)).map(id => `<option value="${id}">${esc(CortexAcademy.tracks.find(t => t.id === id)?.name || id)}</option>`).join('')}</select>
    ...
    function drawCandidates() {
      ...
      host.querySelector('#portfolio-candidates').innerHTML = available.filter(c => track === 'all' || c.track === track).map(c => `<article class="portfolio-source"><h3>${esc(c.title)}</h3><p>${esc(c.kind)} · ${c.provenance.completedAt === null ? 'In progress or completion unrecorded' : 'Participation completion recorded'}</p><p>${c.evidence.length} recorded fields · ${esc(c.provenance.contentSnapshot)}</p><details><summary>Inspect the saved work before adding</summary>${evidenceMarkup(c)}</details><button class="btn" data-add="${esc(c.id)}">Keep this entry</button></article>`).join('');
      host.querySelectorAll('[data-add]').forEach(button => button.onclick = () => action(() => {
        ...
        if (JSON.stringify(latest) !== JSON.stringify(source)) throw Error('The source changed since you opened it. Reopen the portfolio to inspect its latest wording.');
    ...
    host.querySelector('#portfolio-track').onchange = drawCandidates; drawCandidates();
    const entries = host.querySelector('#portfolio-entries');
    entries.innerHTML = state.entries.map((entry, index) => `<article class="portfolio-entry" data-entry="${index}"><h3>${esc(entry.source.title)}</h3>
      ...
      ${entry.hiddenAt === null ? `<label for="portfolio-draft-${index}">Add a separate reflection</label><textarea id="portfolio-draft-${index}" data-draft maxlength="6000" rows="3">${esc(entry.draft)}</textarea>
      ...
      <label class="portfolio-select"><input type="checkbox" data-select> Include this entry and its recorded reflections in the export</label>` : '<button class="btn" data-restore>Restore entry</button>'}</article>`).join('') || '<p>Your portfolio is empty. Choose a saved entry above to begin.</p>';
    entries.querySelectorAll('[data-entry]').forEach(card => {
      ...
      if (input) input.oninput = () => action(() => { entry.draft = input.value; if (save()) { card.querySelector('[data-revise]').disabled = !input.value.trim(); clearPreview(); } });
      card.querySelector('[data-revise]')?.addEventListener('click', () => action(() => { if (Core.revise(entry, card.querySelector('[data-help]').value) && save()) render(host.querySelector('#portfolio-track').value); }));
      card.querySelector('[data-hide]')?.addEventListener('click', () => action(() => { entry.hiddenAt = Date.now(); if (save()) render(host.querySelector('#portfolio-track').value); }));
      card.querySelector('[data-restore]')?.addEventListener('click', () => action(() => { entry.hiddenAt = null; if (save()) render(host.querySelector('#portfolio-track').value); }));
      card.querySelector('[data-select]')?.addEventListener('change', event => { event.target.checked ? selected.add(entry.id) : selected.delete(entry.id); clearPreview(); host.querySelector('#portfolio-prepare').disabled = !selected.size; });
    });
    function clearPreview() { host.querySelector('#portfolio-preview').replaceChildren(); }
    host.querySelector('#portfolio-prepare').onclick = () => action(() => {
      const file = Core.exportFile(state, [...selected]);
      const panel = el('<section class="portfolio-export"><h3>Inspect the exact export</h3><p>Only the selected entries and recorded portfolio reflections are included. Unrecorded reflection drafts, account details and other study records are excluded. This unencrypted JSON file is not published or sent anywhere by Cortex.</p><pre tabindex="0" aria-label="Complete export contents"></pre><label class="portfolio-select"><input type="checkbox" data-reviewed> I reviewed these contents.</label><div class="academy-lesson-nav"><button class="btn btn-solid" data-download disabled>Download this JSON</button><button class="btn" data-cancel>Cancel</button></div></section>');
      panel.querySelector('pre').textContent = file;
      panel.querySelector('[data-reviewed]').onchange = event => { panel.querySelector('[data-download]').disabled = !event.target.checked; };
      panel.querySelector('[data-cancel]').onclick = clearPreview;
      panel.querySelector('[data-download]').onclick = () => action(() => {
        ...
        const link = document.createElement('a'); link.href = url; link.download = 'cortex-selected-portfolio.json'; link.click();
        setTimeout(() => URL.revokeObjectURL(url), 1000); status('Download prepared for the exact preview. Check that the file reached your Downloads folder.');
      });
      host.querySelector('#portfolio-preview').replaceChildren(panel);
    });
  }
  function helpLabel(value) { return ({ used: 'reported help used', 'none-recorded': 'reported no help', unknown: 'unspecified' })[value]; }
  ...
  window.addEventListener('study-storage-recovered', () => { if (document.querySelector('.academy-portfolio')) render(); });
  window.addEventListener('study-storage-paused', () => { const root = document.querySelector('.academy-portfolio'); if (root) { root.querySelector('#portfolio-preview')?.replaceChildren(); root.querySelectorAll('button,input,select,textarea').forEach(node => { node.disabled = true; }); } });
  return { render };
})();
