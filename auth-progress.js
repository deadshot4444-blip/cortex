...
(function (root, factory) {
  ...
  const OWNER = 'cortex-progress-owner-v1',
    JOURNAL = 'cortex-progress-transaction-v1';
  const META = 'cs-sync-meta',
    DIRTY = 'cs-sync-dirty';
  ...
  function create({ storage, client, onState = () => {}, onReload = () => {}, onBlocked = () => {} }) {
    ...
    function replace(snapshot) {
      ...
      if (snapshot.dirty) storage.setItem(DIRTY, snapshot.dirty);
      else storage.removeItem(DIRTY);
    }
    ...
    const meta = () => {
      const m = read(META);
      return m?.owner === owner.id ? m : null;
    };
    ...
    function block(message, kind = 'storage') {
      paused = true;
      clearTimeout(timer);
      setState('paused');
      onBlocked(message, kind);
    }
    ...
    function transition(id, next) {
      ...
      try {
        ...
        try {
          replace(read(backupKey));
          put(OWNER, owner);
          storage.removeItem(JOURNAL);
        } catch {}
        block(
          'Could not replace saved work. Download a recovery copy, restore browser storage access, then reload to recover the previous workspace.'
        );
        throw error;
      }
      onReload();
    ...
    async function pull(id) {
      ...
      if (typeof data.updated_at !== 'string' || !Number.isFinite(Date.parse(data.updated_at)))
        throw Error('Invalid cloud revision');
      return { data: validData(data.data), updated_at: data.updated_at };
    }
    function saveBase(row) {
      put(META, { owner: owner.id, known: true, revision: row?.updated_at || null });
    }
    function sameData(a, b) {
      const keys = Object.keys(a);
      return keys.length === Object.keys(b).length && keys.every(k => a[k] === b[k]);
    }
    function detectConflict(row) {
      conflict = row;
      setState('conflict');
    }
    ...
    async function push(id, generation) {
      ...
      if (base.revision) {
        result = await client
          ...
          .eq('user_id', id)
          .eq('updated_at', base.revision)
          .select('updated_at')
          .maybeSingle();
      } else {
        result = await client
          .from('progress')
          .insert({ user_id: id, data, updated_at })
          .select('updated_at')
          .maybeSingle();
      }
      if (!alive(id, generation)) return;
      ...
      if (typeof result.data?.updated_at !== 'string' || !Number.isFinite(Date.parse(result.data.updated_at)))
    ...
    async function sync() {
      ...
      const operation = (async () => {
        try {
          await reconcile(id, generation);
        } catch {
          if (alive(id, generation)) setState('error');
        }
      })();
      running = operation;
      try {
    ...
    async function setUser(next) {
      ...
      if (owner.id !== id) {
        try {
          transition(id, read(archiveKey(id)) || { data: {}, meta: null });
        } catch {
          block('Saved work could not be switched. Download a recovery copy before reloading.');
        }
        return;
      }
      if (user) await sync();
    ...
    function recovery() {
      ...
      return {
        ...
        device,
        cloud: conflict,
        previousDeviceCopy: read(archiveKey(owner.id)),
        previousCloudCopy: read('cortex-progress-cloud-copy-v1:' + owner.id),
        guest: read(archiveKey('guest')),
      };
    }
    function resolve(choice) {
      ...
      if (choice === 'cloud') {
        transition(owner.id, {
          data: conflict?.data || {},
          meta: { owner: owner.id, known: true, revision: conflict?.updated_at || null },
        });
    ...
    function useGuest() {
      ...
      if (!owns() || !user || !guest || !meta()?.known || ['conflict', 'syncing', 'error'].includes(state))
        return false;
      transition(owner.id, { data: guest.data, meta: meta(), dirty: token() });
      return true;
    }
    ...
    function prepareRestore(data) {
      ...
      if (running || state === 'syncing')
        throw Error('Saving to your account is still in progress. Try again in a moment.');
      const next = validData(data);
      if (Object.keys(next).length !== Object.keys(data).length)
        throw Error('The backup contains unsupported storage keys.');
      const before = gather(),
        keys = [...new Set([...Object.keys(before), ...Object.keys(next)])].sort();
    ...
    function restore(preview) {
      ...
      if (
        ...
        !owns() ||
        running ||
        ['syncing', 'conflict'].includes(state) ||
        !sameData(saved.before, gather())
      ) {
    ...
    return {
      ...
      restore,
      checkOwner() {
        if (!owns()) block('The active workspace changed in another tab. Reload before continuing.', 'workspace');
      },
      get state() {
        return state;
      },
      ...
      get hasGuest() {
        return !!read(archiveKey('guest')) && Object.keys(read(archiveKey('guest')).data).length > 0;
      },
      stop() {
        clearTimeout(timer);
        epoch++;
