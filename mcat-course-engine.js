...
(function (root) {
  ...
  function validHelpContent(content,unitId) {
    ...
    return !!HELP_SOURCES[unitId] && content?.version === 1 && content.source?.url === HELP_SOURCES[unitId] &&
      text(content.source.title) && Array.isArray(content.stages) && content.stages.length === 2 &&
      content.stages.every(s => text(s?.title) && text(s?.text));
  }
  function validHelpGuide(guide,unitId,questionId) {
    return guide?.unitId === unitId && guide.questionId === questionId && validHelpContent(guide.content,unitId) &&
      Number.isFinite(guide.openedAt) && Array.isArray(guide.reveals) && guide.reveals.length <= 2 &&
      guide.reveals.every((n,i)=>Number.isFinite(n) && n >= (i ? guide.reveals[i-1] : guide.openedAt)) &&
      typeof guide.draft === 'string' && guide.draft.length <= 6000 && typeof guide.reflection === 'string' && guide.reflection.length <= 6000 &&
      (guide.reveals.length ? typeof guide.firstNote?.text === 'string' && guide.firstNote.text.length <= 6000 && guide.firstNote.recordedAt === guide.reveals[0] : guide.firstNote === null);
  }
  function helpQuestion(unit,questionId) {
    const content=unit.authoredHelp, check=content?.checks?.[questionId];
    const text=x=>typeof x==='string' && x.trim().length>0 && x.length<=2000;
    if (validHelpContent(content,unit.id) && content.questionIds?.includes(questionId) &&
        check?.id===questionId+'-help' && check.kind==='support' && text(check.stem) && text(check.explanation) &&
        Array.isArray(check.options) && check.options.length>=2 && check.options.length<=6 && check.options.every(text) &&
        Number.isInteger(check.answer) && check.answer>=0 && check.answer<check.options.length) return check;
  ...
  function openHelp(state,unit,questionId,now) {
    const r = record(state,unit.id), original = r.attempts.find(a => a.qId === questionId);
    ...
    r.help ||= {};
    if (r.help[questionId] != null && (typeof r.help[questionId] !== 'object' || Array.isArray(r.help[questionId]))) return null;
    const help = r.help[questionId] ||= {questionSnapshot:snapshot(check),chosen:null,openedAt:now};
    if (!help.guide && validHelpContent(unit.authoredHelp,unit.id) && unit.authoredHelp.questionIds?.includes(questionId)) {
      const content = unit.authoredHelp;
      help.guide = {unitId:unit.id,questionId,openedAt:now,
        content:{version:content.version,source:{title:content.source.title,url:content.source.url},
          stages:content.stages.map(s=>({title:s.title,text:s.text}))},
        reveals:[],draft:'',firstNote:null,reflection:''};
    }
    return help;
  }
  function revealHelp(guide,now) {
    if (!validHelpGuide(guide,guide?.unitId,guide?.questionId) || guide.reveals.length >= guide.content.stages.length ||
      !Number.isFinite(now) || now < Math.max(guide.openedAt,...guide.reveals) || typeof guide.draft !== 'string' || guide.draft.length > 6000) return false;
    if (!guide.reveals.length) guide.firstNote = {text:guide.draft,recordedAt:now};
    guide.reveals.push(now);return true;
  }
  ...
  function recommendation(data,state,logs,now) {
    ...
    const scored = candidates.map((u,i) => {
      ...
      const score = (p && p.chosen !== null && !p.correct ? (p.confidence === 'sure' ? 8 : 6) : 0) + Math.min(5,misses) + (state.placement.background[u.section] === 'new' ? 1 : 0);
    ...
    if (ready.id !== top.unit.id) return {unit:ready,kind:'lesson',reason:`${top.unit.title} builds on this foundation. Complete it first, or choose a lesson directly if your prior background covers it.`};
  ...
  const api = {DAY,normalize,record,answer,complete,recommendation,metrics,nextDelayed,refreshSchedule,snapshot,questionForRecord,validHelpContent,validHelpGuide,helpQuestion,openHelp,revealHelp};
