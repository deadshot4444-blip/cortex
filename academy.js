...
(() => {
  ...
  const tracks = [
    {
      id: 'mcat', path: 'mcat', name: 'MCAT preparation', available: true,
      ...
      scope: '45 foundation lessons, a coverage map, passage coaching, math practice, investigations, and a weekly planner.',
      time: 'Start with 15 minutes', action: 'Open MCAT', kind: 'Exam preparation',
    },
    {
      id: 'socrates', path: 'learn', name: 'Learn to Learn', available: true,
      ...
      time: '12–20 minutes per lesson', action: 'Explore learning methods', kind: 'Learning methods',
    },
    {
      id: 'practice', path: 'practice', name: 'Clinical Scenarios', available: true,
      ...
      time: 'One simulated encounter at a time', action: 'Explore clinical cases', kind: 'Clinical reasoning',
    },
    {
      id: 'anatomy', path: 'anatomy', name: 'Anatomy', available: true,
      ...
      scope: 'Twelve lessons connect foundations with regional anatomy and basic neuroanatomy; the separate atlas explorers retain draft status.',
      time: 'One region or system at a time', action: 'Explore anatomy', kind: 'Foundational science',
    },
    {
      id: 'reference', path: 'medicine', name: 'Medicine', available: true,
      ...
      scope: 'Twenty-two lessons connect physiology, pharmacology, microbiology, ECGs and laboratory interpretation, alongside the separately labeled 81-step draft reference pathway.',
      time: 'One topic or pathway step at a time', action: 'Explore medicine', kind: 'Clinical foundations',
    },
    {
      id: 'neuro', path: 'neuro', name: 'Neuroengineering', available: true,
      ...
      scope: 'Five revised foundation units within a 20-unit draft path; 13 checked coding exercises, 15 simulations, and six project entries. Independent subject review is pending.',
      time: 'Start with the foundations', action: 'Explore neuroengineering', kind: 'Engineering',
    },
  ].map(Object.freeze);
  ...
  function renderCatalog() {
    ...
    const main = el(`<main class="panel academy-shell">
      ...
      <div class="academy-catalog" aria-label="Learning paths">
        ${tracks.map(track => `<article class="academy-course">
    ...
    main.querySelectorAll('#academy-open-today, [data-academy-view]').forEach(link => link.onclick = event => {
      if (event.metaKey || event.ctrlKey || event.shiftKey || event.altKey) return;
      event.preventDefault(); history.pushState({}, '', event.currentTarget.getAttribute('href')); openSection('academy');
    });
