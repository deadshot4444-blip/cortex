...
async function loadMcatCourse() {
  ...
  try {
    const r = await fetch('data/mcat-course.json?v=11');
    if (r.ok) {
      const d = await r.json();
      if (Array.isArray(d.units) && Array.isArray(d.categories)) courseData = d;
      if (courseData) {
...
function saveCourse() {
  try {
    courseSaveFailed = !courseStore.save(courseState);
    return !courseSaveFailed;
  } catch {
    courseSaveFailed = true;
    const status = document.querySelector('#course-note-status');
    if (status) status.textContent = 'Storage is unavailable. Keep this tab open and copy important notes.';
    return false;
  }
}
function courseUnit(id) {
  return courseData?.units.find(u => u.id === id);
}
function courseRecord(id) {
  return McatCourseCore.record(courseState, id);
}
function courseRec() {
  return courseData && McatCourseCore.recommendation(courseData, courseState, QLOG, nowTs());
}
...
function mcatWorkspace(root, view) {
  courseRoute(view);
  const nav = el(
    `<nav class="course-nav" aria-label="MCAT workspace"><span class="course-nav-brand">MCAT<span> / YOUR WORKSPACE</span></span><div>${['today', 'course', 'practice', 'progress'].map(v => `<button data-course-view="${v}" ${v === view ? 'aria-current="page"' : ''}>${v[0].toUpperCase() + v.slice(1)}</button>`).join('')}</div></nav>`
  );
  ...
  if (main) mcatDataNotice(main);
  return root;
}
function courseView(main, view = 'course', unit) {
  const root = el('<div></div>');
  root.appendChild(topbar(view === 'progress' ? 'stats' : 'mcat'));
  root.appendChild(main);
  mcatWorkspace(root, view);
  if (unit) courseRoute(view, unit);
  if (courseSaveFailed)
    main.prepend(
      el(
        '<p class="course-notice" role="alert">This browser could not save your course work. Keep this tab open and copy important notes before leaving.</p>'
      )
    );
  if (main.classList.contains('course-lesson') || main.dataset.v2Activity || main.dataset.study)
    courseStudyChrome(root);
  studySetView(root);
}
function courseStudyChrome(root) {
  ...
  const header = el(
    '<header class="course-study-header"><a class="skip-link" href="#main">Skip to content</a><span>MCAT <small>/ STUDY</small></span><button class="btn" aria-expanded="false" aria-controls="course-study-menu">Study menu</button></header>'
  );
  const toggle = header.querySelector('button');
  toggle.onclick = () => {
    menus.hidden = !menus.hidden;
    toggle.setAttribute('aria-expanded', String(!menus.hidden));
  };
  menus.onkeydown = e => {
    if (e.key === 'Escape') {
      menus.hidden = true;
...
function courseToday(host, compact = false) {
  if (!courseData) {
    host.innerHTML =
      '<div class="course-notice"><strong>Course is temporarily unavailable.</strong><p>Your existing study tools and saved work are available. Reload to retry loading lessons.</p></div>';
    return;
  }
  const rec = courseRec(),
    m = McatCourseCore.metrics(courseData, courseState);
...
function courseModeMarkup() {
  return `<section class="course-mode"><div><span class="label">Study stage</span><p>Choose what to emphasize. Your saved work and any target date stay in place.</p></div><div class="course-mode-options">${[
    ['learn', 'Learn', 'Lessons + recall'],
    ['mixed', 'Apply', 'Mixed passage practice'],
    ['exam', 'Rehearse', 'Timed sections + review'],
  ]
    .map(
      ([id, title, desc]) =>
        `<button data-course-mode="${id}" aria-pressed="${courseState.mode === id}"><strong>${title}</strong><small>${desc}</small></button>`
    )
    .join('')}</div></section>`;
}
function wireCourseModes(main, rerender) {
  main.querySelectorAll('[data-course-mode]').forEach(
    b =>
      (b.onclick = () => {
        courseState.mode = b.dataset.courseMode;
        saveCourse();
        const plan = guidePlan();
        if (plan) studyDailySession(plan, plan.dailyMinutes || 30);
        rerender();
      })
  );
}
function renderCourseHome() {
  ...
  if (!courseData) {
    const main = el(
      '<main class="course-page"><h1>Course unavailable</h1><p>Lessons could not load. Your saved progress is unchanged.</p><button class="btn" id="course-retry">Retry</button><button class="btn" data-course-view="practice">Open practice</button></main>'
    );
    main.querySelector('#course-retry').onclick = async () => {
      await loadMcatCourse();
      renderCourseHome();
    };
    main.querySelector('[data-course-view]').onclick = () => courseGo('practice');
    courseView(main);
    return;
  }
  const m = McatCourseCore.metrics(courseData, courseState),
    rec = courseRec();
  const main =
    el(`<main class="course-page"><header class="course-hero"><div><span class="course-eyebrow">CORTEX / MCAT 2.0</span><h1>Understand it.<br><em>Then put it to work.</em></h1><p>A clear path through the science and reasoning behind the test. Read, predict, explore, explain, and apply.</p><div class="course-actions"><button class="btn btn-solid" id="course-continue">${rec ? 'Continue your course →' : 'Go to practice →'}</button><button class="btn" id="placement">Find my starting point</button></div></div><div class="course-hero-index"><span>THE CONNECTED COURSE</span><strong>${m.available}<span>lessons</span></strong><div>${courseData.chapters.length} chapters · 04 sections<br>03 interactive science labs<br>${courseData.units.reduce((n, u) => n + u.questions.length, 0)} original checks</div></div></header>
  ...
  main.querySelectorAll('[data-course-filter]').forEach(
    b =>
      (b.onclick = () => {
...
function courseStudyTaskDone(r, unitId) {
  const plan = guidePlan(),
    task =
      r.guideTask ||
      plan?.sessions?.[guideDateKey()]?.tasks.find(
        t => t.type === 'course' && t.unitId === unitId && !guideTaskDone(plan, t.day, t.id)
      );
  if (plan && task) {
    plan.completed ||= {};
...
function renderCourseUnit(id, requested) {
  const u = courseUnit(id);
  if (!u) return renderCourseHome();
  coursePauseTools();
  if (typeof v2BeginActivity === 'function') v2BeginActivity('course', id);
  const r = courseRecord(id);
  r.startedAt ||= nowTs();
  if (!r.completedAt) courseState.activeUnit = id;
  if (requested) r.stage = requested;
  else if (r.dueAt && r.dueAt <= nowTs()) {
    r.stage = 'delayed';
    if (r.feedback !== McatCourseCore.nextDelayed(courseState, u)?.id) delete r.feedback;
  }
  const stage = r.stage;
  saveCourse();
  const tabs = [
  ...
  if (stage === 'learn') {
    ...
    body.querySelector('#course-learned').onclick = () => {
      r.learnedAt ||= nowTs();
      if (r.completedAt) {
        r.reviewedAt = nowTs();
        if (
          r.guideTask?.courseKind === 'review' ||
          (typeof v2State !== 'undefined' &&
            v2State.weekly.active?.type === 'course' &&
            v2State.weekly.active.key === id &&
            v2State.weekly.active.kind === 'review')
        )
          courseStudyTaskDone(r, id);
      }
      saveCourse();
      renderCourseUnit(id, 'explore');
    };
  } else if (stage === 'explore' && r.learnedAt) courseExplore(body, u, r);
  else if (stage === 'check' && r.exploredAt) {
    const q = u.questions.find(q => q.id === r.feedback && q.kind === 'check');
    q ? courseAnswerFeedback(body, u, r, q, false) : courseCheckpoint(body, u, r, false);
  } else if (
    stage === 'delayed' &&
    ((r.feedback && r.attempts.some(a => a.qId === r.feedback && a.kind === 'delayed')) ||
      (r.dueAt && r.dueAt <= nowTs() && r.completedAt))
  ) {
    const q = u.questions.find(q => q.id === r.feedback && q.kind === 'delayed');
    q ? courseAnswerFeedback(body, u, r, q, true) : courseCheckpoint(body, u, r, true);
  } else courseLearningRecord(body, u, r);
  if (stage !== 'record') {
    main.dataset.v2Activity = 'course';
    main.dataset.v2Key = id;
  }
...
function courseExplore(body, u, r) {
  ...
  const input = body.querySelector('#course-prediction'),
    button = body.querySelector('#course-reveal');
  input.oninput = () => {
    lab.prediction = input.value;
    saveCourse();
    button.disabled = !input.value.trim();
  };
  const reveal = () => {
    const host = body.querySelector('#course-experiment');
    host.hidden = false;
    button.hidden = true;
    ...
    host.querySelector('#course-reflection').oninput = e => {
      lab.reflection = e.target.value;
      saveCourse();
      host.querySelector('#course-explored').disabled = !!u.lab && !e.target.value.trim();
    };
    host.querySelector('#course-explored').onclick = () => {
      r.exploredAt ||= nowTs();
      saveCourse();
      renderCourseUnit(u.id, 'check');
    };
  };
  button.onclick = () => {
    lab.shown = true;
...
function courseCheckpoint(body, u, r, delayed) {
  ...
  let q = delayed
    ? McatCourseCore.nextDelayed(courseState, u)
    : qs.find(q => !r.attempts.some(a => a.qId === q.id)) || qs.at(-1);
  ...
  let chosen = r.draft?.qId === q.id ? r.draft.chosen : null,
    conf = r.draft?.qId === q.id ? r.draft.confidence : 'unsure';
  body.innerHTML = `<section class="course-check"><span class="label">${delayed ? 'Later application · new question' : `Application ${qs.findIndex(x => x.id === q.id) + 1} of ${qs.length}`}</span><h2>${esc(q.stem)}</h2><fieldset class="course-answers"><legend>Choose one answer</legend>${q.options.map((o, i) => `<label class="${a && i === q.answer ? 'course-correct' : ''}"><input type="radio" name="course-answer" value="${i}" ${a ? (a.chosen === i ? 'checked' : '') : chosen === i ? 'checked' : ''} ${a ? 'disabled' : ''}><span>${'ABCD'[i]}. ${esc(o)}</span></label>`).join('')}</fieldset>${
    a
      ? ''
      : `<label for="course-confidence">How sure are you?</label><select id="course-confidence">${Object.entries(CONF)
          .map(([v, n]) => `<option value="${v}" ${conf === v ? 'selected' : ''}>${n}</option>`)
          .join(
            ''
          )}</select><button class="btn btn-solid" id="course-submit" ${chosen === null ? 'disabled' : ''}>Check answer</button>`
  }${a ? `<div class="course-feedback" role="status"><strong>${a.correct ? 'Correct' : 'Review this distinction'}</strong><p>${esc(q.explanation)}</p><small>First answer saved. Reopening this explanation does not add another result.</small></div><button class="btn btn-solid" id="course-check-next">${qs.some(x => !r.attempts.some(a => a.qId === x.id)) ? 'Next application →' : 'Open learning record →'}</button>` : ''}</section>`;
  const saveDraft = () => {
    r.draft = { qId: q.id, chosen, confidence: conf, questionSnapshot: McatCourseCore.snapshot(q) };
    saveCourse();
  };
  if (!a) saveDraft();
  body.querySelectorAll('input[name="course-answer"]').forEach(
    i =>
      (i.onchange = () => {
        chosen = Number(i.value);
        saveDraft();
        body.querySelector('#course-submit').disabled = false;
      })
  );
  body.querySelector('#course-confidence')?.addEventListener('change', e => {
    conf = e.target.value;
    saveDraft();
  });
  body.querySelector('#course-submit')?.addEventListener('click', () => {
    const result = McatCourseCore.answer(courseState, u, q, chosen, conf, nowTs());
    if (!result) return;
    delete r.draft;
    if (delayed || McatCourseCore.complete(courseState, u, nowTs())) courseStudyTaskDone(r, u.id);
    r.stage = delayed ? 'delayed' : 'check';
    saveCourse();
  ...
  body
    .querySelector('#course-check-next')
    ?.addEventListener('click', () =>
      renderCourseUnit(
        u.id,
        qs.some(x => !r.attempts.some(a => a.qId === x.id)) ? (delayed ? 'delayed' : 'check') : 'record'
      )
    );
}
function courseAnswerFeedback(body, u, r, q, delayed) {
  const a = r.attempts.find(a => a.qId === q.id);
  if (!a) return renderCourseUnit(u.id, 'record');
  ...
  courseFocusPrompt(body.querySelector('h2'));
  body.querySelector('#course-question-help')?.addEventListener('click', () => {
    r.helpOpen = q.id;
    saveCourse();
    courseQuestionHelp(body, u, r, q, delayed);
  });
  body.querySelector('#course-check-next').onclick = () => {
    delete r.helpOpen;
    delete r.feedback;
    saveCourse();
    const pending = u.questions.some(x => x.kind === 'check' && !r.attempts.some(a => a.qId === x.id));
    renderCourseUnit(u.id, pending ? 'check' : 'record');
  };
}
function courseQuestionHelp(body, u, r, q, delayed) {
  const original = r.attempts.find(a => a.qId === q.id);
  if (!original) {
    body.innerHTML = '<p>Save your lesson answer before opening its help.</p>';
    return;
  }
  q = McatCourseCore.questionForRecord(q, original);
  const help = McatCourseCore.openHelp(courseState, u, q.id, nowTs());
  const retry = () => {
    if (body.isConnected) courseQuestionHelp(body, u, r, q, delayed);
  };
  const failed = () => {
    window.addEventListener('study-storage-recovered', retry, { once: true });
  };
  if (!help || (help.guide && !McatCourseCore.validHelpGuide(help.guide, u.id, q.id))) {
    body.innerHTML = '<p>The saved help could not be opened. Its original copy is available through recovery.</p>';
    StudyStorage.sessionFailed();
    return;
  }
  if (StudyStorage.paused || !saveCourse()) {
    body.innerHTML = '<p>Your help is waiting for saved work to recover. Keep this tab open.</p>';
    failed();
    return;
  }
  const check =
    help.questionSnapshot ||
    McatCourseCore.questionForRecord(
      u.questions.find(item => item.kind === 'diagnostic'),
      help
    );
  ...
  if (guide) {
    const host = body.querySelector('#course-authored-help'),
      count = guide.reveals.length;
    host.innerHTML = `<section class="course-worked"><h3>Work through the idea</h3><p class="course-caption">Two authored hints from this lesson's topic. This is a short review, not an assessment of your reasoning. Independent content review is pending.</p>${count ? `<p><b>Your note before hints:</b> ${esc(guide.firstNote.text || 'No note entered.')}</p>` : `<label for="course-help-note">Before the hints, what distinction might you have missed? (optional)</label><textarea id="course-help-note" maxlength="6000" rows="3">${esc(guide.draft)}</textarea>`}<div id="course-help-stages" aria-live="polite">${guide.content.stages
      .slice(0, count)
      .map(
        (stage, i) =>
          `<article><h4>${i + 1}. ${esc(stage.title)}</h4><p>${esc(stage.text)}</p><a href="${esc(guide.content.source.url)}" target="_blank" rel="noopener noreferrer">${esc(guide.content.source.title)} ↗</a></article>`
      )
      .join(
        ''
      )}</div>${count < guide.content.stages.length ? `<button class="btn" id="course-help-reveal">${count ? 'Reveal next hint' : 'Save note & reveal first hint'}</button>` : ''}${count ? `<label for="course-help-reflection">After the hints, what would you change? (optional)</label><textarea id="course-help-reflection" maxlength="6000" rows="3">${esc(guide.reflection)}</textarea>` : ''}<p class="course-caption">Your note, opened hints and later reflection stay separate from the original answer. This work follows your active guest or account workspace.</p><p id="course-help-save" role="status">Help record saved.</p></section>`;
    for (const [selector, key] of [
      ['#course-help-note', 'draft'],
      ['#course-help-reflection', 'reflection'],
    ])
      host.querySelector(selector)?.addEventListener('input', event => {
        if (!body.isConnected || StudyStorage.paused) return;
        guide[key] = event.target.value;
        host.querySelector('#course-help-save').textContent = persist()
          ? 'Help record saved.'
          : 'This note is held in this tab. Saving needs recovery.';
      });
    host.querySelector('#course-help-reveal')?.addEventListener('click', () => {
      if (!ready() || !McatCourseCore.revealHelp(guide, nowTs())) return;
      if (persist()) {
        courseQuestionHelp(body, u, r, q, delayed);
        courseFocusPrompt(body.querySelector('#course-help-stages article:last-child h4'));
      }
    });
  }
  body.querySelectorAll('[name="course-help-answer"]').forEach(input => {
    input.disabled = !!help.answeredAt;
    input.onchange = () => {
      if (!body.isConnected || StudyStorage.paused || help.answeredAt) return;
      help.chosen = Number(input.value);
      if (persist()) body.querySelector('#course-help-check').disabled = false;
    };
  });
  body.querySelector('#course-help-check').onclick = () => {
    if (!ready() || help.answeredAt || help.chosen === null) return;
    help.answeredAt = nowTs();
    if (persist()) courseQuestionHelp(body, u, r, q, delayed);
  };
  body.querySelector('#course-help-return').onclick = () => {
    if (!ready()) return;
    delete r.helpOpen;
    if (persist()) courseAnswerFeedback(body, u, r, q, delayed);
  };
...
function courseLearningRecord(body, u, r) {
  const mapped = QLOG.filter(x => u.questionIds.includes(x.qId)),
    missed = mapped.filter(x => !x.correct);
  ...
  const plan = guidePlan(),
    weekly = typeof v2State !== 'undefined' && v2State.weekly.configured;
  ...
  ${
    r.attempts.length
      ? `<details class="course-map"><summary>Review saved answers</summary>${r.attempts
          .map(a => {
            const q = McatCourseCore.questionForRecord(
              u.questions.find(q => q.id === a.qId),
              a
            );
            return q
              ? `<article class="course-saved-answer"><span class="label">${a.kind === 'delayed' ? 'Later application' : 'First application'} · ${a.correct ? 'Correct' : 'Missed'} · ${esc(CONF[a.confidence] || 'Unsure')}</span><h3>${esc(q.stem)}</h3><p>You: ${esc(q.options[a.chosen])}</p><p>Answer: ${esc(q.options[q.answer])}</p><p>${esc(q.explanation)}</p>${r.help?.[a.qId] ? `<button class="btn" data-course-saved-help="${esc(a.qId)}">Review saved help</button>` : ''}</article>`
              : '';
          })
          .join('')}</details>`
      : ''
  ...
  body.querySelectorAll('[data-course-saved-help]').forEach(
    button =>
      (button.onclick = () => {
        if (StudyStorage.paused || !saveCourse()) return;
        const a = r.attempts.find(a => a.qId === button.dataset.courseSavedHelp);
        if (!a) return;
        r.feedback = a.qId;
        r.helpOpen = a.qId;
        if (saveCourse()) renderCourseUnit(u.id, a.kind === 'delayed' ? 'delayed' : 'check');
      })
  );
  body.querySelector('#course-later')?.addEventListener('click', () => {
    delete r.feedback;
    renderCourseUnit(u.id, 'delayed');
  });
  body.querySelector('#course-today').onclick = () => courseGo('today');
  body.querySelector('#course-focus').onclick = () => {
    courseState.preferredUnit = u.id;
    saveCourse();
    renderCourseUnit(u.id, r.completedAt ? 'learn' : r.stage);
  };
  body.querySelector('#course-cards')?.addEventListener('click', () => {
    const saved = loadResume('flash');
    if (saved) {
      RESUME_SPECS.find(s => s.key === 'flash').resume(saved);
      return;
    }
    guideClearActiveTask();
    flash = { deck: u.section, queue: cards, idx: 0, total: cards.length, again: 0, done: 0 };
    renderFlashCard();
  });
  body.querySelectorAll('[data-course-passage]').forEach(
    b =>
      (b.onclick = () => {
        guideClearActiveTask();
        const p = passages.find(p => p.id === b.dataset.coursePassage);
        u.section === 'cars' ? startCars(p, false) : startPassage(p, false);
      })
  );
  body.querySelectorAll('[data-course-repair]').forEach(
    b =>
      (b.onclick = () => {
        const c = repairConcept(b.dataset.courseRepair);
        if (!repairData || !repairState || !c) return renderRepairHub();
        if (!repairState.active) McatRepairCore.begin(c, repairState, 'repair', nowTs());
        saveMcatRepair();
        renderRepairSession();
      })
  );
...
function courseRelatedLinks(qId) {
  const units =
    courseData?.units.filter(
      u =>
        u.questionIds.includes(qId) ||
        u.passages.some(id => MCAT.sci.find(p => p.id === id)?.questions.some(q => q.id === qId))
    ) || [];
  return units.length
    ? `<div class="course-feedback-links"><span>Revisit the idea</span>${units.map(u => `<button class="ghostbtn" data-course-open="${u.id}">${esc(u.title)} →</button>`).join('')}</div>`
    : '';
}
document.addEventListener('click', e => {
  const b = e.target.closest('[data-course-open]');
  if (b) {
    e.preventDefault();
    renderCourseUnit(b.dataset.courseOpen, b.dataset.courseRevisit ? 'learn' : undefined);
  }
});
function courseLab(host, u, r) {
  const v = (r.lab.values ||= {});
  const slider = (key, label, min, max, step, initial) => {
    if (!Number.isFinite(v[key])) v[key] = initial;
    return `<label class="course-slider" for="lab-${key}"><span>${label}<output id="out-${key}">${v[key]}</output></span><input id="lab-${key}" data-lab-value="${key}" type="range" min="${min}" max="${max}" step="${step}" value="${v[key]}"></label>`;
  };
  let controls = '';
  if (u.lab === 'enzyme')
    controls =
      slider('substrate', 'Substrate [S] · mM', 0, 20, 0.5, 2) + slider('alpha', 'Competitive factor α', 1, 5, 0.5, 1);
  if (u.lab === 'buffer') controls = slider('acid', 'Added strong acid · mol', 0, 0.15, 0.01, 0);
  if (u.lab === 'circuit')
    controls =
      slider('voltage', 'Ideal source · V', 1, 24, 1, 12) +
      slider('r1', 'Resistor 1 · Ω', 1, 12, 1, 6) +
      slider('r2', 'Resistor 2 · Ω', 1, 12, 1, 3) +
      `<label for="lab-connection">Connection</label><select id="lab-connection"><option value="series" ${v.connection !== 'parallel' ? 'selected' : ''}>Series</option><option value="parallel" ${v.connection === 'parallel' ? 'selected' : ''}>Parallel</option></select>`;
  ...
  const draw = () => {
    const visual = host.querySelector('#lab-visual'),
      reading = host.querySelector('#lab-reading');
    if (u.lab === 'enzyme') {
      const rate = (s, a) => (100 * s) / (2 * a + s),
        path = a =>
          Array.from({ length: 81 }, (_, i) => {
            const s = i / 4;
            return `${i ? 'L' : 'M'}${45 + s * 16.5},${210 - rate(s, a) * 1.7}`;
          }).join(' ');
      ...
      const parallel = v.connection === 'parallel',
        req = parallel ? 1 / (1 / v.r1 + 1 / v.r2) : v.r1 + v.r2,
        current = v.voltage / req;
  ...
  host.querySelectorAll('[data-lab-value]').forEach(
    input =>
      (input.oninput = () => {
        v[input.dataset.labValue] = Number(input.value);
        host.querySelector('#out-' + input.dataset.labValue).value = input.value;
        saveCourse();
        draw();
      })
  );
  host.querySelector('#lab-connection')?.addEventListener('change', e => {
    v.connection = e.target.value;
    saveCourse();
    draw();
  });
  draw();
...
function renderCoursePlacement() {
  if (!courseData) return renderCourseHome();
  coursePauseTools();
  const placementUnits = courseData.units.filter(u => u.placementEligible !== false);
  const p = courseState.placement,
    unanswered = placementUnits.find(u => !p.answers[u.id]);
  const main = el(
    `<main class="course-page course-placement"><span class="course-eyebrow">OPTIONAL / STARTING CHECK</span><h1>Find a useful first step.</h1><p>Twelve short questions sample these foundation topics. Skip anything you prefer. This guides a starting recommendation; it cannot estimate your MCAT score or assess the entire outline.</p><details class="course-map" ${p.startedAt ? '' : 'open'}><summary>Your coursework background</summary><div class="course-background">${Object.entries(
      COURSE_SECTIONS
    )
      .map(
        ([s, n]) =>
          `<label for="background-${s}">${n}<select id="background-${s}" data-course-background="${s}">${[
            ['unknown', 'Prefer not to say'],
            ['new', 'New to this'],
            ['some', 'Some coursework'],
            ['comfortable', 'Comfortable with the basics'],
          ]
            .map(([v, n]) => `<option value="${v}" ${p.background[s] === v ? 'selected' : ''}>${n}</option>`)
            .join('')}</select></label>`
      )
      .join(
        ''
      )}</div></details><div id="placement-body"></div><button class="backbtn" id="placement-back">← Back to course</button></main>`
  );
  main.querySelectorAll('[data-course-background]').forEach(
    s =>
      (s.onchange = () => {
        p.background[s.dataset.courseBackground] = s.value;
        saveCourse();
      })
  );
  ...
  if (!p.startedAt) {
    body.innerHTML = '<button class="btn btn-solid" id="placement-start">Start the 12-question check →</button>';
    body.querySelector('#placement-start').onclick = () => {
      p.startedAt = nowTs();
      saveCourse();
    ...
    const u = unanswered,
      q = McatCourseCore.questionForRecord(u.questions[0], p.draft?.unitId === u.id ? p.draft : null);
    let chosen = p.draft?.unitId === u.id ? p.draft.chosen : null,
      confidence = p.draft?.unitId === u.id ? p.draft.confidence : 'unsure';
    body.innerHTML = `<section class="course-check"><span class="label">${Object.keys(p.answers).length + 1} / 12 · ${SEC_ABBR[u.section]}</span><h2>${esc(q.stem)}</h2><fieldset class="course-answers"><legend>Choose one answer, or skip</legend>${q.options.map((o, i) => `<label><input type="radio" name="placement-answer" value="${i}" ${chosen === i ? 'checked' : ''}><span>${esc(o)}</span></label>`).join('')}</fieldset><label for="placement-confidence">How sure are you?</label><select id="placement-confidence">${Object.entries(
      ...
      .map(([v, n]) => `<option value="${v}" ${confidence === v ? 'selected' : ''}>${n}</option>`)
      .join(
        ''
      )}</select><div class="course-actions"><button class="btn btn-solid" id="placement-next" ${chosen === null ? 'disabled' : ''}>Save & continue →</button><button class="btn" id="placement-skip">Skip this question</button></div></section>`;
    const draft = () => {
      p.draft = { unitId: u.id, chosen, confidence, questionSnapshot: McatCourseCore.snapshot(q) };
      saveCourse();
    };
    draft();
    body.querySelectorAll('[name="placement-answer"]').forEach(
      i =>
        (i.onchange = () => {
          chosen = Number(i.value);
          draft();
          body.querySelector('#placement-next').disabled = false;
        })
    );
    body.querySelector('#placement-confidence').onchange = e => {
    ...
    const advance = skip => {
      p.answers[u.id] = {
        qId: q.id,
        chosen: skip ? null : chosen,
        correct: skip ? null : chosen === q.answer,
        confidence,
        ts: nowTs(),
        questionSnapshot: McatCourseCore.snapshot(q),
      };
      delete p.draft;
      if (Object.keys(p.answers).length === placementUnits.length) p.doneAt = nowTs();
      saveCourse();
      renderCoursePlacement();
    };
    body.querySelector('#placement-next').onclick = () => advance(false);
    body.querySelector('#placement-skip').onclick = () => advance(true);
  } else {
    const answers = Object.values(p.answers),
      taken = answers.filter(a => a.chosen !== null),
      rec = courseRec();
    body.innerHTML = `<div class="course-notice"><h2>Your starting check is saved.</h2><p>${taken.filter(a => a.correct).length}/${taken.length} answered correctly · ${answers.length - taken.length} skipped. Skipped questions are excluded from accuracy.</p>${rec ? `<h3>Suggested next: ${esc(rec.unit.title)}</h3><p>${esc(rec.reason)}</p><button class="btn btn-solid" data-course-open="${rec.unit.id}">Open lesson →</button>` : '<p>All available foundation lessons are complete. Continue with passage practice.</p>'}</div><h2>Review your starting answers</h2>${placementUnits
      .map(u => {
        const a = p.answers[u.id],
          q = McatCourseCore.questionForRecord(u.questions[0], a);
        return `<details class="course-map"><summary>${esc(u.title)} <span>${a.chosen === null ? 'Skipped' : a.correct ? 'Correct' : 'Revisit'}</span></summary><div class="course-saved-answer"><h3>${esc(q.stem)}</h3><p>You: ${a.chosen === null ? 'Skipped' : esc(q.options[a.chosen])}</p><p>Answer: ${esc(q.options[q.answer])}</p><p>${esc(q.explanation)}</p><button class="btn" data-course-focus="${u.id}">Choose this as my starting unit</button></div></details>`;
      })
      .join('')}`;
    body.querySelectorAll('[data-course-focus]').forEach(
      b =>
        (b.onclick = () => {
          courseState.preferredUnit = b.dataset.courseFocus;
          saveCourse();
          renderCourseUnit(b.dataset.courseFocus);
        })
    );
  }
  courseView(main, 'course', 'starting-check');
}
function courseExamReports() {
  return StudyStorage.read('cs-mcat-exam-reviews', []);
}
let courseExamSaveFailed = false;
function courseArchiveExam(run) {
  if (run.archived) return;
  run.attemptId ||= studyAttemptId();
  run.finishedAt ||= nowTs();
  const reports = courseExamReports().filter(r => r.attemptId !== run.attemptId);
  const copy =
    run.rehearsalVersion && window.McatRehearsalCore
      ? window.McatRehearsalCore.pack(run)
      : JSON.parse(JSON.stringify(run));
  copy.archived = true;
  reports.unshift(copy);
  courseExamSaveFailed = !StudyStorage.write('cs-mcat-exam-reviews', reports);
  return !courseExamSaveFailed;
}
function courseOpenExam(id) {
  const run = courseExamReports().find(r => r.attemptId === id);
  if (!run) return renderCourseProgress();
  coursePauseTools();
  sim = run;
  if (run.rehearsalVersion && window.McatRehearsal) return window.McatRehearsal.resume(run);
  finishSim();
}
function courseExamReviewControls(main, run) {
  const section = el(
    `<section class="course-notice"><strong>${run.results.length}/${run.queue.length} sections completed</strong>${courseExamSaveFailed ? '<p role="alert">This browser could not save the exam archive. Keep this tab open and copy important results before leaving.</p>' : ''}<p>Save a takeaway after reviewing the answers. This does not change your original score.</p><label for="exam-reflection">What will you change in the next session?</label><textarea id="exam-reflection" rows="3">${esc(courseState.examReviews[run.attemptId]?.note || '')}</textarea><button class="btn" id="exam-reviewed" ${courseState.examReviews[run.attemptId]?.note?.trim() ? '' : 'disabled'}>${courseState.examReviews[run.attemptId]?.reviewedAt ? 'Review saved' : 'Mark review complete'}</button><small id="exam-review-status" role="status"></small></section>`
  );
  const input = section.querySelector('textarea'),
    button = section.querySelector('#exam-reviewed');
  input.oninput = () => {
    if (StudyStorage.paused) return;
    courseState.examReviews[run.attemptId] ||= {};
    courseState.examReviews[run.attemptId].note = input.value;
    if (saveCourse()) button.disabled = !input.value.trim();
  };
  button.onclick = () => {
    if (StudyStorage.paused || !courseState.examReviews[run.attemptId]?.note?.trim()) return;
    courseState.examReviews[run.attemptId].reviewedAt ||= nowTs();
    if (!saveCourse()) return;
    const plan = guidePlan();
    if (plan?.active?.type === 'examReview' && plan.active.examId === run.attemptId)
      guideCompleteActiveTask('examReview');
    button.textContent = 'Review saved';
    section.querySelector('#exam-review-status').textContent = 'Saved to Progress.';
  };
  main.querySelector('.drill-review')?.before(section);
}
function courseSavedWork(main) {
  const section = el(
    '<section class="course-progress-section"><h2>Saved work to continue</h2><p>Unfinished sessions stay here. Their completed results appear in the records below.</p><div class="course-actions"></div></section>'
  );
  const actions = section.querySelector('.course-actions');
  const add = (label, open) => {
  ...
  for (const u of courseData.units) {
    const r = courseRecord(u.id);
    if (r.startedAt && !r.completedAt) add(u.title + ' · Lesson in progress', () => renderCourseUnit(u.id));
  }
  for (const spec of RESUME_SPECS) {
    const saved = loadResume(spec.key);
    if (saved) add(spec.mod + ' · Saved session', () => spec.resume(saved));
  }
  if (typeof v2State !== 'undefined') {
    for (const r of [v2State.coach.active, ...(v2State.coach.parked || [])].filter(Boolean))
      add(
        (v2Coach(r.coachId)?.title || 'Workshop') +
          ` · ${r.answers.length} ${r.answers.length === 1 ? 'answer' : 'answers'} saved`,
        () => v2ResumeCoach(r.id)
      );
