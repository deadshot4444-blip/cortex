...
function unavailable() {
  return new Response('<!doctype html><html lang="en"><meta name="viewport" content="width=device-width"><title>Download unavailable | Cortex</title><main><h1>This course is not available offline.</h1><p>The download may be missing or this route is outside its scope. Saved study work has not been removed.</p><p>Reconnect, then <a href="/academy?view=storage">open study backups and downloads</a>.</p></main></html>', { status: 503, headers: { 'Content-Type': 'text/html; charset=utf-8', 'Cache-Control': 'no-store' } });
}
async function downloaded(request, name) {
  const item = await Offline.info(name);
  if (!item) return request.mode === 'navigate' ? unavailable() : new Response('Offline download missing', { status: 503 });
  const url = new URL(request.url), cache = await caches.open(name);
  if (request.mode === 'navigate') {
    if (url.pathname !== '/' && url.pathname.endsWith('/')) { url.pathname = url.pathname.replace(/\/+$/, ''); return Response.redirect(url.href, 307); }
  ...
  if (!item.pack.files.some(f => f.url === url.pathname) && url.pathname !== '/offline-manifest.json') return new Response('This file is not in the downloaded course.', { status: 503 });
  return await cache.match(url.pathname) || new Response('A downloaded file is missing. Reconnect and download again.', { status: 503 });
}
async function navigate(request) {
  const url = new URL(request.url), selected = url.searchParams.get('offline');
  ...
  try { return await fetch(request); }
    ...
    for (const item of packs) {
      ...
      url.searchParams.set('offline', item.name); return Response.redirect(url.href, 307);
    }
    return unavailable();
  }
}
...
self.addEventListener('message', event => {
  if (event.data?.type === 'DOWNLOAD_CLIENT' && event.ports[0]) { event.ports[0].postMessage(event.source?.id ? { clientId: event.source.id } : { error: 'The download tab could not be identified. Reload and try again.' }); return; }
  ...
  event.waitUntil((async () => {
    try {
      ...
      if (event.data.expectedIncomplete && complete) throw Error('This download finished after the list was opened. Refresh the list before removing it.');
      if (Offline.cacheNameOK(event.data.name) && (await caches.keys()).includes(event.data.name) && !complete) {
        ...
        if (owner && tabs.some(tab => tab.id === owner)) throw Error('Another open tab owns this unfinished download. Stop it there, or close that tab before discarding the files.');
      }
      await Offline.remove(event.data.name, { inUse }); event.ports[0].postMessage({ ok: true });
