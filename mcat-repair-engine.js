...
(function (root) {
  ...
  const DAY = 86400000;
  function empty() {
    return { version: 1, records: {}, active: null };
  }
  function normalize(value, concepts) {
        ...
        ? r.attempts
            .filter(
              a =>
                ...
                ['diagnose', 'transfer', 'later', 'practice'].includes(a.mode)
            )
            .map(a => ({ ...a, firstExposure: a.firstExposure === true }))
        : [];
      state.records[concept.id] = {
        attempts,
        reviewedAt: Number.isFinite(r.reviewedAt) ? r.reviewedAt : 0,
        dueAt: Number.isFinite(r.dueAt) ? r.dueAt : 0,
      };
    }
    ...
    if (
      c &&
      ['repair', 'later', 'practice'].includes(a.kind) &&
      ['diagnose', 'lesson', 'question', 'feedback', 'done'].includes(a.phase) &&
      [c.diagnostic.id, ...c.checks.map(q => q.id)].includes(a.questionId) &&
  ...
  function recordFor(state, id) {
    return (state.records[id] ||= { attempts: [], reviewedAt: 0, dueAt: 0 });
  }
  ...
  function gaps(concepts, state, logs) {
    return concepts
      .map(concept => {
        ...
        for (const a of logs || [])
          if (
            concept.questionIds.includes(a.qId) &&
            Number.isFinite(a.ts) &&
            a.ts > (state.records[concept.id]?.reviewedAt || 0)
          ) {
            const old = latest.get(a.qId);
            if (!old || a.ts >= old.ts) latest.set(a.qId, a);
          }
      ...
      .filter(g => g.misses)
      .sort((a, b) => b.sure - a.sure || b.misses - a.misses);
  }
  function recommend(concepts, state, logs, now) {
    if (state.active)
      return {
        concept: concepts.find(c => c.id === state.active.conceptId),
        kind: state.active.kind,
        reason: 'Your place is saved. Continue where you left off.',
        resume: true,
      };
    const due = concepts
      .filter(c => state.records[c.id]?.dueAt && state.records[c.id].dueAt <= now && unseenChecks(c, state).length)
      .sort((a, b) => state.records[a.id].dueAt - state.records[b.id].dueAt)[0];
    if (due)
      return { concept: due, kind: 'later', reason: 'A fresh application is due. Try it before reopening the lesson.' };
    const gap = gaps(concepts, state, logs)[0];
    if (gap)
      return {
        concept: gap.concept,
        kind: 'repair',
        reason: `${gap.misses} recent missed ${gap.misses === 1 ? 'question' : 'questions'}${gap.sure ? `, including ${gap.sure} marked Sure` : ''}. Check the idea behind the answer.`,
      };
    ...
    return fresh
      ? {
          concept: fresh,
          kind: 'repair',
          reason: 'Try one concept: a quick question, a short lesson, and a new application.',
        }
      : null;
  }
  function begin(concept, state, kind, now) {
    ...
    state.active = {
      ...
      kind,
      phase: kind === 'later' ? 'question' : kind === 'practice' ? 'lesson' : 'diagnose',
      questionId: kind === 'later' ? remaining[0].id : concept.diagnostic.id,
      startedAt: now,
      confidence: 'unsure',
      result: null,
    };
  ...
  function answer(concept, state, choice, confidence, now) {
    ...
    let mode =
      a.phase === 'diagnose'
        ? 'diagnose'
        : a.kind === 'later' && r.dueAt && now >= r.dueAt
          ? 'later'
          : a.kind === 'practice'
            ? 'practice'
            : 'transfer';
    ...
    const result = {
      ...
      confidence: ['guess', 'unsure', 'sure'].includes(confidence) ? confidence : 'unsure',
      at: now,
      firstExposure,
      mode,
    };
    ...
    if (mode !== 'diagnose') {
      ...
      if (mode !== 'practice')
        r.dueAt = unseenChecks(concept, state).length ? now + (mode === 'later' && result.correct ? 3 : 1) * DAY : 0;
    }
    a.result = result;
    a.phase = 'feedback';
  ...
  function afterLesson(concept, state, now) {
    ...
    const q =
      a.kind === 'practice' || early
        ? concept.checks.find(q => seen.has(q.id)) || concept.diagnostic
        : unseenChecks(concept, state)[0] || concept.checks[0];
    a.questionId = q.id;
    a.phase = 'question';
    a.result = null;
    a.confidence = 'unsure';
  ...
  function stats(concepts, state) {
    ...
    return {
      practiced: concepts.filter(c => (state.records[c.id]?.attempts || []).some(a => a.mode !== 'diagnose')).length,
      laterPassed: concepts.filter(c => status(c, state) === 'Passed a later check').length,
      laterCorrect: later.filter(a => a.correct).length,
