...
window.AcademyStorage = (() => {
  ...
  function render() {
    ...
    const text = (id, message) => {
      if (active()) main.querySelector(id).textContent = message;
    };
    const requireSaved = () => {
      if (StudyStorage.paused)
        throw Error(
          'Saving is paused. Download recovery copies and resolve the save problem before preparing or restoring a portable backup.'
        );
    };
    ...
    main.querySelector('#storage-export').onclick = async e => {
      ...
      const host = main.querySelector('#storage-download');
      clearPrepared();
      host.replaceChildren();
      try {
        requireSaved();
        const snapshot = CortexAccount.snapshot(),
          identity = JSON.stringify(snapshot);
        ...
        link.textContent = 'Download prepared backup';
        preparedURL = URL.createObjectURL(new Blob([file], { type: 'application/json' }));
        link.href = preparedURL;
        link.download = 'cortex-study-' + new Date().toISOString().slice(0, 10) + '.json';
        link.onclick = event => {
          try {
            requireSaved();
            if (!active() || JSON.stringify(CortexAccount.snapshot()) !== identity)
              throw Error('Saved work changed. Prepare a new backup before downloading.');
            text('#storage-export-status', 'Download requested. Check that the file reached your Downloads folder.');
          } catch (error) {
            event.preventDefault();
            clearPrepared();
            host.replaceChildren();
            text('#storage-export-status', error.message);
          }
        };
        ...
        text(
          ...
          `Backup prepared from ${count} saved record${count === 1 ? '' : 's'}. Download the file below.`
        );
      } catch (error) {
        text('#storage-export-status', error.message);
      } finally {
    ...
    main.querySelector('#storage-recovery').onclick = () => {
      try {
        ...
        text(
          '#storage-export-status',
          'Recovery download prepared. This separate recovery format may contain prior device, cloud and guest copies; it is not a portable import file.'
        );
      } catch {
        text('#storage-export-status', 'Could not prepare recovery copies. Keep this tab open.');
      }
    };
    let selection = 0;
    main.querySelector('#storage-file').onchange = async e => {
      const request = ++selection,
        ...
        host = main.querySelector('#storage-preview');
      host.replaceChildren();
      if (!file) {
        text('#storage-import-status', 'No file selected.');
        return;
      }
      ...
      try {
        ...
        requireSaved();
        const preview = CortexAccount.prepareRestore(parsed.data);
        const panel =
          el(`<div class="academy-day-summary"><h3>Review the replacement</h3><p data-file></p><p data-target></p>
        ...
        panel.querySelector('[data-file]').textContent =
          'Created ' +
        ...
        for (const action of ['add', 'replace', 'remove', 'keep']) {
          ...
          li.textContent = `${count} record${count === 1 ? '' : 's'}: ${action}`;
          list.appendChild(li);
        }
        panel.querySelector('[data-records]').textContent = JSON.stringify(
          Object.fromEntries(
        ...
        const apply = panel.querySelector('[data-apply]'),
        ...
        panel.querySelector('[data-cancel]').onclick = () => {
          selection++;
          host.replaceChildren();
          main.querySelector('#storage-file').value = '';
          text('#storage-import-status', 'Restoration canceled. Your work is unchanged.');
        };
        apply.onclick = () => {
          ...
          try {
            ...
            text('#storage-import-status', 'Workspace restored. Reloading…');
          } catch (error) {
            selection++;
            host.replaceChildren();
            main.querySelector('#storage-file').value = '';
        ...
        text('#storage-import-status', 'File checked. Nothing has been restored yet.');
      } catch (error) {
        if (request === selection && active()) {
          main.querySelector('#storage-file').value = '';
          text('#storage-import-status', error.message);
        }
      }
    };
