...
const SIM_SECTIONS = {
  chemPhys: { abbr: 'C/P', min: 95 },
  cars: { abbr: 'CARS', min: 90 },
  bioBiochem: { abbr: 'B/B', min: 95 },
  psychSoc: { abbr: 'P/S', min: 95 },
};
...
function saveSRS() {
  return StudyStorage.write('cs-mcat-srs', SRS);
}
function saveQ() {
  if (QLOG.length > 1000) QLOG.splice(0, QLOG.length - 1000);
  const historySaved = StudyStorage.write('cs-mcat-q', QHIST);
  const logSaved = StudyStorage.write('cs-mcat-log', QLOG);
  return historySaved && logSaved;
}
...
function schedule(rec, rating) {
  if (rating === 'again') {
    ...
    rec.reps++;
    rec.due = nowTs() + rec.interval * DAY;
  }
  rec.last = nowTs();
  return rec;
}
...
function dueCount() {
  const t = nowTs();
  return MCAT.cards.filter(c => SRS[c.id] && SRS[c.id].due <= t && SRS[c.id].reps > 0).length;
}
function newCount() {
  return MCAT.cards.filter(c => !SRS[c.id] || SRS[c.id].reps === 0).length;
}
...
const MCAT_DATA = [
  ['outline', 'data/mcat-outline.json?v=2', 'content map'],
  ['cards', 'data/mcat-cards.json?v=2', 'flashcards'],
  ['questions', 'data/mcat-questions.json?v=5', 'question drills'],
  ['cars', 'data/mcat-cars.json?v=4', 'CARS passages'],
  ['sci', 'data/mcat-science-passages.json?v=7', 'science passages'],
];
function mcatDataPresent(key) {
  return key === 'outline'
    ? Array.isArray(MCAT.outline?.concepts) && MCAT.outline.concepts.length > 0
    : Array.isArray(MCAT[key]) && MCAT[key].length > 0;
}
function mcatDataNotice(main) {
  const missing = MCAT_DATA.filter(([key]) => !mcatDataPresent(key)).map(([, , label]) => label);
  if (!courseData) missing.push('course lessons');
  if (!v2Data) missing.push('coaching tools');
  if (!repairData) missing.push('concept sessions');
  if (!missing.length) return;
  const notice = el(
    `<aside class="course-notice" role="status"><strong>Some study material could not load.</strong><p>Unavailable: ${esc(missing.join(', '))}. Your saved progress is unchanged. Loaded activities remain available.</p><button class="btn" id="mcat-data-retry">Retry loading study material</button></aside>`
  );
...
function loadResume(key) {
  return StudyStorage.read('cs-mcat-r-' + key, null);
}
function clearResume(key) {
  return StudyStorage.remove('cs-mcat-r-' + key);
}
...
const RESUME_SPECS = [
  {
    ...
    mod: 'Exam Simulator',
    progressOf(r) {
      return r.queue?.length && (!r.finishedAt || (r.rehearsalVersion && !r.archived)) ? 1 : 0;
    },
    label(r) {
      const s = r.queue[r.si];
      if (r.rehearsalVersion && r.phase === 'finished') return 'Save completed rehearsal';
      return r.onBreak
        ? `Break &middot; ${SIM_SECTIONS[s.key].abbr} next`
        : `${SIM_SECTIONS[s.key].abbr} Q ${(r.idx || 0) + 1}/${s.items.length}`;
    },
    resume(r) {
      if (r.rehearsalVersion && window.McatRehearsal) return window.McatRehearsal.resume(r);
      studyRestoreGuide(r);
      sim = r;
      if (r.finishedAt) return finishSim();
      ...
      sim.deadline = nowTs() + (r._remain ?? 600000);
      simTimerId = setInterval(simTick, 500);
      renderSimQ();
    },
  },
  {
    ...
    mod: 'Question Drills',
    progressOf(r) {
      return (r.results ? r.results.length : 0) || r.idx || (r.qs?.length ? 1 : 0);
    },
    label(r) {
      return `Q ${(r.idx || 0) + 1}/${r.qs.length}`;
    },
    resume(r) {
      studyRestoreGuide(r);
      drill = r;
  ...
  },
  {
    ...
    mod: 'CARS Studio',
    progressOf(r) {
      return r.phase === 'blind' || r.phase === 'done' ? 1 : r.results?.length || (r.p?.questions?.length ? 1 : 0);
    },
    label(r) {
      return r.phase === 'blind'
        ? 'Blind review'
        : r.phase === 'done'
          ? 'Review results'
          : `Q ${(r.idx || 0) + 1}/${r.p.questions.length}`;
    },
    resume(r) {
      resumeCarsSession(r);
    },
  },
  {
    ...
    mod: 'Passage Lab',
    progressOf(r) {
      return r.phase === 'analysis' || r.phase === 'done' ? 1 : r.results?.length || (r.p?.questions?.length ? 1 : 0);
    },
    label(r) {
      return r.phase === 'analysis'
        ? 'Experiment notebook'
        : r.phase === 'done'
          ? 'Review results'
          : `Q ${(r.idx || 0) + 1}/${r.p.questions.length}`;
    ...
    resume(r) {
      resumePassageSession(r);
    },
  },
  {
    key: 'flash',
    mod: 'Flashcard Reactor',
    progressOf(r) {
      return r.queue?.length ? 1 : 0;
    },
    label(r) {
      return `${r.done || 0} / ${r.total || 0} cards`;
    },
    resume(r) {
      studyRestoreGuide(r);
      flash = r;
...
function resumeBtn(key) {
  ...
  let lbl;
  try {
    if (!(spec.progressOf(r) > 0)) return null;
    lbl = spec.label(r);
  } catch {
    StudyStorage.sessionFailed();
    return null;
...
function hubResumeChip() {
  ...
  const btn = el(
    `<button class="btn btn-resume" id="hub-resume">&#8634; Resume &middot; ${best.spec.mod} &middot; ${best.lbl}</button>`
  );
  btn.addEventListener('click', () => {
    try {
      best.spec.resume(best.r);
    } catch {
      StudyStorage.sessionFailed();
    }
  });
...
async function renderMCATEntry() {
  coursePauseTools();
  if (new URLSearchParams(location.search).get('view') === 'quality' && window.McatItemQuality)
    return McatItemQuality.render();
  await loadMCAT();
  const params = new URLSearchParams(location.search),
    view = params.get('view'),
    unit = params.get('unit');
  if (view === 'course') {
    if (unit === 'starting-check') return renderCoursePlacement();
    return unit ? renderCourseUnit(unit) : renderCourseHome();
  }
...
function mcatTaskHeader(segs, right, exitLabel) {
  const parts = (segs || [])
    .filter(Boolean)
    .map(s => `<span class="crumb-seg">${s}</span>`)
    .join('<span class="crumb-sep">&middot;</span>');
...
function wireRunHeader(root, onExit) {
  const c = root.querySelector('#crumbmcat');
  if (c) c.addEventListener('click', renderMCATEntry);
  const x = root.querySelector('#exit');
  if (x) x.addEventListener('click', onExit);
}
function confirmExit(hasProgress, onLeave) {
  ...
  const m =
    el(`<dialog class="modal" id="cfx" aria-labelledby="cfx-title" aria-describedby="cfx-description"><div class="modal-box">
  ...
  m.addEventListener('click', e => {
    if (e.target.id === 'cfx' || e.target.id === 'cfx-cancel') close();
  });
  m.querySelector('#cfx-quit').addEventListener('click', () => {
    close();
    onLeave();
  });
...
async function renderMCAT() {
  ...
  if (cars) {
    clearInterval(cars.timerId);
    cars = null;
  }
  if (plab) {
    clearInterval(plab.timerId);
    plab = null;
  }
  if (simTimerId) {
  ...
  const due = dueCount(),
    fresh = newCount();
  const qn = MCAT.questions.length,
    cn = MCAT.cards.length;
  const t = mcatTotals();

  const carsN = MCAT.cars.length,
    sciN = MCAT.sci.length;
  const conceptsN = MCAT.outline ? MCAT.outline.concepts.length : 0;

  const tools = [
    {
      name: 'Flashcards',
      desc: 'Recall facts on a spaced schedule',
      stat: cn ? `${due} due &middot; ${fresh} new` : 'Unavailable',
      go: renderFlashHome,
      on: cn > 0,
      core: true,
    },
    {
      name: 'Question drills',
      desc: 'Practice discrete questions and review every answer',
      stat: qn ? `${qn} questions` : 'Unavailable',
      go: renderDrillSetup,
      on: qn > 0,
      core: true,
    },
    {
      name: 'CARS practice',
      desc: 'Work through passages with blind review',
      stat: carsN ? `${carsN} passages` : 'Unavailable',
      go: renderCarsHome,
      on: carsN > 0,
      core: true,
    },
    {
      ...
      desc: 'Interpret experiments, figures, and data tables',
      stat: sciN ? `${sciN} passages` : 'Unavailable',
      go: renderPassageHome,
      on: sciN > 0,
      core: true,
    },
    {
      name: 'Practice exam',
      desc: 'Train with intact passage sets, then review each timed run',
      stat: 'Timed',
      go: renderSimHome,
      on: qn > 0 && !!MCAT.outline,
      core: true,
    },
    {
      name: 'Mistake lab',
      desc: 'Work through gaps and check your learning',
      stat: t.answered ? `${t.answered} answered &middot; ${t.acc}%` : 'No data yet',
      go: renderMistakeLab,
      on: true,
      core: false,
    },
    {
      name: 'Coverage map',
      desc: 'Find learning objectives, practice, and remaining gaps',
      stat: `${conceptsN} concepts`,
      go: renderBlueprint,
      on: !!MCAT.outline,
      core: false,
    },
    {
      name: 'Study plan',
      desc: 'Build a 120, 90, or 60-day schedule',
      stat: guidePlan() ? 'Plan active' : 'Build a plan',
      go: renderGuide,
      on: !!MCAT.outline,
      core: false,
    },
    {
      name: 'Course mapper',
      desc: 'Mark the prerequisite courses already completed',
      stat: 'Pre-study check',
      go: renderMapper,
      on: !!MCAT.outline,
      core: false,
    },
  ...
  const method = {
    intro:
      'The study tools combine scheduled review, practice questions, and written reflection. Here is what each tool does with your answers. Practice records describe your work in Cortex; they do not establish mastery or predict an MCAT score.',
    points: [
      [
        'Scheduled review',
        'Flashcard intervals respond to your ratings. Course checks return after scheduled delays that respond to previous answers. These schedules do not measure exactly when you will forget something.',
      ],
      [
        'Answer before feedback',
        'Choose an answer before reading its explanation. Some activities also ask for a written explanation or passage evidence; multiple-choice drills record your selected answer.',
      ],
      [
        'Mixed practice',
        'Mixed sessions include questions or activities from different topics. Topic filters remain available when you want to focus on one area.',
      ],
      [
        'Confidence and accuracy',
        'Record how sure you are, then compare confidence with the accuracy of your saved answers. This can highlight answers you may want to revisit.',
      ],
      [
        'Answer explanations',
        'Read the authored explanation after answering. Where available, additional notes explain the distractors and suggest a related concept to review.',
      ],
      [
        'Blind review',
        'Reconsider CARS answers and write your reasoning before seeing explanations. Your first answers and later revisions remain separate in the review.',
      ],
      [
        'Teach-back',
        'Write an explanation in your own words, then compare it with the authored model where provided. Your writing is saved for self-review and is not automatically graded for mastery.',
      ],
    ],
  };
  ...
  mcatWorkspace(root, 'practice');
  studySetView(root);
}

function mcatTotals() {
  let answered = QLOG.length,
    correct = QLOG.filter(x => x.correct).length;
...
function renderFlashHome() {
  ...
  refresh();
  main.querySelectorAll('#fdeck .mode').forEach(b =>
    b.addEventListener('click', () => {
      deck = b.dataset.deck;
      main.querySelectorAll('#fdeck .mode').forEach(x => x.classList.toggle('active', x === b));
      refresh();
    })
  );
...
function renderFlashCard() {
  ...
  if (flash.idx >= flash.queue.length) {
    finishFlash();
    return;
  }
  ...
  wireRunHeader(root, () =>
    confirmExit(flash && (flash.done > 0 || flash.again > 0 || flash.idx > 0), () =>
      guideLeaveActive('flash', renderFlashHome)
    )
  );
  setView(root);
  ...
  root.querySelector('#fc').addEventListener('click', () => {
    if (root.querySelector('#flash-back').style.display === 'none') showReveal();
  });
...
function rateFlash(c, rating) {
  ...
  if (rec.lastRatingId !== ratingId) {
    schedule(rec, rating);
    rec.lastRatingId = ratingId;
...
function renderDrillSetup() {
  ...
  let scope = 'all',
    ...
    mode = 'standard';
  const seg = (sel, set) =>
    main.querySelectorAll(sel).forEach(b =>
      b.addEventListener('click', () => {
        set(b);
        main.querySelectorAll(sel).forEach(x => x.classList.toggle('active', x === b));
      })
    );
  ...
  seg('#dmode .mode', b => {
    mode = b.dataset.dm;
    main.querySelector('#dmh').textContent =
      mode === 'blind'
        ? 'Blind review: answer everything first with no feedback, then review all at once — trains honest judgment.'
        : 'Standard: feedback after each question.';
  });
...
function renderDrillQ() {
  ...
  wireRunHeader(root, () =>
    confirmExit(drill && drill.results.length > 0, () => guideLeaveActive('drill', renderDrillSetup))
  );
  let conf = 'unsure';
  root.querySelectorAll('#conf .mode').forEach(b =>
    b.addEventListener('click', () => {
      conf = b.dataset.c;
      root.querySelectorAll('#conf .mode').forEach(x => x.classList.toggle('active', x === b));
    })
  );
  root
    .querySelectorAll('.opt')
    .forEach(b => b.addEventListener('click', () => answerDrill(root, q, +b.dataset.i, conf)));
  window.McatRehearsal?.tag(root, { questionId: q.id, context: 'question-drill' });
  setView(root);
  const saved = drill.results[drill.idx];
  if (saved) answerDrill(root, q, saved.chosen, saved.conf);
}
...
function answerDrill(root, q, choice, conf) {
  ...
  const res = (drill.results[drill.idx] ||= {
    id: q.id,
    section: q.section,
    category: q.category,
    chosen: choice,
  ...
  if (drill.mode === 'blind') {
    ...
    const autopsy = (q.distractors || [])
      .filter(d => d.i !== q.answer)
      .map(d => `<div class="autopsy-row"><span class="ak">${'ABCD'[d.i]}</span><span>${esc(d.why)}</span></div>`)
      .join('');
    ...
    after.querySelectorAll('.rc-chip').forEach(ch => {
      ch.classList.toggle('on', ch.dataset.rc === res.root);
      ch.addEventListener('click', () => {
        res.root = ch.dataset.rc;
        saveResume('drill', drill);
        after.querySelectorAll('.rc-chip').forEach(x => x.classList.toggle('on', x === ch));
      });
    });
  }
...
function finishDrill() {
  ...
  if (reserved) {
    if (saveResume('drill', drill)) window.McatRehearsal.allow({ questionId: reserved.id }, finishDrill);
    return;
  }
  const guided = guideCompleteActiveTask('drill');
  // commit to history + log
  const t = (drill.finishedAt ||=
    drill.results.map(r => QHIST[r.id]).find(h => h?.lastAttemptId === drill.attemptId)?.ts || nowTs());
  drill.results.forEach(r => {
    ...
    h.lastAttemptId = drill.attemptId;
...
function currentPracticeLog() {
  ...
  return QLOG.map(record =>
    categories.has(record.qId) ? { ...record, category: categories.get(record.qId) } : record
  );
}
function renderCategory(cat, con, secKey) {
  ...
  if (qs.length)
    main.querySelector('#drill').addEventListener('click', () => {
      drill = { qs: shuffleArr(qs).slice(0, 10), idx: 0, mode: 'standard', results: [], scope: cat.id };
      renderDrillQ();
    });
  if (cards.length)
    main.querySelector('#study').addEventListener('click', () => {
      flash = { deck: cat.id, queue: shuffleArr(cards), idx: 0, total: cards.length, again: 0, done: 0 };
      renderFlashCard();
    });
  main.querySelectorAll('[data-category-passage]').forEach(
    button =>
      (button.onclick = () =>
        startPassage(
          passages.find(p => p.id === button.dataset.categoryPassage),
          false
        ))
  );
  if (passages.length) {
...
function renderMistakeLab() {
  ...
  const byCat = {};
  currentPracticeLog().forEach(x => {
    byCat[x.category] ||= { n: 0, c: 0 };
    byCat[x.category].n++;
    if (x.correct) byCat[x.category].c++;
  });
  const weak = Object.entries(byCat)
    .filter(([, v]) => v.n >= 2)
    .map(([k, v]) => ({ cat: k, acc: Math.round((100 * v.c) / v.n), n: v.n }))
    .sort((a, b) => a.acc - b.acc)
    .slice(0, 6);
  // root causes
  const roots = {};
  Object.values(QHIST).forEach(h => {
    if (h.root) roots[h.root] = (roots[h.root] || 0) + 1;
  });
  ...
  const missed = Object.entries(QHIST)
    .filter(([id, h]) => h.lastCorrect === false && MCAT.questions.some(q => q.id === id))
    .map(([id]) => id);
  ...
  const main = el(`<main class="panel">
    ...
    ${
      ...
      <div class="metric"><span class="m-num">${weak.length ? weak[0].acc + '%' : '&mdash;'}</span><span class="m-lab">Weakest area</span><span class="m-sub">${weak.length ? esc(catTitle(weak[0].cat)) : ''}</span></div>
        ...
        : '<div class="empty">No drill data yet — run a Question Drill and your weak spots show up here.</div>'
    }
  ...
  if (weak.length) {
    ...
    weak.forEach(w => {
      ...
      const row = el(
        `<button class="bp-cat" ${qs.length ? '' : 'disabled'}><span class="bp-cat-id">${esc(w.cat)}</span><span class="bp-cat-title">${cat ? esc(cat.title) : ''}</span><span class="bp-cat-stat no">${w.acc}% &middot; ${w.n}q</span></button>`
      );
      if (qs.length)
        row.addEventListener('click', () => {
          drill = { qs: shuffleArr(qs).slice(0, 10), idx: 0, mode: 'standard', results: [], scope: w.cat };
          renderDrillQ();
        });
      wc.appendChild(row);
    });
  ...
  if (redo)
    redo.addEventListener('click', () => {
      ...
      if (qs.length) {
        drill = { qs: shuffleArr(qs).slice(0, 20), idx: 0, mode: 'standard', results: [], scope: 'missed' };
        renderDrillQ();
      }
    });
  main.querySelector('#home').addEventListener('click', renderMCAT);
...
function findCat(id) {
  if (!MCAT.outline) return null;
  for (const c of MCAT.outline.concepts) {
    const f = c.categories.find(x => x.id === id);
    if (f) return f;
  }
  return null;
}
...
function renderCarsHome() {
  ...
  let timed = false;
  main.querySelectorAll('#ctime .mode').forEach(b =>
    b.addEventListener('click', () => {
      timed = b.dataset.t === 'on';
      main.querySelectorAll('#ctime .mode').forEach(x => x.classList.toggle('active', x === b));
    })
  );
  ...
  MCAT.cars.forEach(p => {
    ...
    const row = el(
      `<button class="row"><span class="row-main"><span class="row-spec">${esc(p.discipline)}</span><span class="row-title">${esc(p.title)}</span></span><span class="row-right">${acc != null ? `<span class="pill ${acc >= 75 ? 'ok' : acc >= 50 ? 'mid' : 'no'}">${log.filter(x => x.correct).length}/${log.length} recorded correct</span>` : `<span class="row-when">${p.questions.length}q &rarr;</span>`}</span></button>`
    );
  ...
  main
    .querySelector('#rand')
    .addEventListener('click', () => startCars(MCAT.cars[Math.floor(Math.random() * MCAT.cars.length)], timed));
  main.querySelector('#back').addEventListener('click', renderMCAT);
...
function startCars(p, timed) {
  ...
  if (
    window.McatRehearsal &&
    !window.McatRehearsal.allow(
      { passageId: p.id, questionId: p.questions[0]?.id },
      () => startCars(p, timed),
      renderCarsHome
    )
  )
  ...
  cars = {
    p,
    phase: 'attempt',
    attemptId: studyAttemptId(),
    flags: {},
    confidence: {},
    idx: 0,
    results: [],
    timed,
    deadline: timed ? nowTs() + 600000 : 0,
    timerId: null,
  };
...
function carsTick() {
  if (!cars || (cars.phase && cars.phase !== 'attempt')) return;
  const left = (cars.deadline - nowTs()) / 1000;
  const t = document.getElementById('cars-timer');
  if (t) {
...
function renderCarsRunner() {
  ...
  if (
    window.McatRehearsal &&
    !window.McatRehearsal.allow(
      { passageId: p.id, questionId: p.questions[cars.idx]?.id },
      () => resumeCarsSession(loadResume('cars') || cars),
      renderCarsHome
    )
  )
    return;
  if (cars.idx >= p.questions.length) {
    finishCars();
    return;
  }
  ...
  const root = el(`<div>
    ...
    <main class="cars-stage">
      <div class="cars-passage"><span class="label">${esc(p.title)}</span>${p.text
        .split(/\n\n+/)
        .map(par => `<p>${esc(par)}</p>`)
        .join('')}</div>
      <div class="cars-q">
        <p class="q">${esc(q.stem)}</p>
        <fieldset class="repair-confidence"><legend>How sure are you?</legend>${Object.entries(CONF)
          .map(
            ([key, label]) =>
              `<label><input type="radio" name="cars-confidence" value="${key}" ${(cars.confidence[q.id] || 'unsure') === key ? 'checked' : ''}>${label}</label>`
          )
          .join('')}</fieldset>
        <label class="study-flag"><input type="checkbox" id="cars-flag" ${cars.flags[q.id] ? 'checked' : ''}> Flag for blind review</label>
        <div class="opts">${McatV2Core.optionOrder(q.options, q.displayOrder)
          .map(
            (i, position) =>
              `<button class="opt" data-i="${i}"><span class="key">${'ABCD'[position]}</span><span>${esc(q.options[i])}</span></button>`
          )
          .join('')}</div>
      </div>
    </main>
  ...
  root.querySelectorAll('[name="cars-confidence"]').forEach(
    input =>
      (input.onchange = () => {
        cars.confidence[q.id] = input.value;
        saveResume('cars', cars);
      })
  );
  root.querySelector('#cars-flag').onchange = e => {
    cars.flags[q.id] = e.target.checked;
    saveResume('cars', cars);
  };
  root.querySelectorAll('.opt').forEach(b =>
    b.addEventListener('click', () => {
      cars.results.push({
        q,
        chosen: +b.dataset.i,
        correct: +b.dataset.i === q.answer,
        conf: cars.confidence[q.id] || 'unsure',
        flagged: !!cars.flags[q.id],
      });
      cars.idx++;
      renderCarsRunner();
      window.scrollTo(0, 0);
...
function renderPassageHome() {
  ...
  const refresh = () => {
    ...
    MCAT.sci
      .filter(p => sec === 'all' || p.section === sec)
      .forEach(p => {
        const log = QLOG.filter(x => x.passage === p.id);
        const acc = log.length ? Math.round((100 * log.filter(x => x.correct).length) / log.length) : null;
        const row = el(
          `<button class="row"><span class="row-main"><span class="row-spec">${SEC_ABBR[p.section]} &middot; ${esc(p.type)}</span><span class="row-title">${esc(p.title)}</span></span><span class="row-right">${acc != null ? `<span class="pill ${acc >= 75 ? 'ok' : acc >= 50 ? 'mid' : 'no'}">${log.filter(x => x.correct).length}/${log.length} recorded correct</span>` : `<span class="row-when">${p.questions.length}q &rarr;</span>`}</span></button>`
        );
        row.disabled = paused;
  ...
  refresh();
  main.querySelectorAll('#psec .mode').forEach(b =>
    b.addEventListener('click', () => {
      sec = b.dataset.s;
      main.querySelectorAll('#psec .mode').forEach(x => x.classList.toggle('active', x === b));
      refresh();
    })
  );
  main.querySelectorAll('#ptime .mode').forEach(b =>
    b.addEventListener('click', () => {
      timed = b.dataset.t === 'on';
      main.querySelectorAll('#ptime .mode').forEach(x => x.classList.toggle('active', x === b));
    })
  );
  main.querySelector('#back').addEventListener('click', renderMCAT);
  studyLockPassageList('plab', main);
...
function plabTick() {
  if (!plab || (plab.phase && plab.phase !== 'attempt')) return;
  const left = (plab.deadline - nowTs()) / 1000;
  const t = document.getElementById('plab-timer');
  if (t) {
    t.textContent = fmtTime(left);
    t.classList.toggle('crit', left <= 60);
  }
...
function renderPassageRunner() {
  ...
  if (
    window.McatRehearsal &&
    !window.McatRehearsal.allow(
      { passageId: p.id, questionId: p.questions[plab.idx]?.id },
      () => resumePassageSession(loadResume('plab') || plab),
      renderPassageHome
    )
  )
    return;
  if (plab.idx >= p.questions.length) {
    finishPassage();
    return;
  }
  ...
  root.querySelectorAll('.opt').forEach(b =>
    b.addEventListener('click', () => {
      plab.results.push({ q, chosen: +b.dataset.i, correct: +b.dataset.i === q.answer });
      plab.idx++;
      renderPassageRunner();
    })
  );
  ...
  setView(root);
  window.scrollTo(0, 0);
  if (plab.timed) plabTick();
}
function finishPassage() {
  if (window.McatRehearsal && !window.McatRehearsal.allow({ passageId: plab.p.id }, finishPassage, renderPassageHome))
    return;
  if (plab.phase !== 'done') {
    studyFinishAttempt(plab);
    renderExperimentNotebook();
    return;
  }
  if (!plab.archived) {
    studyLogAttempt(plab, 'plab');
    plab.guided ||= guideCompleteActiveTask('passage');
    studySaveReport('plab', plab);
    clearResume('plab');
  }
  const guided = plab.guided,
    p = plab.p;
  const correct = plab.results.filter(r => r.correct).length,
    total = plab.results.length;
  const root = el('<div></div>');
  root.appendChild(topbar('mcat'));
  ...
  main.querySelector('#dr').innerHTML =
    experimentComparison(plab) +
    `<span class="label">Question review · original score</span>` +
    plab.results
      .map((r, i) => {
        const autopsy = (r.q.distractors || [])
          .filter(d => d.i !== r.q.answer)
          .map(d => `<div class="autopsy-row"><span class="ak">${'ABCD'[d.i]}</span><span>${esc(d.why)}</span></div>`)
          .join('');
        return `<details class="rev" ${r.correct ? '' : 'open'}><summary><span class="${r.correct ? 'ok' : 'no'}">${r.correct ? '&#10003;' : '&#10007;'}</span> Q${i + 1}</summary><div class="rev-body"><div class="rev-ans">You: ${r.chosen == null ? 'Unanswered' : 'ABCD'[r.chosen]} &middot; Correct: <b>${'ABCD'[r.q.answer]}</b></div><p>${esc(r.q.explanation)}</p>${courseRelatedLinks(r.q.id)}${autopsy ? `<div class="autopsy">${autopsy}</div>` : ''}</div></details>`;
      })
      .join('');
  if (guided) main.querySelector('#guide').addEventListener('click', renderGuide);
...
function renderSimHome() {
  return window.McatRehearsal ? window.McatRehearsal.home() : renderLegacySimHome();
}
function renderLegacySimHome() {
  ...
  ['chemPhys', 'cars', 'bioBiochem', 'psychSoc'].forEach(k => {
    ...
    const row = el(
      `<button class="bp-cat" ${n ? '' : 'disabled'}><span class="bp-cat-id">${SIM_SECTIONS[k].abbr}</span><span class="bp-cat-title">${esc(MCAT.outline.sections[k].name)}</span><span class="bp-cat-stat">${n} q</span></button>`
    );
  ...
  if (rb) {
    const sb = main.querySelector('.endbtns');
    if (sb) sb.prepend(rb);
  }
  root.appendChild(main);
  setView(root);
...
function simPool(secKey) {
  if (secKey === 'cars') {
    const groups = MCAT.cars.map(p =>
      p.questions.map(q => ({ q, passageId: p.id, passageText: p.text, passageTitle: p.title }))
    );
    return shuffleArr(groups).flat();
  }
  // science: passage sets (kept together) first, then discretes — like a real section
  const groups = MCAT.sci
    .filter(p => p.section === secKey)
    .map(p =>
      p.questions.map(q => ({
        q,
        passageId: p.id,
  ...
  const discretes = shuffleArr(MCAT.questions.filter(q => q.section === secKey))
    .slice(0, 15)
    .map(q => ({ q }));
  return passageItems.concat(discretes);
}
function simPracticeItems(key) {
  const pool = simPool(key),
    chosen = [],
    passageLimit = key === 'cars' ? 30 : 25;
  const groups = new Map();
  pool
    .filter(it => it.passageId)
    .forEach(it => {
      if (!groups.has(it.passageId)) groups.set(it.passageId, []);
      groups.get(it.passageId).push(it);
    });
  for (const group of groups.values()) {
    if (chosen.length && chosen.length + group.length > passageLimit) break;
    chosen.push(...group);
  }
...
function startSim(sectionKeys) {
  ...
  if (saved?.queue?.length) {
    RESUME_SPECS.find(s => s.key === 'sim').resume(saved);
    return;
  }
...
function beginSection() {
  ...
  sim.sectionDurationMs = minutes * 60000;
  sim.deadline = nowTs() + sim.sectionDurationMs;
  sim.idx = 0;
  if (simTimerId) clearInterval(simTimerId);
  simTimerId = setInterval(simTick, 500);
  renderSimQ();
...
function simTick() {
  if (!sim) return;
  const left = (sim.deadline - nowTs()) / 1000;
...
function renderSimQ() {
  ...
  if (
    window.McatRehearsal &&
    !window.McatRehearsal.allow(
      { passageId: it.passageId, questionId: q.id },
      () => RESUME_SPECS.find(s => s.key === 'sim').resume(loadResume('sim') || sim),
      renderLegacySimHome
    )
  )
    return;
  ...
  sim.seen ||= [];
  if (!sim.seen.includes(key)) sim.seen.push(key);
  saveResume('sim', sim);
  const root = el(`<div>
    ...
    <main class="case">
      ...
      <p class="q">${esc(q.stem)}</p>
      <div class="opts" id="opts">${McatV2Core.optionOrder(q.options, q.displayOrder)
        .map(
          (i, position) =>
            `<button class="opt ${chosen === i ? 'picked' : ''}" data-i="${i}"><span class="key">${'ABCD'[position]}</span><span>${esc(q.options[i])}</span></button>`
        )
        .join('')}</div>
      <div class="sim-bar">
  ...
  wireRunHeader(root, () =>
    confirmExit(sim && Object.keys(sim.answers).length > 0, () => {
      coursePauseTools();
      guideLeaveActive('exam', renderSimHome);
    })
  );
  root.querySelectorAll('.opt').forEach(b =>
    b.addEventListener('click', () => {
      sim.answers[key] = +b.dataset.i;
      saveResume('sim', sim);
      root.querySelectorAll('.opt').forEach(x => x.classList.toggle('picked', x === b));
    })
  );
  ...
  root.querySelector('#next').addEventListener('click', () => {
    if (sim.idx + 1 >= s.items.length) renderSimReview();
    else {
      sim.idx++;
      renderSimQ();
    }
  ...
  const pt = root.querySelector('#pt');
  if (pt) pt.addEventListener('click', periodicModal);
...
function toggleNav(root) {
  ...
  const s = sim.queue[sim.si];
  w.innerHTML = `<div class="navgrid">${s.items
    .map((it, i) => {
      const k = sim.si + ':' + i;
      const cls = sim.answers[k] != null ? 'ans' : '';
      const fl = sim.flags[k] ? 'fl' : '';
      return `<button class="navcell ${cls} ${fl}" data-i="${i}">${i + 1}</button>`;
    })
    .join('')}</div>`;
  w.querySelectorAll('.navcell').forEach(c =>
...
function renderSimReview() {
  ...
  ng.innerHTML = s.items
    ...
    .join('');
  ng.querySelectorAll('.navcell').forEach(c =>
    c.addEventListener('click', () => {
      sim.idx = +c.dataset.i;
      renderSimQ();
    })
...
function submitSection() {
  ...
  sim.results.push({
    ...
    items: s.items,
    answers: Object.assign({}, sim.answers),
    elapsedMs:
      sim.sectionDurationMs == null
        ? null
        : Math.max(0, Math.min(sim.sectionDurationMs, sim.sectionDurationMs - Math.max(0, sim.deadline - t))),
  });
  if (sim.si + 1 < sim.queue.length) {
    sim.si++;
    renderBreak();
  } else {
    if (typeof bumpStreak === 'function') bumpStreak();
...
function finishSim() {
  ...
  const reserved =
    window.McatRehearsal &&
    sim.results.flatMap(r => r.items).find(it => !window.McatRehearsal.allowed(it.q.id, it.passageId));
  if (reserved) {
    if (sim.archived || saveResume('sim', sim))
      window.McatRehearsal.allow(
        { questionId: reserved.q.id, passageId: reserved.passageId },
        finishSim,
        renderLegacySimHome
      );
    return;
  ...
  if (!sim.archived) {
    courseArchiveExam(sim);
    clearResume('sim');
  }

  const tot = sim.results.reduce((a, r) => a + r.total, 0),
    cor = sim.results.reduce((a, r) => a + r.correct, 0);
  const root = el('<div></div>');
  root.appendChild(topbar('mcat'));
  const secRows = sim.results
    .map(
      r =>
        `<div class="calib-row"><span class="cl">${SIM_SECTIONS[r.key].abbr}</span><span class="cbar"><i style="width:${Math.round((100 * r.correct) / r.total)}%"></i></span><span class="cv">${r.correct}/${r.total}</span></div>`
    )
    .join('');
  const main = el(`<main class="panel"><section class="summary" style="border:0;margin-top:8px">
  ...
  const dr = main.querySelector('#dr');
  dr.innerHTML = `<span class="label">Review</span>`;
...
function periodicModal() {
  ...
  const picture = m.querySelector('img'),
    zoom = m.querySelector('select');
  zoom.addEventListener('change', () => {
    picture.style.width = `${zoom.value}%`;
  });
...
const TRACKS = {
  120: {
    label: '120-day steady',
    days: 120,
    newCards: 10,
    questions: 10,
    minutes: '60–90 min/day',
    carsDays: [1, 2, 4, 6],
    passageDays: [3, 6],
  },
  90: {
    label: '90-day focused',
    days: 90,
    newCards: 15,
    questions: 12,
    minutes: '90–120 min/day',
    carsDays: [1, 2, 4, 5, 6],
    passageDays: [2, 4, 6],
  },
  60: {
    label: '60-day intensive',
    days: 60,
    newCards: 20,
    questions: 15,
    minutes: '2–3 hr/day',
    carsDays: [1, 2, 3, 4, 5, 6],
    passageDays: [2, 3, 5, 6],
  },
};
...
function guideAddDays(key, days) {
  const d = guideDateFromKey(key);
  d.setDate(d.getDate() + days);
  return guideDateKey(d);
}
function guideFormatDate(key) {
  return guideDateFromKey(key).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' });
}
function saveGuidePlan(plan) {
  return StudyStorage.write('cs-mcat-plan', plan);
}
function guidePlan() {
...
function guideCategories() {
  ...
  MCAT.outline.concepts
    .filter(c => c.section !== 'cars')
    .forEach(c => c.categories.forEach(cat => out.push({ id: cat.id, title: cat.title, section: c.section })));
  return out;
}
function guideWeakCategory() {
  const byCat = {};
  currentPracticeLog()
    .filter(x => x.section !== 'cars')
    .forEach(x => {
      (byCat[x.category] ||= { n: 0, c: 0 }).n++;
      if (x.correct) byCat[x.category].c++;
    });
  const weak = Object.entries(byCat)
    .filter(([, v]) => v.n >= 2)
    .map(([id, v]) => ({ id, n: v.n, acc: Math.round((100 * v.c) / v.n) }))
    .sort((a, b) => a.acc - b.acc || b.n - a.n)[0];
...
function guideTaskKey(day, id) {
  return `${day}:${id}`;
}
function guideTaskDone(plan, day, id) {
  return !!(plan.completed && plan.completed[guideTaskKey(day, id)]);
}
function guideDayTasks(plan, day) {
  ...
  const tasks = [];
  const add = task =>
    tasks.push(Object.assign({ day, category: focus?.id || null, section: focus?.section || 'bioBiochem' }, task));
  ...
  if (phase === 'Taper') {
    add({
      ...
      desc: 'Review only what is due. Add no new material.',
      meta: 'Due cards · about 15 min',
      newCards: 0,
    });
    add({
      id: 'drill',
      type: 'drill',
      title: 'Light weak-area review',
      desc: focus ? `${focus.id} · ${focus.title}` : 'Review the weakest tested category',
  ...
  if (phase === 'Exam prep') {
    add({
      ...
      type: 'flash',
      title: 'Due flashcards',
      desc: 'Keep older material retrievable without adding much new content.',
      meta: 'Due cards · about 20 min',
      newCards: 0,
    });
    add({
      id: 'drill',
      type: 'drill',
      title: focus?.adaptive ? 'Weak-area repair' : 'Mixed weak-area drill',
      desc: focus ? `${focus.id} · ${focus.title}` : 'Target recent misses',
      meta: '20 questions · review every miss',
      questions: 20,
    });
    if (weekday !== 7)
      add({
        ...
        type: 'cars',
        title: 'CARS passage',
        desc: 'One passage with full answer review.',
        meta: '1 passage · untimed first',
      });
    if ((day - (plan.durationDays - 20)) % 7 === 0) {
      ...
      add({
        id: 'exam',
        type: 'exam',
        title: `${SEC_ABBR[section]} section simulation`,
        desc: 'Practice under the same no-feedback conditions as test day.',
        meta: 'Timed section',
        section,
      });
    } else if (weekday % 2 === 0) {
      add({
        ...
        desc: 'Read the experiment, work the data, then review every answer.',
        meta: '1 passage · about 20 min',
      });
    }
    return tasks;
  }
  ...
  add({
    ...
    desc: focus ? `${focus.id} · ${focus.title}` : 'New and due cards',
    meta: `${track.newCards} new + anything due`,
    newCards: track.newCards,
  });
  add({
    id: 'drill',
    type: 'drill',
    title: focus?.adaptive ? 'Weak-area drill' : 'Focused question drill',
    desc: focus ? `${focus.id} · ${focus.title}` : 'Current content block',
    meta: `${track.questions} questions · full review`,
    questions: track.questions,
  });
  if (track.carsDays.includes(weekday))
    add({
      id: 'cars',
      type: 'cars',
      title: 'CARS passage',
      desc: 'Read for structure and justify every answer from the text.',
      meta: '1 passage · about 15 min',
    });
  if (track.passageDays.includes(weekday))
    add({
      id: 'passage',
      type: 'passage',
      title: 'Science passage',
      desc: `Apply ${focus ? focus.id : 'today’s content'} to an experiment or data set.`,
      meta: '1 passage · about 20 min',
    });
  return tasks;
}
function guideLeastAttempted(list) {
  ...
  return list
    ...
    .sort(
      (a, b) =>
        QLOG.filter(x => x.passage === a.id).length - QLOG.filter(x => x.passage === b.id).length ||
...
function guideStartTask(task, plan) {
  ...
  studyTouch(plan);
  if (task.type === 'flash') {
    startFlash('all', task.newCards, task.limitDue ?? 60, task.category, task.limitCards ?? Infinity);
    return;
  }
  if (task.type === 'drill') {
    ...
    drill = {
      qs: shuffleArr(pool).slice(0, Math.min(task.questions, pool.length)),
      idx: 0,
      mode: 'standard',
      results: [],
      scope: task.category || task.section,
    };
    renderDrillQ();
    return;
  }
  if (task.type === 'cars') {
    const passage = guideLeastAttempted(MCAT.cars);
    if (passage) startCars(passage, true);
    else {
      guideClearActiveTask();
      renderGuide();
    }
  ...
  if (task.type === 'passage') {
    ...
    if (passage) startPassage(passage, false);
    else {
      guideClearActiveTask();
      renderGuide();
    }
...
function guideLeaveActive(expectedType, fallback) {
  const plan = guidePlan();
  if (!plan?.active || plan.active.type !== expectedType) {
    fallback();
    return;
  }
  renderGuide();
...
function buildPlan(track) {
  ...
  for (let w = 0; w < weekCount; w++) {
    const firstDay = w * 7 + 1,
      lastDay = Math.min(config.days, firstDay + 6);
    const ids = [];
    ...
    const note =
      phase === 'Build'
        ? titles.join(' · ')
        : phase === 'Taper'
          ? 'Light review, sleep, logistics, and confidence.'
          : 'Section practice, weak-area repair, daily recall, and passage review.';
    plan.weeks.push({ n: w + 1, firstDay, lastDay, phase, focus: ids, note });
  }
  return plan;
}
function showPlan(host, plan) {
  host.innerHTML = plan.weeks
    .map(
      w =>
        `<div class="guide-week"><span class="guide-week-num">Week ${w.n}</span><span class="guide-week-days">Days ${w.firstDay}–${w.lastDay}</span><strong>${w.phase}</strong><p>${esc(w.note)}</p></div>`
    )
    .join('');
}
...
function renderGuide(useOriginal = false) {
  ...
  if (!existing) {
    ...
    const main = el(`<main class="panel guide-page today-page guide-welcome">
      ...
      <div class="guide-track-list">
        ${TRACK_ORDER.map(key => {
          ...
          return `<button class="guide-track ${key === defaultTrack ? 'active' : ''}" data-track="${key}" aria-pressed="${key === defaultTrack}"><span><strong>${track.label}</strong><small>Reference pace: ${track.minutes}</small></span><span>Target ${guideFormatDate(guideAddDays(guideDateKey(), track.days - 1))}</span></button>`;
        }).join('')}
      </div>
    ...
    let minutes = 15;
    const preview = () => {
      const session = studyBuildSession(buildFlexiblePlan(minutes), minutes),
        next = session.tasks[0];
      main.querySelector('#first-session-preview').innerHTML = next
        ? `<div class="study-next-step"><span class="label">${next.resume ? 'SAVED WORK' : 'UP FIRST'} · About ${next.minutes} min</span><h2>${esc(next.title)}</h2><p>${esc(next.desc || '')}</p></div>`
        : '<p role="status">Study activities could not load. Reload to try again, or browse your saved work below.</p>';
      main.querySelector('#first-session-detail').textContent = next
        ? `${session.tasks.length} ${session.tasks.length === 1 ? 'activity' : 'activities'} · About ${session.tasks.reduce((n, t) => n + t.minutes, 0)} minutes including review`
        : '';
    ...
    main.querySelectorAll('[data-first-minutes]').forEach(
      button =>
        (button.onclick = () => {
          minutes = Number(button.dataset.firstMinutes);
          main
            .querySelectorAll('[data-first-minutes]')
    ...
    main.querySelector('#first-start').onclick = () => {
      ...
      if (next) studyLaunchTask(next, plan);
      else renderGuide();
    };
    preview();
    ...
    main.querySelectorAll('[data-track]').forEach(button =>
      button.addEventListener('click', () => {
        track = button.dataset.track;
        main.querySelectorAll('[data-track]').forEach(item => {
    ...
    main.querySelector('#begin').addEventListener('click', () => {
      saveGuidePlan(buildPlan(track));
      renderGuide();
    });
    main.querySelector('#back').addEventListener('click', renderMCAT);
    root.appendChild(main);
    mcatWorkspace(root, 'today');
    studySetView(root);
    window.scrollTo(0, 0);
    return;
  }
  ...
  const activeTask = tasks.find(
    task =>
      !guideTaskDone(plan, day, task.id) &&
      plan.active?.day === day &&
      plan.active?.id === task.id &&
      plan.active?.type === task.type
  );
  const nextTask = activeTask || tasks.find(task => !guideTaskDone(plan, day, task.id));
  const phase = guidePhase(plan, day);
  const focus = guideFocusCategory(plan, day);
  const calendarStart = guideDateFromKey(plan.startDate),
    calendarNow = new Date();
  const elapsedDays = Math.max(
    0,
    (Date.UTC(calendarNow.getFullYear(), calendarNow.getMonth(), calendarNow.getDate()) -
  ...
  const main = el(`<main class="panel guide-page today-page">
    ...
    <section class="guide-today">
      ...
      <div class="guide-task-list">${tasks
        .map((task, index) => {
  ...
  main.querySelectorAll('[data-guide-task]').forEach(button =>
    button.addEventListener('click', () => {
  ...
  main.querySelector('#restart')?.addEventListener('click', () => {
    if (
      !confirm(
        'Restart your guided MCAT plan? Completed plan days will be cleared. Your flashcard and question history will stay.'
      )
    )
      return;
    StudyStorage.remove('cs-mcat-plan');
    Object.values(courseState.units).forEach(r => delete r.guideTask);
    saveCourse();
    // Keep unfinished study work, but detach assignments from the retired calendar.
    RESUME_SPECS.forEach(spec => {
      const saved = loadResume(spec.key);
      if (saved?.guideTask) {
        delete saved.guideTask;
        saveResume(spec.key, saved);
      }
    });
    if (repairState?.active?.guideTask) {
      delete repairState.active.guideTask;
      saveMcatRepair();
    }
    renderGuide();
  });
...
function mapperState() {
  return StudyStorage.read('cs-mcat-coursework', {});
}
function renderMapper() {
  ...
  ['bioBiochem', 'chemPhys', 'psychSoc'].forEach(sk => {
    ...
    const wrap = el(
      `<div class="statblock"><span class="label">${sec.abbr} — ${esc(sec.name)}</span><div class="maprows"></div></div>`
    );
    const mr = wrap.querySelector('.maprows');
    MCAT.outline.concepts
      .filter(c => c.section === sk)
      .forEach(con =>
        con.categories.forEach(cat => {
          const v = state[cat.id] || '';
          const row = el(
            `<div class="maprow"><span class="mr-id">${cat.id}</span><span class="mr-title">${esc(cat.title)}</span><span class="mr-rate">${['strong', 'ok', 'weak'].map(r => `<button class="rb ${v === r ? 'on ' + r : ''}" data-cat="${cat.id}" data-r="${r}">${r[0].toUpperCase()}</button>`).join('')}</span></div>`
          );
          row.querySelectorAll('.rb').forEach(b =>
            b.addEventListener('click', () => {
              const s = mapperState();
              s[b.dataset.cat] = s[b.dataset.cat] === b.dataset.r ? '' : b.dataset.r;
              StudyStorage.write('cs-mcat-coursework', s);
              renderMapper();
            })
          );
          mr.appendChild(row);
  ...
  if (!start.length)
    sl.innerHTML = '<div class="empty">All rated — nice. Drill your weak ones from the Blueprint.</div>';
  else
    start.forEach(cat => {
      const qn = MCAT.questions.filter(q => q.category === cat.id).length;
      const row = el(
        `<button class="bp-cat"><span class="bp-cat-id">${cat.id}</span><span class="bp-cat-title">${esc(cat.title)}</span><span class="bp-cat-stat ${state[cat.id] === 'weak' ? 'no' : ''}">${state[cat.id] === 'weak' ? 'weak' : 'unrated'}</span></button>`
      );
      if (qn)
        row.addEventListener('click', () => {
          drill = {
            qs: shuffleArr(MCAT.questions.filter(q => q.category === cat.id)).slice(0, 10),
            idx: 0,
            mode: 'standard',
            results: [],
            scope: cat.id,
...
document.addEventListener('keydown', e => {
  ...
  if (flash && document.querySelector('.flash-stage')) {
    if (e.key === ' ' && document.querySelector('#flash-back')?.style.display === 'none') {
      e.preventDefault();
      flash._reveal && flash._reveal();
      return;
    }
    const map = { 1: 'again', 2: 'hard', 3: 'good', 4: 'easy' };
    if (map[e.key] && document.querySelector('.ratebtn')) {
      document.querySelector(`.ratebtn.${map[e.key]}`)?.click();
    }
    return;
  }
...
function resetMcatState() {
  ...
  repairState = McatRepairCore.empty();
  repairSaveFailed = false;
...
window.addEventListener('pagehide', () => {
  if (sim && simTimerId) saveResume('sim', sim);
});
let mcatPausedTimers = [];
window.addEventListener('study-storage-paused', () => {
  if (typeof v2Clock !== 'undefined' && v2Clock) {
    v2Tick();
    v2Clock = null;
    v2Save();
  }
  for (const [key, run] of [
    ...
    if (run?.timerId) {
      mcatPausedTimers.push({ key, run, remaining: Math.max(0, run.deadline - nowTs()) });
      clearInterval(run.timerId);
      run.timerId = null;
      saveResume(key, run);
    }
  if (sim && simTimerId) {
    mcatPausedTimers.push({ key: 'sim', run: sim, remaining: Math.max(0, sim.deadline - nowTs()) });
    clearInterval(simTimerId);
    simTimerId = null;
    saveResume('sim', sim);
  }
...
window.addEventListener('study-storage-recovered', () => {
  courseSaveFailed = false;
  courseExamSaveFailed = false;
  repairSaveFailed = false;
  v2SaveFailed = false;
  const note = document.querySelector('#course-note-status');
  if (note) note.textContent = 'Saved in this browser.';
  const status = document.querySelector('#v2-save-status');
  ...
  for (const { key, run, remaining } of mcatPausedTimers) {
    if (key === 'sim' && sim === run && !run.finishedAt && document.querySelector('#sim-timer')) {
      run.deadline = nowTs() + remaining;
      simTimerId = setInterval(simTick, 500);
    }
    if (key === 'cars' && cars === run && run.phase === 'attempt' && document.querySelector('#cars-timer')) {
      run.deadline = nowTs() + remaining;
      run.timerId = setInterval(carsTick, 500);
    }
    if (key === 'plab' && plab === run && run.phase === 'attempt' && document.querySelector('#plab-timer')) {
      run.deadline = nowTs() + remaining;
      run.timerId = setInterval(plabTick, 500);
    }
  }
  mcatPausedTimers = [];
  const activity = document.querySelector('[data-v2-activity]');
  if (activity) v2BeginActivity(activity.dataset.v2Activity, activity.dataset.v2Key);
});
