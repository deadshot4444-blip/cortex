...
for (const coach of path.coaches) {
  ...
  assert.equal(coach.model.filter(item => item.label.startsWith('Paragraph')).length, coach.passage.text.split(/\n\n+/).length);
  for (const q of coach.passage.questions) {
    assert.equal(ids.has(q.id), false); ids.add(q.id);
...
const bad = structuredClone(path); bad.coaches[0].passage.questions[0].evidenceParagraphs = [99];
assert.equal(core.validCarsPath(bad), false);
assert.deepEqual(core.priorQuestionIds(path.coaches[0].passage, { coachExposures: { [path.coaches[0].passageId]: { ts: 1 } } }), path.coaches[0].passage.questions.map(q => q.id));

let fail = false, lastHtml = '', saved;
const nodes = new Map();
function element(html) {
  ...
  }, querySelectorAll: () => [], classList: { add() {} }, dataset: {}, appendChild() {} };
}
const context = vm.createContext({ console, Date, Math, JSON, Set, Map, URL, URLSearchParams, path, base,
  window: { addEventListener() {} }, document: { addEventListener() {}, querySelector: () => null }, setInterval() {},
  StudyStorage: { paused: false, read: (_,fallback) => fallback, watch: () => ({ save(value) { if(fail)return false;saved=JSON.stringify(value);return true; } }) },
  McatV2Core: core, MCAT: { cars:bank,sci:[] }, QLOG:[],QHIST:{},loadResume:()=>null,loadJSON:(_,fallback)=>fallback,esc:String,el:element,
});
vm.runInContext(fs.readFileSync('mcat-v2.js', 'utf8'), context);
const run = code => vm.runInContext(code, context);
run('v2Data=JSON.parse(JSON.stringify(base));v2CarsPath=JSON.parse(JSON.stringify(path));v2Data.coaches.push(...v2CarsPath.coaches);v2Shell=()=>{};v2CompleteActivity=()=>{};');
assert.equal(run('v2CoachAllowed(v2Coach("coach-cars-archive"))'), false);
run('v2State.coach.history=v2CarsPath.steps.slice(0,3).map(step=>({id:step.coachId,coachId:step.coachId,completedAt:1,answers:[]}));');
...
run(`var active=v2State.coach.active;active.stage='blind';active.answers=active.content.passage.questions.map((q,i)=>({qId:q.id,chosen:i? q.answer:(q.answer+1)%4,correct:i!==0,evidence:'First evidence',ts:10,assisted:false,repeat:false}));var first=JSON.stringify(active.answers);renderV2Coach=realRender;renderV2CoachBlind();`);
assert.doesNotMatch(lastHtml, /Authored model|Before-feedback review:|Your option:/);
for (let i=0;i<6;i++) {
  run(`var question=active.content.passage.questions[${i}];active.revisionDraft[question.id]={chosen:question.answer,evidence:'Revised reasoning tied to the passage.'};`);
  ...
  if(i===0){fail=false;run('v2Save();renderV2CoachBlind();');}
}
...
console.log('Two original passages, 12 rationale-linked checks, reserved-pool separation, staged access, exposure tracking, content snapshots, and separate before-feedback revision through save failure passed.');
