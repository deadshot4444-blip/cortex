...
const outline = read('mcat-outline'),
  course = read('mcat-course');
const banks = {
  questions: read('mcat-questions'),
  cards: read('mcat-cards'),
  sci: read('mcat-science-passages'),
  cars: read('mcat-cars'),
};
const categories = new Map(
  outline.concepts.flatMap(concept =>
    concept.categories.map(category => [category.id, { ...category, section: concept.section }])
  )
);
...
for (const item of [
  ...banks.questions,
  ...banks.cards,
  ...banks.sci.flatMap(p => p.questions.map(q => ({ ...q, section: p.section }))),
]) {
...
for (const unit of course.units) {
  ...
  assert.ok(
    unit.categories.every(id => categories.has(id)),
    unit.id
  );
  for (const topic of unit.topics)
    assert.ok(
      unit.categories.some(id => categories.get(id).topics.includes(topic)),
      `${unit.id}: unlinked topic ${topic}`
    );
}
...
assert.equal(
  banks.sci.find(p => p.id === 'cp8').questions.every(q => q.category === '5E'),
  true
);
...
const duplicates = rows(outline, course, {
  ...banks,
  questions: [...banks.questions, ...banks.questions],
  cards: [...banks.cards, ...banks.cards],
  sci: [...banks.sci, ...banks.sci],
  cars: [...banks.cars, ...banks.cars],
});
assert.deepEqual(
  duplicates.map(row => [row.id, row.practiceCount, row.cardCount]),
  actual.map(row => [row.id, row.practiceCount, row.cardCount])
);
console.log(
  'Canonical categories, section and topic alignment, available assessments, gaps, and unique inventory counts passed.'
);
