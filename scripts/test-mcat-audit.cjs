...
const now = new Date(2026, 8, 7, 9).getTime(),
  start = core.dateKey(new Date(now));
...
test('A completed exam review remains visible and consumes its 20-minute reservation', () => {
  const s = state();
  s.weekly.exams = [{ id: 'exam', name: 'Practice', date: core.addDays(start, -1), minutes: 120, reviewedAt: now }];
  const today = core.week(s, input())[0],
    review = today.tasks.find(t => t.type === 'externalReview');
  assert.ok(review?.done);
  assert.equal(review.minutes, 20);
  assert.ok(today.tasks.reduce((n, t) => n + t.minutes, 0) <= 30);
  assert.equal(
    core
      .week(s, input())
      .flatMap(d => d.tasks)
      .filter(t => t.type === 'externalReview').length,
    1
  );
});
...
test('Later checks only enter Today after the actual unlock time', () => {
  const s = state(),
    i = input();
  i.units = [{ id: 'unit', title: 'Unit', section: 'chemPhys' }];
  i.records.unit = { completedAt: now - 86400000, dueAt: now + 8 * 3600000 };
  let days = core.week(s, i);
  assert.equal(
    days[0].tasks.some(t => t.kind === 'delayed'),
    false
  );
  assert.ok(days.slice(1).some(d => d.tasks.some(t => t.kind === 'delayed')));
  days = core.week(s, { ...i, now: now + 8 * 3600000 });
  assert.ok(days[0].tasks.some(t => t.kind === 'delayed'));
});
...
function evaluate(text) {
  ...
  assert.match(expression, /^[\d\s+*/().\-Mathlog]+$/);
  return Function('return ' + expression)();
}
test('Every notation and logarithm variant has exactly one numerically correct setup', () => {
  for (const skill of ['notation', 'logs'])
    for (let v = 0; v < 8; v++) {
      const q = core.quant(skill, v),
        matches = q.setups.map(evaluate).map(x => Math.abs(x - q.value) < 1e-5);
      assert.equal(matches.filter(Boolean).length, 1, q.id);
      assert.equal(matches[q.answer], true, q.id);
    }
});
test('Interrupted passage exposure counts, but unseen exam queue entries do not', () => {
  const p = { id: 'p', questions: [{ id: 'p1' }, { id: 'p2' }] },
    item = { passageId: 'p', q: { id: 'p1' } };
  ...
  sim.seen = ['0:1'];
  assert.deepEqual(core.priorQuestionIds(p, { resumes: { sim } }), ['p1', 'p2']);
  delete sim.seen;
  sim.answers['0:1'] = 0;
  assert.deepEqual(core.priorQuestionIds(p, { resumes: { sim } }), ['p1', 'p2']);
});
test('Completed rehearsal displays and answered legacy passages make every later coach question repeated', () => {
  ...
  assert.deepEqual(core.priorQuestionIds(p, { qlog: [{ passage: 'p', qId: 'p1', ts: 100, unanswered: true }] }), [
    'p1',
  ]);
...
test('Exposure correction preserves original choices, correctness and prior flags, and ignores later displays', () => {
  const p = { id: 'p', questions: [{ id: 'p1' }, { id: 'p2' }] },
    run = {
      ...
      prior: ['p1'],
      answers: [
        { qId: 'p1', chosen: 3, correct: false, repeat: true },
        { qId: 'p2', chosen: 1, correct: true, repeat: false },
      ],
    };
  const state = core.normalize({ coach: { active: null, parked: [], history: [run] } }),
    saved = state.coach.history[0];
  ...
  assert.deepEqual(
    saved.answers.map(a => [a.chosen, a.correct, a.repeat]),
    [
      [3, false, true],
      [1, true, true],
    ]
  );
  assert.deepEqual(saved.exposureCorrection.previousRepeat, [
    { qId: 'p1', repeat: true },
    { qId: 'p2', repeat: false },
  ]);
  assert.deepEqual(saved.prior, ['p1', 'p2']);
  assert.equal(core.reconcileCoachExposure(state, { passageDisplays: { p: 100 } }), false);
});
