...
test('Legacy workshop records normalize without changing saved answers', () => {
  const active = {id:'a',coachId:'one',answers:[{qId:'q',chosen:2,correct:false}],notes:['map'],hints:{q:1},draft:{q2:{chosen:1}}};
...
test('Parking and switching survive serialization with both complete drafts and plan links', () => {
  ...
  const a = {id:'a',coachId:'one',stage:'questions',answers:[{qId:'q1',chosen:2,correct:false}],notes:['map'],roles:['context'],hints:{q1:1},draft:{q2:{chosen:1,evidence:'draft'}}};
