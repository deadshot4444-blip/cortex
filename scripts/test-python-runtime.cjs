...
class BrowserWorker {
  constructor(url) {
    assert.match(url, /^python-runtime-worker\.js\?v=\d+$/);
...
const context = vm.createContext({
  ...
  setTimeout: (handler, milliseconds) =>
    setTimeout(handler, milliseconds === 10000 ? 100 : milliseconds === 45000 ? 600 : milliseconds),
  clearTimeout,
});
...
(async () => {
  ...
  const busy = await context.runPythonCode('second run');
  ...
  assert.match(timedOut.stdout, /before loop/);
  assert.match(output.stdout, /before loop/);
  assert.equal(workers[0].terminated, true);
  await workers[0].exited;
  const retried = await context.runPythonCode('normal');
  assert.equal(retried.ok, true);
  assert.equal(retried.stdout, 'first\nsecond\n');
  ...
  const fresh = await context.runPythonCode('globals');
  assert.equal(JSON.parse(fresh.stdout).RECORDING, undefined, 'Input globals do not leak into another run');
  const overflow = await context.runPythonCode('overflow');
  assert.equal(overflow.ok, false);
  assert.match(overflow.stderr, /Output limit/);
  assert.ok(overflow.stdout.length <= 32768);
  nextMode = 'load-error';
  const loadError = await context.runPythonCode('normal');
  assert.equal(loadError.ok, false);
  assert.match(loadError.stderr, /download failure/);
  nextMode = 'load-stall';
  const controller = new AbortController();
  const loading = context.runPythonCode('normal', { signal: controller.signal });
  controller.abort();
  const stopped = await loading;
  assert.equal(stopped.reason, 'stopped');
  assert.equal(workers.at(-1).terminated, true);
  const count = workers.length;
  const alreadyAborted = await context.runPythonCode('normal', { signal: controller.signal });
  assert.equal(alreadyAborted.reason, 'stopped');
  assert.equal(workers.length, count);
  const abortAtStatus = new AbortController();
  const canceledBeforeWorker = await context.runPythonCode('normal', {
    signal: abortAtStatus.signal,
    onStatus: () => abortAtStatus.abort(),
  });
  assert.equal(canceledBeforeWorker.reason, 'stopped');
  assert.equal(workers.length, count, 'Synchronous cancellation cannot leave an orphan worker');
  nextMode = 'load-stall';
  const stalled = await context.runPythonCode('normal');
  assert.equal(stalled.reason, 'load-timeout');
  assert.equal(workers.at(-1).terminated, true);
  const unavailablePackage = await context.runPythonCode('normal', { packages: ['missing'] });
  assert.equal(unavailablePackage.ok, false);
  const savingPaused = context.runPythonCode('spin');
  events.get('study-storage-paused')();
  assert.equal((await savingPaused).reason, 'stopped');
  const pageClosed = context.runPythonCode('spin');
  events.get('pagehide')();
  assert.equal((await pageClosed).reason, 'stopped');
  const noWorker = vm.createContext({ window: { addEventListener() {} } });
  vm.runInContext(runtime, noWorker);
  assert.equal((await noWorker.runPythonCode('normal')).reason, 'unsupported');
  assert.ok(workers.every(worker => worker.terminated));
  await Promise.all(workers.map(worker => worker.exited));
  console.log(
    'Worker contract: blocked-loop termination, responsive main thread, streaming/newlines, output cap, clean retry, fresh globals, load/package failure, abort, page exit and save-pause cancellation passed. Real browser/Pyodide verification remains pending.'
  );
})().catch(async error => {
