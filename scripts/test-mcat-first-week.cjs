...
function setup() {
  ...
  const context = vm.createContext({
    ...
    repairData,
    repairConcept: id => repairData.concepts.find(c => c.id === id),
    inputCourse: json('data/mcat-course.json'),
    inputMcat: {
      outline: json('data/mcat-outline.json'),
      cards: json('data/mcat-cards.json'),
      questions: json('data/mcat-questions.json'),
      cars: json('data/mcat-cars.json'),
      sci: json('data/mcat-science-passages.json'),
    },
  });
  const run = code => vm.runInContext(code, context);
  for (const file of ['study-storage.js', 'mcat.js', 'mcat-course.js', 'mcat-workflows.js'])
...
test('A first 15-minute session starts one lesson without choosing a test date', () => {
  const { run } = setup();
  const result = run(
    `var plan=buildFlexiblePlan(15); var session=studyDailySession(plan); ({plan:guidePlan(),session})`
  );
...
test('The first-session action resumes an unfinished five-minute concept without replacing its answer', () => {
  ...
  assert.equal(result.task.type, 'repair');
  assert.equal(result.task.resume, true);
...
test('Existing dated tracks retain their schedule and completion identities', () => {
  const { run, advance } = setup();
  const before = run(
    `var plan=buildPlan('90');plan.completed['1:flash']=Date.now();saveGuidePlan(plan);JSON.stringify(plan)`
  );
...
test('Seven days of flexible study stay bounded and do not invent completed work', () => {
  for (const budget of [15, 30, 60]) {
    ...
    for (let day = 1; day <= 7; day++) {
      ...
      assert.ok(!seen.has(session.date));
...
test('A lesson completed after an interruption belongs to its actual completion day', () => {
  ...
  assert.equal(result.day.tasks.length, 1);
  assert.equal(result.day.tasks[0].done, true);
