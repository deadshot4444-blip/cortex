...
function setup(seed=new Map()){
  ...
  const context=vm.createContext({console,Date:TestDate,Event,localStorage:storage,CortexProgress:{OWNER:'owner'},loadJSON:(key,fallback)=>JSON.parse(data.get(key)||'null')??fallback,location:{reload(){}},
...
test('Successful saves, reads, and removals remain compatible',()=>{
  const s=setup();assert.equal(s.run('McatStorage.write("draft",{note:"first"})'),true);
  assert.equal(s.run('McatStorage.read("draft",null).note'),'first');
  assert.equal(s.run('McatStorage.remove("draft")'),true);assert.equal(s.data.has('draft'),false);
});
test('Failed saves preserve both copies, expose a modal, and retry the newest queued draft',()=>{
  ...
  assert.equal(s.run('McatStorage.write("draft",{note:"unsaved"})'),false);
  assert.equal(s.nodes.get('#mcat-conflict-title').textContent,'Your latest work could not be saved.');
  assert.equal(s.run('McatStorage.read("draft",null).note'),'unsaved');
  s.run('McatStorage.write("draft",{note:"newest"})');
  assert.equal(JSON.parse(s.data.get('draft')).note,'saved');
  assert.equal(s.run('McatStorage.recovery().records.draft.thisTab.note'),'newest');
  assert.equal(s.run('McatStorage.recovery().records.draft.saved.note'),'saved');
  assert.equal(s.run('McatStorage.retry()'),false);
  s.failWrite(null);assert.equal(s.run('McatStorage.retry()'),true);
  assert.equal(JSON.parse(s.data.get('draft')).note,'newest');assert.equal(s.run('McatStorage.paused'),false);
});
test('Queued archive writes finish before the resumable copy is deleted',()=>{
  ...
  s.run('McatStorage.write("archive",{answer:1});McatStorage.remove("resume")');
  assert.ok(s.data.has('resume'));s.failWrite(null);s.run('McatStorage.retry()');
...
test('An accidental close is guarded, while choosing Reload still keeps stale writes paused',()=>{
  const s=setup();s.failWrite(()=>true);s.run('McatStorage.write("draft",{note:"pending"})');
  let prompts=0;const event={type:'beforeunload',preventDefault(){prompts++;}};
  s.dispatch(event);assert.equal(prompts,1);
  s.nodes.get('#mcat-conflict-reload').onclick();s.dispatch(event);assert.equal(prompts,1);
  assert.equal(s.run('McatStorage.paused'),true);assert.equal(s.run('McatStorage.write("draft",{note:"late event"})'),false);
});
test('Retry preflights every observed key before changing any saved copy',()=>{
  const s=setup(new Map([['first','1'],['second','2']]));s.run('McatStorage.read("second",null)');s.failWrite(()=>true);
  s.run('McatStorage.write("first",3)');s.data.set('second','4');s.failWrite(null);
  assert.equal(s.run('McatStorage.retry()'),false);assert.equal(s.data.get('first'),'1');
  assert.equal(s.run('McatStorage.conflicted'),true);assert.equal(s.nodes.get('#mcat-save-retry').hidden,true);
});
test('Cross-tab changes pause immediately and retain the old tab draft for recovery',()=>{
  const s=setup(new Map([['draft','{"note":"old"}']]));s.run('var current={note:"this tab"};McatStorage.watch("draft",()=>current)');
  s.data.set('draft','{"note":"other tab"}');s.dispatch({type:'storage',storageArea:s.storage,key:'draft'});
  assert.equal(s.run('McatStorage.conflicted'),true);
  assert.equal(s.run('McatStorage.recovery().records.draft.thisTab.note'),'this tab');
  assert.equal(s.run('McatStorage.recovery().records.draft.saved.note'),'other tab');
  assert.equal(s.run('McatStorage.write("draft",current)'),false);
});
test('Read failures and malformed JSON never get replaced with default progress',()=>{
  for(const damaged of [false,true]){
    ...
    s.run('McatStorage.read("draft",{});McatStorage.write("draft",{})');s.failRead(null);
    assert.equal(s.run('McatStorage.retry()'),false);assert.equal(s.data.get('draft'),damaged?'{broken':'{"note":"saved"}');
  }
});
test('An account switch prevents writes and excludes the new account from the recovery export',()=>{
  const s=setup(new Map([['owner','A'],['draft','{"note":"A saved"}']]));s.run('McatStorage.read("draft",null)');
  s.data.set('owner','B');s.data.set('draft','{"note":"B private"}');
  assert.equal(s.run('McatStorage.write("draft",{note:"A pending"})'),false);
  const text=s.run('JSON.stringify(McatStorage.recovery())');assert.ok(!text.includes('B private'));assert.ok(text.includes('A saved'));assert.ok(text.includes('A pending'));
  assert.equal(s.run('McatStorage.recovery().accountChanged'),true);
});
test('A serialization failure cannot falsely report recovery or delete the resumable copy',()=>{
  ...
  s.run('var circular={};circular.self=circular;McatStorage.write("archive",circular);McatStorage.remove("resume")');
  assert.equal(s.run('McatStorage.retry()'),false);assert.ok(s.data.has('resume'));assert.equal(s.data.has('archive'),false);
  assert.doesNotThrow(()=>s.run('JSON.stringify(McatStorage.recovery())'));
  s.run('delete circular.self;circular.answer=1');assert.equal(s.run('McatStorage.retry()'),true);assert.equal(s.data.has('resume'),false);
});
...
test('A saved session that cannot open is kept for recovery instead of deleted',()=>{
  ...
  assert.equal(s.run('McatStorage.paused'),true);assert.equal(s.nodes.get('#mcat-conflict-title').textContent,'Your saved session could not open.');
});
...
test('Timed work pauses during failed saving and resumes with the same time remaining',()=>{
  ...
  s.advance(120000);s.failWrite(null);s.run('McatStorage.retry()');
