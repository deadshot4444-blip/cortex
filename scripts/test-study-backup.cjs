...
(async () => {
  await test('Raw study records, Unicode notes and legacy preferences round-trip without rewriting', async () => {
    const data = {
      'cs-neuro': '{ "code": {"x": {"draft": "def f(x):\\n    return x < 2"}}, "note": "α → β" }',
      'cs-mcat-log': '[{"correct":false}]',
      'cs-mode': 'timed',
      'cs-seen-ver': '2.4.0-local.1',
    };
    const text = await Backup.create(data, '2.4.0-local.1', options),
      file = await Backup.parse(text, options);
    assert.deepEqual(file.data, data);
    assert.equal(file.createdAt, options.now);
    assert.equal(file.scope, 'active-workspace');
  ...
  await test('Integrity covers metadata and raw data, independent of record ordering', async () => {
    const text = await Backup.create({ 'cs-mode': 'timed', 'cs-diff': 'all' }, 'test', options),
      file = JSON.parse(text);
    file.data = Object.fromEntries(Object.entries(file.data).reverse());
    await Backup.parse(JSON.stringify(file), options);
    file.data['cs-mode'] = 'untimed';
    await assert.rejects(Backup.parse(JSON.stringify(file), options), /integrity/i);
    const metadata = JSON.parse(text);
    metadata.appVersion = 'changed';
    await assert.rejects(Backup.parse(JSON.stringify(metadata), options), /integrity/i);
  });
  await test('Future formats, recovery files, truncated JSON and oversized files are rejected', async () => {
    const file = JSON.parse(await Backup.create({}, 'test', options));
    file.version = 2;
  ...
  await test('Unknown records, tokens, malformed JSON, wrong root shapes and prototype keys are rejected', async () => {
    for (const data of [
      ...
      { 'cs-neuro': '[]' },
      { 'cs-mcat-log': '{}' },
      { 'cs-neuro': '{"__proto__":{"pwned":true}}' },
      { 'cs-neuro': '{"x":{"constructor":{}}}' },
    ])
      await assert.rejects(Backup.create(data, 'test', options));
  ...
  await test('Unsafe source protocols, injected IDs, nonfinite values and excessive nesting are rejected', async () => {
    for (const value of [
      ...
      '{"url":"https://example.org/\\" onclick=bad"}',
      '{"id":"x\\" onfocus=bad"}',
      '{"x":1e999}',
    ])
      await assert.rejects(Backup.create({ 'cs-neuro': value }, 'test', options));
    let deep = {};
    for (let i = 0; i < 70; i++) deep = { child: deep };
    await assert.rejects(Backup.create({ 'cs-neuro': JSON.stringify(deep) }, 'test', options), /complex/);
      ...
      createElement: () => ({
        ...
        content: {
          querySelectorAll: () => [
            ...
            { tagName: 'B', attributes: [{ name: 'onclick' }] },
          ],
        },
      }),
    };
  ...
  await test('Learner free text with angle-bracket shorthand exports verbatim and restores', async () => {
    const { JSDOM } = require('jsdom'),
      ...
      o = { ...options, checkMarkup: value => Backup.safeMarkup(value, doc) };
    assert.equal(
      Backup.safeMarkup('Na<K and K>Cl', doc),
      false,
      'the HTML parser reads <K and K> as a start tag, so this is why learner text is exempt'
    );
    const data = {
      'cs-clinical-shift-v1':
        '{"version":1,"active":{"note":{"assessment":"Na<K and K>Cl in this patient","plan":"Replete K<3.5 first"},"drafts":{},"differential":{"rationale":"<3 hours of symptoms"}},"history":[{"encounter":{"note":{"assessment":"HCO3<18"}}}]}',
      'cs-mcat-course-v1': '{"units":{"u1":{"notes":"Km<Ka means higher affinity"}}}',
      'cs-cogpsych':
        '{"lessons":{"l1":{"steps":{"0":{"draft":"RT<200 ms is implausible","content":{"body":"<p>Authored</p>"}}}}}}',
      ...
      'cs-academy-anatomy-v1':
        '{"lessons":{"a":{"draft":"<2 cm nodes","firstDraft":"<2 cm","comparison":"I said <2 cm"}}}',
    ...
    const text = await Backup.create(data, 'test', o);
    assert.deepEqual((await Backup.parse(text, o)).data, data);
  ...
  await test('Authored snapshot fields still reject script, image and event-handler markup on export and on restore', async () => {
    ...
    for (const html of [
      ...
      const data = {
        'cs-cogpsych': JSON.stringify({ lessons: { l1: { steps: { 0: { content: { body: html }, draft: 'ok' } } } } }),
      };
      await assert.rejects(
        Backup.create(data, 'test', o),
        error =>
          /no backup was prepared/.test(error.message) &&
          /cs-cogpsych/.test(error.message) &&
    ...
    const inert = {
      'cs-cogpsych': '{"lessons":{"l1":{"steps":{"0":{"content":{"body":"<p>First <b>answer</b>.</p>"}}}}}}',
    };
  ...
  await test('All five existing MCAT resume kinds remain portable', async () => {
    const data = Object.fromEntries(
      ['flash', 'drill', 'cars', 'plab', 'sim'].map(k => ['cs-mcat-r-' + k, '{"idx":1,"results":[]}'])
    );
  ...
  console.log(
    `${checks} portable backup checks passed. DOM parsing and browser file dialogs remain browser-test work.`
  );
})().catch(error => {
  console.error(error);
  process.exitCode = 1;
