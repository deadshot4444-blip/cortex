...
function inventory(load) {
  ...
  for (const file of files) {
    ...
    if (['mcat-science-passages', 'mcat-cars'].includes(file))
      d.forEach(p => p.questions.forEach(q => add(file, q, p.id, p)));
    if (file === 'mcat-course') d.units.forEach(u => u.questions.forEach(q => add(file, q, u.id)));
    if (file === 'mcat-cars-path')
      d.coaches
        .filter(c => c.passage)
        .forEach(c => c.passage.questions.forEach(q => add(file, q, c.passage.id, c.passage)));
    if (file === 'mcat-math-transfer')
      d.items.forEach(q =>
        add(file, { ...q, options: q.setups }, '', { numericTarget: q.value, table: q.table, graph: q.graph })
      );
  }
  return map;
}
(async () => {
  ...
  const current = inventory(file => JSON.parse(fs.readFileSync('data/' + file + '.json'))),
    entries = [];
  ...
  const report = {
    scope: 'Authored content history only. No learner data, item calibration or independent reviewer signoff.',
    baselineCommit: BASE,
