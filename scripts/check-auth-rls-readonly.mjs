...
if (tokens.some(t => !t))
  throw Error(
    'Set CORTEX_TEST_ACCOUNT_A_TOKEN and CORTEX_TEST_ACCOUNT_B_TOKEN locally using two dedicated test accounts. Do not paste tokens into reports.'
  );
async function get(path, token) {
  const response = await fetch(endpoint + path, {
    headers: { apikey, ...(token ? { Authorization: 'Bearer ' + token } : {}) },
  });
  if (!response.ok) throw Error(`Read-only check returned HTTP ${response.status}`);
  return response.json();
}
try {
  const users = await Promise.all(tokens.map(t => get('/auth/v1/user', t)));
  assert.ok(users.every(u => typeof u.id === 'string'));
  assert.notEqual(users[0].id, users[1].id, 'Use two different test accounts');
  for (let i = 0; i < 2; i++) {
    const own = await get('/rest/v1/progress?select=user_id&user_id=eq.' + encodeURIComponent(users[i].id), tokens[i]);
    assert.equal(
      own.length,
      1,
      'Each dedicated test account needs an existing progress row before this check is conclusive'
    );
    const other = await get(
      '/rest/v1/progress?select=user_id&user_id=eq.' + encodeURIComponent(users[1 - i].id),
      tokens[i]
    );
