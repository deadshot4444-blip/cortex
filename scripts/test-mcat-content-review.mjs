...
try {
  for (const width of [1280, 390, 320]) {
    ...
    await open('review');
    const assertLearnerCopy = async () =>
      assert.doesNotMatch(
        await page.locator('main').innerText(),
        /AI content review|human review|subject-matter review pending|learner pilot|local reviewer attestations/i
      );
    ...
    assert.deepEqual(
      await page.evaluate(() => {
        const s = v2PilotSnapshot(v2Pilot.active.startedAt);
        return [s.lessons.length, s.coach.length, s.math.length, s.laterAnswers.length];
      }),
      [0, 0, 0, 0]
    );
    ...
    await page.evaluate(() => {
      ...
      data.sessions.push({
        ...
        startedAt: 1,
        finishedAt: 2,
        note: 'OLD PILOT FIXTURE',
        consent: 'Voluntary local feedback',
      });
    ...
    await open('course');
    await page.locator('[data-course-open="protein-structure"]').first().click();
    await page.waitForSelector('.course-notebook');
    await assertLearnerCopy();
    await open('coach');
    ...
    assert.ok(
      await page.evaluate(() =>
        simPool('bioBiochem')
          .filter(i => i.passageId === 'bb1')
          .every(i => i.contentNote?.includes('illustrative data'))
      )
    );
    assert.deepEqual(errors, []);
    await context.close();
    console.log(
      `PASS ${width}px learner copy, reflection opt-in/save/reload/export, legacy records, zero fabricated observations, and passage disclosures`
    );
  }
} finally {
  await browser.close();
