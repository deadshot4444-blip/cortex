...
const source = fs
  .readFileSync('academy-lessons.js', 'utf8')
  .replace(
    'window.AcademyLessons = Object.freeze',
...
function harness(saved = new Map(), data = original) {
  ...
  function node(html = '') {
    return {
      ...
      querySelectorAll(selector) {
        if (selector === '[data-academy-view]')
          return [...this.html.matchAll(/data-academy-view="([a-z0-9-]+)"/g)].map(match => {
            const button = { dataset: { academyView: match[1] }, focus() {} };
            nodes.set('view-' + match[1], button);
            return button;
          });
        if (selector !== '[data-answer]') return [];
        return [...this.html.matchAll(/data-answer="(\d+)"/g)].map(match => {
          const button = { dataset: { answer: match[1] } };
          nodes.set('answer-' + match[1], button);
          return button;
        });
      },
    };
  ...
  const context = vm.createContext({
    ...
    sectionUrl: () => (section === 'reference' ? '/medicine' : '/' + section),
    history: {
      pushState(_, __, url) {
    ...
    el: html => {
      ...
      if (html.includes('<main')) lastMain = result;
      return result;
    },
    topbar: () => node(),
    setView() {
  ...
  return {
    api: context.window.AcademyLessons,
    internal: context.window.lessonTest,
    saved,
    nodes,
    win,
    get invalid() {
...
(async () => {
  ...
  assert.equal(
    original.lessons.flatMap(lesson => lesson.steps.filter(step => step.kind === 'check')).length,
    section === 'anatomy' ? 30 : 60
  );
  if (original.groups) {
  ...
  for (const lesson of original.lessons) {
    ...
    for (const [i, step] of lesson.steps.entries()) {
      ...
      if (step.table) {
        assert.ok(h.html.includes(step.table.caption));
        assert.match(h.html, /scope="col"/);
        assert.match(h.html, /scope="row"/);
      }
      if (step.kind === 'check') {
        const first = checks++ === 0,
          chosen = first ? (step.answer + 1) % step.options.length : step.answer;
        h.win.scrollY = 480;
        h.nodes.get('answer-' + chosen).onclick();
        ...
        const input = h.nodes.get('#academy-response');
        input.value = 'Original explanation with a specific relationship.';
        input.oninput();
        h.nodes.get('#academy-next').onclick();
        assert.equal(record.completedAt, undefined, 'Comparison is required before completion');
        h.win.scrollY = 320;
        h.nodes.get('#academy-reveal').onclick();
        ...
        if (step.required) {
          ...
          h.nodes.get('#academy-next').onclick();
          assert.equal(record.completedAt, undefined, 'Required comparison cannot be skipped');
          const comparison = h.nodes.get('#academy-comparison');
          comparison.value = 'My timing agrees; clinical cause and outcome remain uncertain.';
          comparison.oninput();
        }
      } else if (step.views) {
        h.win.scrollY = 200;
        h.nodes.get('view-' + step.views[1].id).onclick();
        assert.equal(record.steps[step.id].view, step.views[1].id);
        assert.equal(h.win.scrollY, 200);
        assert.equal(h.focused, `[data-academy-view="${step.views[1].id}"]`);
        assert.ok(h.html.includes(step.views[1].description));
        h.fail = true;
        h.nodes.get('view-' + step.views[0].id).onclick();
        assert.equal(JSON.parse(h.saved.get(course.key)).lessons[lesson.id].steps[step.id].view, step.views[1].id);
        h.fail = false;
        h.api.open(section, lesson.id, i);
  ...
  if (requiredLesson) {
    const fresh = harness();
    await fresh.api.load(section, 'test');
    fresh.api.open(section, requiredLesson.id);
    ...
    for (const [index, step] of requiredLesson.steps.entries()) {
      ...
      if (step.required) {
        ...
        comparison.oninput();
        assert.equal(
          fresh.nodes.get('#academy-next').disabled,
          true,
          'A failed comparison save cannot enable completion'
        );
        ...
        fresh.fail = false;
        comparison.oninput();
        fresh.nodes.get('#academy-next').onclick();
        assert.ok(record.completedAt);
        const broken = structuredClone(record);
        delete broken.steps[step.id].firstDraft;
        assert.equal(
          fresh.internal.validRecord(broken),
          false,
          'A required pre-feedback explanation cannot disappear from a completed record'
        );
      } else if (index + 1 < requiredLesson.steps.length) fresh.nodes.get('#academy-next').onclick();
    }
    const updated = structuredClone(original);
    const changedStep = updated.lessons.find(lesson => lesson.id === requiredLesson.id).steps.find(step => step.trace);
    if (changedStep) {
      changedStep.trace.values[100] = 0.9;
      const reload = harness(fresh.saved, updated);
      await reload.api.load(section, 'test');
      reload.api.open(section, requiredLesson.id, 0);
      assert.deepEqual(
        plainTrace(
          reload.internal.courses
            .get(section)
            .progress.lessons[requiredLesson.id].content.steps.find(step => step.trace).trace
        ),
        plainTrace(record.content.steps.find(step => step.trace).trace)
      );
    }
  }
  ...
  const reload = harness(h.saved, changed);
  await reload.api.load(section, 'test');
  ...
  await partial.api.load(section, 'test');
  partial.api.open(section, first.id, 0);
  partial.nodes.get('#academy-next').onclick();
  ...
  const revised = bump(original, 2, lesson => {
    lesson.title = 'Revised title';
    lesson.steps[0].paragraphs[0] = 'Revised paragraph';
    lesson.steps[1].options.reverse();
    lesson.steps[1].answer = 0;
    lesson.steps[1].explanation = 'Revised explanation';
  });
  const refreshed = harness(partial.saved, revised);
  await refreshed.api.load(section, 'test');
  refreshed.api.open(section, first.id, 1);
  const refreshedRecord = refreshed.internal.courses.get(section).progress.lessons[first.id];
  assert.deepEqual(
    JSON.parse(JSON.stringify(refreshedRecord)),
    JSON.parse(JSON.stringify(before)),
    'An unfinished attempt keeps its wording, answer meaning and timestamps'
  );
  assert.match(refreshed.html, /Revised title/);
  assert.match(refreshed.html, /Revised explanation/);
  ...
  const reshaped = bump(revised, 3, lesson => {
    lesson.steps.splice(1, 1);
    lesson.steps.push({ id: 'inserted-step', kind: 'read', paragraphs: ['Inserted step'] });
  });
  const rebuilt = harness(refreshed.saved, reshaped);
  await rebuilt.api.load(section, 'test');
  rebuilt.api.open(section, first.id, 1);
  const rebuiltRecord = rebuilt.internal.courses.get(section).progress.lessons[first.id];
  assert.deepEqual(
    JSON.parse(JSON.stringify(rebuiltRecord)),
    JSON.parse(JSON.stringify(before)),
    'Removing an authored step does not erase its saved answer'
  );
  assert.match(rebuilt.html, /Inserted step/);
  assert.match(rebuilt.html, /Your first answer/);
  assert.ok(rebuilt.internal.validRecord(rebuiltRecord));
  const snapshot = harness(h.saved, revised);
  await snapshot.api.load(section, 'test');
  snapshot.api.open(section, first.id, 1);
  ...
  assert.ok(snapshot.html.includes(first.steps[1].explanation));
  assert.match(snapshot.html, /revised since you started/);
  assert.match(snapshot.html, /Revised explanation/);
  assert.doesNotMatch(snapshot.html, /<h1>Revised title/);
  snapshot.api.open(section, first.id, 0);
  assert.match(snapshot.html, /Revised paragraph/, 'Current teaching is accessible from any saved step');
  const damaged = structuredClone(existing);
  damaged.steps[damaged.content.steps[1].id].selected = 99;
  ...
  await bad.api.load(section, 'test');
  assert.equal(bad.invalid, 1);
  assert.equal(bad.writes, 0);
  const regional = original.lessons.find(lesson => lesson.steps[0].views);
  if (regional) {
    const badLesson = structuredClone(regional);
    badLesson.steps[0].views[0].rows[0].relation = '';
    assert.equal(h.internal.validLesson(badLesson), false);
    const badView = structuredClone(course.progress.lessons[regional.id]);
    badView.steps[regional.steps[0].id].view = 'missing';
    assert.equal(h.internal.validRecord(badView), false);
    const update = structuredClone(original);
    update.lessons.find(lesson => lesson.id === regional.id).steps[0].views[0].title = 'Revised after this session';
    const viewReload = harness(h.saved, update);
    await viewReload.api.load(section, 'test');
    viewReload.api.open(section, regional.id, 0);
  ...
  if (comparison) {
    ...
    const update = structuredClone(original);
    update.lessons.find(lesson => lesson.id === comparison.id).steps[stepIndex].table.rows[0][1] =
      'Changed after completion';
    const tableReload = harness(h.saved, update);
    await tableReload.api.load(section, 'test');
    tableReload.api.open(section, comparison.id, stepIndex);
    assert.doesNotMatch(tableReload.html, /Changed after completion/);
    const damagedTable = structuredClone(comparison);
    damagedTable.steps[stepIndex].table.rows[0].pop();
    ...
    tableReload.api.open(section, comparison.id, comparison.steps.length - 1);
    tableReload.nodes.get('#academy-next').onclick();
    tableReload.nodes.get('#academy-next-lesson').onclick();
    ...
    for (const lesson of original.lessons.filter(lesson => lesson.group !== 'physiology')) {
      ...
      assert.equal(lesson.review.reviewedAt, null);
      assert.equal(lesson.steps.filter(step => step.kind === 'check').length, 3);
      assert.ok(lesson.sourceCheckedOn);
    }
  }
  console.log(
    `${original.lessons.length} authored lessons: required-step gates, immutable first answers, frozen completed content, revision disclosure, in-place feedback focus, written work, idempotent completion, saved relationship views, reload compatibility and corrupt-record protection passed.`
  );
})().catch(error => {
  console.error(error);
  process.exitCode = 1;
