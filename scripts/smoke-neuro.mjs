...
await page.addInitScript(() => localStorage.setItem('cs-seen-ver', '1.10.0'));
...
for (let i = 0; i < 40; i++) {
  let acted = false;
  const more = page.locator('#neunitstages details.neuro-sandbox-more').last();
  if (await more.count()) await more.evaluate(el => { el.open = true; });
  for (const sel of ['[data-reveal]', '.opt:not([disabled])', '[data-check-code]', '[data-code-done]', '#nesimdone', '[data-cont]:not([disabled])']) {
