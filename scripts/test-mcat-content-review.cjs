...
const course = read('mcat-course.json'),
  v2 = read('mcat-v2.json');
...
test('ANOVA reported in passage and question agrees with its four summary groups', () => {
  const p = passage('ps1'),
    rows = p.table.rows,
    n = 20;
  const means = rows.map(r => Number(r[2])),
    sd = rows.map(r => Number(r[3]));
  ...
  for (const text of [p.text, p.questions.find(q => q.id === 'ps1-4').stem]) {
    ...
    assert.ok(match);
    near(Number(match[1]), f, 0.05);
  }
  near((100 * means[3]) / 60, 45.6666666667, 1e-7);
  assert.ok(
    p.questions.every(q => !/option\s+[A-D]\b/i.test(q.explanation)),
    'Avoid stale option-letter explanations'
  );
});
...
test('signal-detection indices reproduce the supplied rates under the stated normal model', () => {
  ...
  function normalCDF(z) {
    const n = 256,
      h = Math.abs(z) / n,
      density = x => Math.exp((-x * x) / 2) / Math.sqrt(2 * Math.PI);
    let sum = density(0) + density(Math.abs(z));
    for (let i = 1; i < n; i++) sum += (i % 2 ? 4 : 2) * density(i * h);
    return 0.5 + (Math.sign(z) * h * sum) / 3;
  }
  const p = passage('ps2');
  for (const row of p.table.rows) {
    const hit = Number(row[2]),
      falseAlarm = Number(row[3]),
      d = Number(row[4]),
      c = Number(row[6]);
    near(normalCDF(d / 2 - c), hit, 0.004);
    near(normalCDF(-d / 2 - c), falseAlarm, 0.004);
...
test('the HRT week-eight ANOVA agrees with its displayed group means and common within-group SD', () => {
  const p = passage('ps8'),
    rows = p.table.rows,
    n = 32;
  const means = rows.slice(0, 3).map(row => Number(row[2])),
    grand = means.reduce((a, b) => a + b) / 3;
  const sd = Number(rows[3][2]),
    f = (n * means.reduce((sum, x) => sum + (x - grand) ** 2, 0)) / 2 / (sd * sd);
  near(Number(p.text.match(/F\(2, 93\) = ([\d.]+)/)[1]), f, 0.05);
  const baseline = Number(rows[0][1]),
    after = Number(rows[0][2]);
  const q = p.questions.find(q => q.id === 'ps8-4');
  near(Number(q.options[q.answer].replace('%', '')), (100 * (baseline - after)) / baseline, 0.5);
});
...
test('Titration table agrees with the stated weak-acid equilibrium and stoichiometry', () => {
  ...
  for (const [volumeText, pHText] of p.table.rows) {
    const volume = Number(volumeText) / 1000,
      baseMoles = volume * 0.1,
      totalVolume = 0.025 + volume;
    let pH;
    if (!volume) {
      const h = (-ka + Math.sqrt(ka * ka + 4 * ka * 0.1)) / 2;
      pH = -Math.log10(h);
    } else if (Math.abs(baseMoles - acidMoles) < 1e-10) {
      const kb = kw / ka,
        concentration = acidMoles / totalVolume;
...
test('Enzyme table fits a common Vmax and different apparent Km values', () => {
  for (const row of passage('bb1').table.rows) {
    ...
    near(control, (100 * s) / (2 + s), 0.051);
    near(inhibited, (100 * s) / (6 + s), 0.051);
  }
  const p = passage('bb2'),
    mock = p.table.rows.find(r => r[0] === 'Mock'),
    knockdown = p.table.rows.find(r => r[0] === 'si-HSF1');
  near(Number(knockdown[4]) / Number(mock[4]), 0.257142857, 1e-8);
...
test('ALP rate units and volume keep the stated early-window consumption below five percent', () => {
  ...
  for (const row of p.table.rows) {
    ...
    for (const rate of row.slice(1).map(Number)) assert.ok((rate * seconds) / 60 / initialNmol < 0.05);
  }
...
test('regional HWE arithmetic has a heterozygote deficit when pooled, not a homozygote deficit', () => {
  ...
  rows.forEach(row => {
    ...
    near(q * q, Number(row[2]));
    near(2 * q * (1 - q), Number(row[4]));
  ...
  near(pooledQ, 0.25);
  near(regionalHeterozygotes, 0.35);
  near(2 * pooledQ * (1 - pooledQ), 0.375);
  assert.ok(regionalHeterozygotes < 2 * pooledQ * (1 - pooledQ));
  const q = p.questions.find(q => q.id === 'bb5-1');
  near(Number(q.options[q.answer].match(/[\d,]+/)[0].replace(/,/g, '')), 2000 * Number(rows[3][4]));
...
test('hemodynamic rows and local linkage intervals agree with their independent arithmetic', () => {
  passage('bb11').table.rows.forEach(row => {
    const [, hr, sv, co, map, cvp, svr] = row.map(Number);
    near((hr * sv) / 1000, co, 0.005);
    near((map - cvp) / co, svr, 0.051);
  });
  const rows = passage('bb12').table.rows,
    n = rows.reduce((sum, row) => sum + Number(row[1]), 0);
  const doubles = rows.filter(row => row[2] === 'Double crossover').reduce((sum, row) => sum + Number(row[1]), 0);
  const interval = label =>
    (doubles + rows.filter(row => row[2].includes(label)).reduce((sum, row) => sum + Number(row[1]), 0)) / n;
  near(interval('nt–pf'), 0.11);
  near(interval('sg–nt'), 0.1);
  const expected = interval('nt–pf') * interval('sg–nt') * n;
  near(expected, 11);
  near(doubles / expected, 10 / 11);
  near(1 - doubles / expected, 1 / 11);
...
test('Doubling absorbance squares transmission; the former second correct choice is removed', () => {
  ...
  for (const absorbance of [0.2, 0.7, 1.3]) near(10 ** (-2 * absorbance), (10 ** -absorbance) ** 2);
...
test('All 48 math values agree with independently parsed prompt inputs', () => {
  for (const skill of Object.keys(core.SKILLS))
    for (let i = 0; i < 8; i++) {
      ...
      let expected;
      if (skill === 'units') {
        const m = q.stem.match(/(\d+) mg of solute in ([\d.]+) L/);
        expected = Number(m[1]) / 1000 / Number(m[2]);
      }
      if (skill === 'notation') {
        const m = q.stem.match(/\((\d+) × 10\^−(\d+)\) × \((\d+) × 10²\)/);
        expected = (Number(m[1]) / 10 ** Number(m[2])) * Number(m[3]) * 100;
      }
      if (skill === 'logs') {
        const m = q.stem.match(/= (\d+) × 10\^−(\d+) mol/);
        expected = -Math.log10(Number(m[1]) / 10 ** Number(m[2]));
      }
      if (skill === 'ratios') {
        const m = q.stem.match(/resistance (\d+) units.*increases (\d+)-fold/);
        expected = Number(m[1]) * (1 / Number(m[2])) ** 4;
      }
      if (skill === 'slopes') {
        const [[x0, y0], [x1, y1]] = q.graph.points;
        expected = (y1 - y0) / (x1 - x0);
      }
      if (skill === 'estimation') {
        const m = q.stem.match(/a (\d+) kg object at ([\d.]+) m\/s²/);
...
test('Review metadata preserves pending human validation and all four science disclosures', () => {
  assert.match(v2.aiReview.reviewer, /AI/);
  assert.match(v2.aiReview.status, /independent human review remains pending/);
  for (const id of ['cp2', 'bb1', 'bb2', 'ps1']) {
    const p = passage(id);
    assert.match(p.contentNote, /illustrative data/);
    assert.doesNotMatch(p.text, /Figure 1/);
  }
