...
const require=createRequire(import.meta.url), core=require('../mcat-repair-engine.js');
...
try {
 for(const width of [1280,390,320]) {
  ...
  assert.equal((await page.locator('[data-go="socrates"] .nav-availability').innerText()).toLowerCase(),'coming soon');
  assert.equal((await page.locator('[data-go="neuro"] .nav-availability').innerText()).toLowerCase(),'in review');
  await overflow('homepage');
  await page.click('#m-quick'); await page.waitForSelector('[data-repair-answer]');
  ...
  const oldNow=Date.now()-2*86400000,s=core.empty();core.begin(c,s,'repair',oldNow);core.answer(c,s,c.diagnostic.answer,'sure',oldNow);s.active.phase='lesson';core.afterLesson(c,s);core.answer(c,s,c.checks[0].answer,'sure',oldNow+1);s.active=null;
  await page.evaluate(s=>localStorage.setItem('cs-mcat-repairs-v1',JSON.stringify(s)),s);
  await page.reload({waitUntil:'networkidle'});await page.getByText('Concept repair & learning evidence',{exact:true}).click();await page.click('[data-repair-start]');
  ...
  await page.click(`[data-repair-answer="${c.checks[1].answer}"]`);await page.click('#repair-next');
  ...
  await page.evaluate(()=>localStorage.setItem('cs-mcat-log',JSON.stringify([{qId:'cp-chem-1',category:'5A',section:'chemPhys',ts:Date.now(),correct:false,conf:'sure'}])));
  await page.reload({waitUntil:'networkidle'});await page.click('#back');await page.click('.mcat-simple-fold summary');await page.click('[data-mcat-tool="5"]');
  assert.equal(await page.locator('.repair-priority h2').innerText(),'Buffers after adding strong acid');
  await page.click('[data-repair-start]');
  const buffer=data.concepts[1];await page.click(`[data-repair-answer="${buffer.diagnostic.answer}"]`);await page.click('#repair-next');await page.click('#repair-next');await page.click(`[data-repair-answer="${(buffer.checks[0].answer+1)%4}"]`);await page.click('#repair-next');
  assert.equal(await page.locator('.repair-lesson').count(),1);await page.click('#repair-next');assert.equal(await page.locator('.repair-result h1').innerText(),'Needs practice');
  assert.deepEqual(errors,[]);
  await ctx.close();console.log(`PASS ${width}px: quick entry, save/resume, distinct questions, later checks, plan preservation, missed-question priority, no overflow/errors`);
 }
