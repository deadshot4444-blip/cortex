...
const transfer = read('mcat-math-transfer'),
  passages = read('mcat-science-passages');
const course = read('mcat-course'),
  models = read('mcat-experiments');
assert.equal(core.validMathTransfer(transfer), true);
for (const damage of [
  d => (d.items[0] = null),
  d => (d.items[0].answer = 3),
  d => (d.items[0].value = Infinity),
  d => (d.items[0].id = d.items[1].id),
  d =>
    (d.items[0].graph = {
      points: [
        [0, 'bad'],
  ...
  const bad = structuredClone(transfer);
  damage(bad);
  assert.equal(core.validMathTransfer(bad), false);
}
const expected = [
  360000 / 60 / (2 / 10000),
  0.12 / 0.00025 / 120,
...
transfer.items.forEach((q, i) => {
  ...
  assert.ok(
    q.unitIds.every(id => lessons.has(id)),
    q.id
  );
...
for (const skill of Object.keys(core.SKILLS)) {
  ...
  state.math.history = [
    { qId: `math-${skill}-0`, completedAt: 1 },
    { qId: `math-${skill}-0`, completedAt: 2 },
    { qId: `math-transfer-${skill}-1`, completedAt: 3 },
    { qId: `math-${skill}-1` },
  ];
  assert.equal(
    core.mathTransferAllowed(state, skill),
    false,
    'Repeats, transfer runs and unfinished variants do not unlock'
  );
...
const context = vm.createContext({
  ...
  courseUnit: id => course.units.find(unit => unit.id === id),
  el: html => {
    lastHtml = html;
...
for (const skill of Object.keys(core.SKILLS)) {
  ...
  for (let i = 0; i < 2; i++) {
    ...
    submit({ preventDefault() {} });
    assert.equal(
      run('v2State.math.history.length'),
      before + 1,
      'Double submission must preserve one first calculation'
    );
...
for (const p of science) {
  ...
  assert.ok(
    ['hypothesis', 'independent', 'dependent', 'control', 'conclusion', 'limitation'].every(field => model[field])
  );
  for (const row of p.table.rows)
    for (const column of [model.graph.xColumn, ...model.graph.series.map(s => s.column)]) {
...
assert.equal(-Number(optics.table.rows[0][1]) / Number(optics.table.rows[0][0]), -0.5);
const delta = assay.table.rows.map(row => Number(row[2]) - Number(row[1]) - (Number(row[4]) - Number(row[3])));
assert.ok(Math.abs(delta[1] - 0.22) < 1e-10);
assert.ok(Math.abs((delta[1] / 10000 / 60) * 1e6 - 0.3666666666667) < 1e-10);
...
vm.runInContext(
  'experimentNotes=JSON.parse(JSON.stringify(models));var session={p:{id:"cp-transfer-decay"}};experimentModel(session);experimentNotes[session.p.id].conclusion="Changed later";',
  experimentContext
);
assert.equal(vm.runInContext('experimentModel(session).conclusion', experimentContext), models[decay.id].conclusion);
console.log(
  'MCAT transfer: 12 independent numerical checks, 12 persisted handler journeys, access/exposure/failure guards, three experiment models and frozen notes passed. Browser verification remains separate.'
);
