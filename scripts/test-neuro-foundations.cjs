...
function harness(saved = new Map(), content = data) {
  let invalid = 0, fail = false, blocked = '';
  const requests = [];
  const storage = { paused: false, read: (key, fallback) => saved.has(key) ? JSON.parse(saved.get(key)) : fallback,
    ...
    write(key, value) { if (fail) return false; saved.set(key, JSON.stringify(value)); return true; } };
  const context = vm.createContext({ console, Date, Math, JSON, URL, URLSearchParams, Set, Map,
    StudyStorage: storage, window: { addEventListener() {} },
    runPythonCode: async code => python(code),
    fetch: async file => { requests.push(file); return { ok: !blocked || !file.includes(blocked), json: async () => file.includes('milestones') ? { milestones: [] } : structuredClone(content) }; },
  });
  vm.runInContext(evaluator, context); vm.runInContext(source, context);
  return { context, run: code => vm.runInContext(code, context), saved, storage, requests,
    get invalid() { return invalid; }, set fail(value) { fail = value; }, set blocked(value) { blocked = value; } };
}
(async () => {
  ...
  const old = { pathStarted: true, pathDone: [data.learningPaths[0].steps[19].id, data.learningPaths[0].steps[19].id, 'retired-unit'], topicQuiz: { legacy: { c: 3, t: 4 } }, topicAtlas: {}, sims: {}, code: { legacy: { passed: true, ts: 1234 } }, milestones: { legacy: { passed: true } } };
  const saved = new Map([['cs-neuro', JSON.stringify(old)]]), h = harness(saved);
  h.blocked = 'neuro.json'; await assert.rejects(h.run('loadNeuro()'), /did not download/);
  assert.equal(h.run('NEURO.loaded'), false);
  h.blocked = 'milestones'; await Promise.all([h.run('loadNeuro()'), h.run('loadNeuro()')]);
  ...
  for (const step of data.learningPaths[0].steps.slice(0, 5)) {
    ...
    const record = h.run('record'), content = record.content, lesson = content.code;
    assert.equal(content.unit.revision, step.order === 4 ? 2 : 3);
    const application = data.unitLessons[String(step.order)].simulation || data.simulations.find(sim => sim.id === step.simulationId);
    assert.deepEqual(JSON.parse(JSON.stringify(content.sim)), application, 'Snapshot uses the application matched to this foundation');
    ...
    content.unit.activeRecallPrompts.forEach((prompt, i) => record.recall[i] = { draft: 'First written explanation ' + i, revealedAt: Date.now() });
    ...
    const printOnly = await h.context.neuroCodeEvaluateOJT('print(' + JSON.stringify(lesson.expectedOutput) + ')', lesson);
    ...
    h.fail = true; assert.equal(advance('debrief'), false);
    const completedAt = record.completedAt;
    assert.ok(completedAt);
    assert.equal(JSON.parse(saved.get('cs-neuro')).units[step.id].completedAt, undefined, 'Failed persistence must not pretend to reach disk');
    h.fail = false; assert.equal(h.run('saveNeuroProg()'), true);
    assert.equal(advance('debrief'), false); assert.equal(record.completedAt, completedAt);
  ...
  const mutating = await h.context.neuroCodeEvaluateOJT('def append_sample(samples, value):\n    samples.append(value)\n    return samples', list);
  ...
  const badCount = await h.context.neuroCodeEvaluateOJT('def count_events(samples, threshold):\n    return sum(value >= threshold for value in samples)', loops);
  ...
  const equalityWrong = await h.context.neuroCodeEvaluateOJT('def at_or_above(sample, threshold):\n    return sample > threshold', condition);
  assert.equal(equalityWrong.passed, false, 'Boundary equality is tested');
  const prose = await h.context.neuroCodeEvaluateOJT('noise', { id: 'reflection', solution: 'Smoothing can hide details.', expectedOutput: 'noise' });
  assert.equal(prose.passed, false); assert.equal(prose.needsSelfReview, true);
  const changed = structuredClone(data); changed.unitLessons['1'].shortLesson = 'Replacement content';
  changed.simulations.find(sim => sim.id === 'sim-sampling-time').decisionQuestion = 'Changed application';
  const reload = harness(saved, changed); await reload.run('loadNeuro()');
  ...
  assert.notEqual(reload.run('NEURO_PROG.units[firstId].content.sim.decisionQuestion'), 'Changed application', 'Saved applications remain frozen when the catalog changes');
  ...
  const corrupted = new Map([['cs-neuro', JSON.stringify(damaged)]]), bad = harness(corrupted);
  assert.equal(bad.invalid, 1); assert.equal(corrupted.get('cs-neuro'), JSON.stringify(damaged));
  console.log(`Neuro foundations: 5 complete state journeys, ${cases} Python reference cases, wrong-code counterexamples, first-answer retention, immutable content, legacy progress, loading retry and damaged-record protection passed. Browser walkthroughs remain separate.`);
