...
const START = new Date(2026, 8, 7, 10).getTime(), DAY = 86400000;

function harness({ repairs = true, seed = new Map(), realHub = false } = {}) {
  const dom = new JSDOM('<!doctype html><html><body><div id="app"></div></body></html>', { url: 'http://localhost/mcat?view=today', runScripts: 'outside-only', pretendToBeVisual: true });
  const w = dom.window, context = dom.getInternalVMContext(), errors = [], calls = { guide: 0, hub: 0, session: 0 };
  let clock = START;
  ...
  w.setView = root => { w.document.getElementById('app').replaceChildren(root); };
  w.sectionUrl = () => '/mcat';
  ...
  for (const name of ['study-storage.js', 'mcat-course-engine.js', 'mcat-v2-engine.js', 'mcat-repair-engine.js', 'mcat.js', 'mcat-repair.js', 'mcat-course.js', 'mcat-workflows.js']) run(fs.readFileSync(name, 'utf8'));
  run(`courseData=${courseJson}; var v2Data=null; renderMCATEntry=()=>{}; renderAcademyStats=()=>{}; loadMCAT=async()=>{};`);
  if (repairs) run(`repairData=${repairsJson}; repairState=McatRepairCore.normalize(StudyStorage.read('${REPAIR_KEY}',null),repairData.concepts);`);
  w.testCalls = calls;
  ...
  return { w, run, find, click, errors, calls, advance: ms => { clock += ms; }, raw: key => w.localStorage.getItem(key), close: () => dom.window.close() };
}
const plain = value => JSON.parse(JSON.stringify(value));
/* One completed repair session on the first concept, finished `daysAgo` days before the clock. */
const seededRecords = daysAgo => `(()=>{const c=repairData.concepts[0],at=Date.now()-${daysAgo}*${DAY},s=McatRepairCore.empty();
...
test('Saved-work section still lists an in-progress concept session when repairs are loaded', () => {
  const h = harness();
  h.run("McatRepairCore.begin(repairData.concepts[2],repairState,'repair',Date.now());saveMcatRepair();renderCourseProgress()");
  assert.equal(h.errors.length, 0, h.errors[0]?.stack);
...
test('Learning-record repair button opens the hub and leaves saved records untouched when lessons are missing', () => {
  const stored = JSON.stringify({ version: 1, records: { 'enzyme-inhibition': { attempts: [], reviewedAt: 5, dueAt: 0 } }, active: null });
  const h = harness({ repairs: false, seed: new Map([[REPAIR_KEY, stored]]) });
  h.run("var body=el('<section></section>');document.getElementById('app').appendChild(body);var unit=courseUnit('enzyme-rates');courseLearningRecord(body,unit,courseRecord(unit.id));");
...
test('A stale due-check task whose check was already taken today is marked complete instead of throwing', () => {
  ...
  const task = plain(h.run("var plan=buildFlexiblePlan(30);var task=studyDailySession(plan).tasks.find(t=>t.type==='repair');task"));
  ...
  h.run("(()=>{const c=repairConcept(task.conceptId);McatRepairCore.begin(c,repairState,'later',Date.now());McatRepairCore.answer(c,repairState,c.checks[1].answer,'sure',Date.now()+5);repairState.active=null;saveMcatRepair();})()");
...
test('A repair task launched while lessons are unavailable opens the hub without throwing', () => {
  const h = harness({ repairs: false });
  h.run("studyLaunchTask({id:'flex-x-repair',day:1,type:'repair',conceptId:'enzyme-inhibition',repairKind:'later',minutes:5},{sessions:{},completed:{}})");
  assert.equal(h.errors.length, 0, h.errors[0]?.stack);
  assert.equal(h.calls.hub, 1);
  h.close();
});
...
test('An early repair visit practices a seen question and the result screen keeps its reservation promise', () => {
  ...
  h.run("const c=repairData.concepts[0];McatRepairCore.begin(c,repairState,'repair',Date.now());McatRepairCore.answer(c,repairState,c.diagnostic.answer,'sure',Date.now());repairState.active.phase='lesson';McatRepairCore.afterLesson(c,repairState,Date.now());");
  assert.equal(h.run('repairState.active.questionId'), h.run('repairData.concepts[0].checks[0].id'), 'the seen check is replayed');
  h.run("McatRepairCore.answer(repairData.concepts[0],repairState,repairData.concepts[0].checks[0].answer,'sure',Date.now())");
  assert.equal(h.run('repairState.active.result.mode'), 'practice');
  assert.equal(h.run('repairState.records["enzyme-inhibition"].dueAt'), before.dueAt, 'the scheduled check keeps its date');
  assert.equal(h.run('McatRepairCore.unseenChecks(repairData.concepts[0],repairState).length'), 2, 'unseen checks stay reserved');
  h.close();
});
