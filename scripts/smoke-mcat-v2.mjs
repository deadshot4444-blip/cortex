...
for(const width of [1280,390,320]){
 ...
 for(const view of ['today','course','practice','coach','math','weekly','diagnose','review','progress']){
  await open(view);assert.equal(await page.locator('main').count(),1,view);assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth-innerWidth)<=1,`${width} ${view} overflow`);
  if(width!==320)await page.screenshot({path:`output/playwright/mcat-v2-${view}-${width}.png`,fullPage:true});
 }
 assert.deepEqual(errors,[]);console.log(`${width}: all v2 views render without errors or horizontal overflow`);await ctx.close();
}
