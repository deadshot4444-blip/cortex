  ...
  async open(name) {
    ...
    return {
      ...
      async put(key, response) {
        if (store.fail?.(name, key)) throw Error('Quota exceeded');
...
async function fixture() {
  const files = {
    '/index.html': '<!doctype html><title>Saved build</title>',
    '/app.js': '/* saved app */',
    '/data/lesson.json': '{"title":"saved lesson"}',
  };
  const entries = await Promise.all(
    Object.entries(files).map(async ([url, text]) => ({
      url,
      bytes: Buffer.byteLength(text),
      sha256: await Core.hash(Buffer.from(text), crypto),
    }))
  );
  const manifest = {
    format: 1,
    build: 'a'.repeat(20),
    packs: [{ id: 'mcat', title: 'MCAT', scope: 'Fixture', entry: '/mcat', routes: ['/mcat'], files: entries }],
  };
  const caches = new CacheStorage(),
    fetch = async url => new Response(files[url] || '', { status: files[url] ? 200 : 404 });
  const options = { caches, fetch, crypto, nonce: 'first' };
  return { files, manifest, caches, options };
}
function worker(f) {
  const handlers = {},
    clients = new Map();
  let networkFails = false,
    calls = 0;
  const scope = {
    location: { origin: 'https://cortex.test' },
    CortexOfflineCore: {
      ...Core,
      info: name => Core.info(name, f.caches),
      list: () => Core.list(f.options),
      verify: item => Core.verify(item, f.options),
      remove: (name, options) => Core.remove(name, { ...options, caches: f.caches }),
    },
  ...
  const context = vm.createContext({
    ...
    fetch: async request => {
      ...
      return new Response('online ' + (request.url || request));
    },
  });
  ...
  return {
    ...
    async request(path, mode = 'cors', clientId = '', method = 'GET') {
      let promise;
      handlers.fetch({
        request: { url: new URL(path, 'https://cortex.test').href, mode, method },
        clientId,
        respondWith(value) {
          promise = value;
        },
...
(async () => {
  await test('A complete download verifies every byte and becomes available only after completion', async () => {
    const f = await fixture(),
      progress = [];
    const name = await Core.download(f.manifest, 'mcat', {
      ...f.options,
      async onProgress(done, total) {
        progress.push([done, total]);
      },
    });
    const items = await Core.list({ ...f.options, verify: true });
  ...
  await test('Changed bytes, missing files and quota failure discard only the incomplete replacement', async () => {
    for (const failure of ['changed', 'missing', 'quota']) {
      const f = await fixture(),
        old = await Core.download(f.manifest, 'mcat', f.options);
      ...
      await assert.rejects(Core.download(f.manifest, 'mcat', options));
      assert.deepEqual(await f.caches.keys(), [old]);
      assert.equal(await Core.verify(await Core.info(old, f.caches), f.options), true);
    }
  });
  await test('Stopping a replacement or interrupted marker write retains the prior complete course', async () => {
    ...
    await assert.rejects(Core.download(f.manifest, 'mcat', { ...f.options, nonce: 'marker' }));
    assert.deepEqual(await f.caches.keys(), [old]);
  });
  await test('Incomplete orphan caches never appear ready and later file eviction is detected', async () => {
    const f = await fixture(),
      name = await Core.download(f.manifest, 'mcat', f.options);
    await f.caches.open(Core.PREFIX + f.manifest.build + '-mcat-orphan');
    assert.equal((await Core.list(f.options)).length, 1);
    await (await f.caches.open(name)).delete('/data/lesson.json');
    assert.equal((await Core.list({ ...f.options, verify: true }))[0].available, false);
  });
  await test('Malformed manifests, private endpoints, traversal, duplicate files and future formats are rejected', async () => {
    ...
    for (const mutate of [
      ...
      m => {
        m.packs[0].files[0].url = 'https://private.test/token';
      },
      m => {
        m.packs[0].files[0].url = '/api/session';
      },
      m => {
        m.packs[0].files[0].url = '/../private.json';
      },
      m => {
        m.packs[0].files.push(m.packs[0].files[0]);
      },
    ]) {
      const bad = structuredClone(f.manifest);
      mutate(bad);
      assert.throws(() => Core.validate(bad));
    }
  ...
  await test('Offline navigation redirects to a pinned complete version and remains pinned after reconnect', async () => {
    const f = await fixture(),
      ...
      w = worker(f);
    w.offline = true;
    const redirect = await w.request('/mcat?view=today', 'navigate');
    assert.equal(redirect.status, 307);
    const url = new URL(redirect.headers.get('location'));
    assert.equal(url.searchParams.get('offline'), name);
    assert.equal(url.searchParams.get('view'), 'today');
    assert.equal(await (await w.request(url.href, 'navigate')).text(), f.files['/index.html']);
    w.clients.set('learner', { url: url.href });
    w.offline = false;
    assert.equal(await (await w.request('/app.js?v=old', 'script', 'learner')).text(), f.files['/app.js']);
    assert.equal(
      await (await w.request('/data/lesson.json?v=old', 'cors', 'learner')).text(),
      f.files['/data/lesson.json']
    );
  ...
  await test('Normal online asset failures never combine an online page with another cached build', async () => {
    const f = await fixture();
    await Core.download(f.manifest, 'mcat', f.options);
    const w = worker(f);
    w.clients.set('online', { url: 'https://cortex.test/mcat' });
    w.offline = true;
  ...
  await test('External auth, POST submissions and non-course endpoints bypass the offline handler', async () => {
    const f = await fixture(),
      w = worker(f);
    assert.equal(await w.request('https://account.supabase.co/auth/v1/token'), null);
    assert.equal(await w.request('/', 'cors', '', 'POST'), null);
    assert.equal(await w.request('/cx-visits/incr'), null);
  ...
  await test('Missing courses and unsupported routes give recovery navigation without clearing work', async () => {
    ...
    const trailing = await w.request('/mcat/?offline=' + name, 'navigate');
    assert.equal(trailing.status, 307);
    assert.equal(new URL(trailing.headers.get('location')).pathname, '/mcat');
  });
  await test('Removing a download rejects in-use versions and unrelated browser caches', async () => {
    const f = await fixture(),
      name = await Core.download(f.manifest, 'mcat', f.options),
      w = worker(f);
    w.clients.set('learner', { url: 'https://cortex.test/mcat?offline=' + name });
    assert.match((await w.remove(name)).error, /Close tabs/);
    assert.ok(await Core.info(name, f.caches));
    w.clients.clear();
    assert.equal((await w.remove(name)).ok, true);
    assert.deepEqual(await f.caches.keys(), []);
    assert.match((await w.remove('other-app-cache')).error, /not a Cortex/);
  });
  await test('An unfinished download cannot be discarded from another tab while its owner is open', async () => {
    const f = await fixture(),
      name = Core.PREFIX + f.manifest.build + '-mcat-pending',
      cache = await f.caches.open(name),
      w = worker(f);
    ...
    assert.match((await w.remove(name)).error, /Another open tab/);
    assert.ok((await f.caches.keys()).includes(name));
    w.clients.clear();
    assert.equal((await w.remove(name)).ok, true);
    assert.deepEqual(await f.caches.keys(), []);
  });
  await test('The current six-course inventory has matching files and includes its app dependencies', async () => {
    ...
    for (const pack of manifest.packs) {
      for (const file of pack.files) {
        ...
        assert.equal(body.byteLength, file.bytes, file.url);
        assert.equal(await Core.hash(body, crypto), file.sha256, file.url);
      }
      for (const file of [
        '/index.html',
        '/app.js',
        ...
        assert.ok(
          pack.files.some(f => f.url === file),
          pack.id + ' missing ' + file
        );
    }
  });
  console.log(
    `${checks} offline cache and worker protocol checks passed. Real browser lifecycle, airplane mode and device storage are pending.`
  );
})().catch(error => {
  console.error(error);
  process.exitCode = 1;
