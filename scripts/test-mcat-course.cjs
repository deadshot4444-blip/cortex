...
assert.equal(data.units.length,36);assert.equal(data.categories.length,34);assert.equal(new Set(data.categories.map(c=>c.id)).size,34);
...
s=core.normalize({placement:{answers:{'enzyme-rates':{chosen:1,correct:false,confidence:'sure'}},background:{}}});assert.equal(core.recommendation(data,s,[],now).unit.id,'enzyme-rates');
