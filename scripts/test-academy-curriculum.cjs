...
test('index resolves all current lesson IDs and keeps drafts distinct from revised material', () => {
  ...
  assert.equal(
    data.entries.find(e => e.id === 'neuro-sim:sim-dec-overfit').status,
    'draft',
    'A citation or narrow copy correction does not promote a wider draft scenario'
  );
...
test('missing prerequisites, cycles, invalid destinations and broken connections are rejected', () => {
  for (const damage of [
    d => d.entries[0].prerequisites.push('missing'),
    d => d.entries[0].prerequisites.push(d.entries[0].id),
    d => (d.entries[0].url = '//evil.test'),
    d => (d.connections[0].to = 'missing'),
    d => d.entries[0].objectives.push('missing'),
    d => (d.cards[0].links[0].url = '/account'),
    d => d.cards.push(d.cards[0]),
  ]) {
    const d = clone(data);
    damage(d);
    assert.throws(() => Core.validate(d));
  }
});
test('search honors intersecting track, preparation, text and shared-objective filters', () => {
  ...
  const result = Core.search(data, {
    ...
    level: 'applied',
    q: 'measurement',
    objective: 'measurement-artifact',
  });
  assert.ok(result.length);
  assert.ok(
    result.every(e => e.track === 'reference' && e.level === 'applied' && e.objectives.includes('measurement-artifact'))
  );
...
test('return destinations reject redirects and preserve valid original progress routes', () => {
  for (const value of [
    ...
    assert.equal(Core.safeReturn(value), null, value);
  ...
  assert.equal(url.searchParams.get('returnTo'), start);
  assert.equal(url.searchParams.get('offline'), 'pack');
...
test('retrieval requires opt-in, preserves first writing and freezes content through a new revision', () => {
  const state = Core.emptyState(),
    card = clone(data.cards[0]);
  assert.equal(Core.enqueue(state, card, 'run-1', 100), null);
  state.enabled = true;
  const record = Core.enqueue(state, card, 'run-1', 100);
  assert.equal(Core.enqueue(state, card, 'run-2', 101), record);
  card.model = 'Changed model';
  card.links[0].title = 'Changed title';
  assert.notEqual(record.content.model, card.model);
  assert.notEqual(record.content.links[0].title, card.links[0].title);
  ...
  assert.equal(Core.complete(state, 'run-1', 105), true);
  assert.equal(Core.complete(state, 'run-1', 106), false);
  assert.equal(Core.edit(state, 'run-1', 'comparison', 'Overwrite'), false);
  assert.equal(Core.validState(clone(state)), true);
  assert.ok(Core.enqueue(state, card, 'run-2', 110));
...
test('pause, set-aside, remove and restore keep work and prevent duplicate pending attempts', () => {
  ...
  Core.edit(state, record.id, 'draft', 'Keep this writing.');
  state.enabled = false;
  assert.equal(Core.edit(state, record.id, 'draft', 'Overwrite'), false);
  assert.equal(Core.reveal(state, record.id, 101), false);
  assert.equal(record.draft, 'Keep this writing.');
  assert.equal(Core.move(state, record.id, 'deferred'), true);
  state.enabled = true;
  ...
  Core.move(state, 'run-2', 'removed');
  assert.equal(Core.move(state, record.id, 'pending'), true);
  assert.equal(record.draft, 'Keep this writing.');
  assert.ok(Core.validState(state));
});
test('damaged first explanations and completion records are not silently reset', () => {
  const state = Core.emptyState();
  state.enabled = true;
  const record = Core.enqueue(state, data.cards[0], 'run-1', 100);
  Core.edit(state, record.id, 'draft', 'First');
  Core.reveal(state, record.id, 101);
  Core.edit(state, record.id, 'comparison', 'Compared');
  Core.complete(state, record.id, 102);
  for (const damage of [
    s => delete s.records[0].firstDraft,
    s => (s.records[0].comparison = ''),
    s => (s.records[0].draft = 'altered'),
    s => (s.records[0].status = 'removed'),
    s => s.records.push(s.records[0]),
  ]) {
    const d = clone(state);
    damage(d);
    assert.equal(Core.validState(d), false);
  }
});
test('new retrieval records round-trip through portable backup without adding course credit', async () => {
  ...
  const options = { crypto },
    text = await Backup.create(workspace, '2.4.0-local.1', options);
  const restored = await Backup.parse(text, options);
  assert.deepEqual(restored.data, workspace);
...
function harness(saved = new Map(), initial = '/academy?view=queue', catalog = clone(data)) {
  ...
  const elements = [],
    listeners = new Map(),
    watchers = new Map(),
    location = new URL(initial, 'http://localhost');
  function node(html = '') {
    const children = [],
      parsed = [];
    for (const match of html.matchAll(/<(a|input|button|textarea|form|select|p)\b([^>]*)>(?:([^<]*)(?=<))?/g)) {
      ...
      const child = {
        ...
        attrs,
        dataset: {},
        value: match[1] === 'textarea' ? match[3] || '' : attrs.value || '',
      ...
      for (const [key, value] of Object.entries(attrs))
        if (key.startsWith('data-'))
          child.dataset[key.slice(5).replace(/-([a-z])/g, (_, c) => c.toUpperCase())] = value;
      parsed.push(child);
    }
    const matches = (child, selector) =>
      selector.startsWith('#')
        ? child.attrs?.id === selector.slice(1)
        : selector.startsWith('[')
          ? selector.slice(1, -1) in (child.attrs || {})
          : child.tag === selector;
    const root = {
      html,
      children,
      ...
      querySelectorAll(selector) {
        return selector
          .split(',')
          .flatMap(s => [
            ...parsed.filter(child => matches(child, s.trim())),
            ...children.flatMap(child => child.querySelectorAll(s.trim())),
          ]);
      },
      querySelector(selector) {
        if (selector === 'main' && html.startsWith('<main')) return root;
        return (
          this.querySelectorAll(selector)[0] ||
          children.map(child => child.querySelector(selector)).find(Boolean) ||
          null
        );
      },
    };
  ...
  const storage = {
    ...
    read(key, fallback) {
      reads++;
      return saved.has(key) ? JSON.parse(saved.get(key)) : fallback;
    },
    watch(key, reader) {
      watchers.set(key, reader);
    },
    sessionFailed() {
      invalid++;
      storage.paused = true;
  ...
  const tracks = Object.keys(Core.paths)
    .filter(key => key !== 'academy')
    .map(id => ({ id, name: id }));
  const context = vm.createContext({
    ...
    sectionUrl(key) {
      const u = new URL(Core.paths[key], location.origin);
      for (const k of ['gates', 'offline', 'returnTo'])
        if (location.searchParams.get(k)) u.searchParams.set(k, location.searchParams.get(k));
      return u.pathname + u.search;
    },
    el: node,
    topbar: () => node(),
    esc: value => String(value).replaceAll('&', '&amp;').replaceAll('<', '&lt;').replaceAll('"', '&quot;'),
    setView: root => (screen = root),
    openSection() {},
    fetch: async () => {
      if (fetchGate) await fetchGate;
      return { ok: fetchOK, json: async () => catalog };
    },
  });
  ...
  return {
    ...
    api,
    render: () => api.render(),
    find: s => screen.querySelector(s),
    all: s => screen.querySelectorAll(s),
    get html() {
      return screen.children.map(c => c.html).join('');
    },
    get writes() {
      return writes;
    },
...
test('discovery does not read or write private progress; loading after navigation cannot replace the new page', async () => {
  ...
  assert.match(h.html, /Connected to:/);
  assert.match(h.html, /Page 1 of/);
  const delayed = harness(new Map(), '/academy?view=curriculum');
  let release;
  delayed.fetchGate = new Promise(r => (release = r));
  const pending = delayed.render();
  delayed.location.href = 'http://localhost/mcat?view=today';
  release();
  await pending;
...
test('queue handlers opt in, save drafts, freeze first writing, require a comparison and resume it', async () => {
  ...
  await h.render();
  assert.equal(h.writes, 0);
  assert.ok(h.all('[data-connect-add]').every(b => b.disabled));
  h.find('#connect-enable').onclick();
  await tick();
  assert.equal(h.state.enabled, true);
  h.all('[data-connect-add]')[0].onclick();
  await tick();
  assert.equal(h.state.records.length, 1);
  const record = h.state.records[0];
  assert.equal(h.location.searchParams.get('run'), record.id);
  h.find('#connect-reveal').onclick();
  assert.equal(record.revealedAt, undefined);
  const draft = h.find('#connect-draft');
  draft.value = 'I will retrieve before checking.';
  draft.oninput();
  h.find('#connect-reveal').onclick();
  await tick();
  assert.equal(record.firstDraft, draft.value);
  h.find('#connect-complete').onclick();
  assert.equal(record.completedAt, undefined);
  const comparison = h.find('#connect-comparison');
  comparison.value = 'I also need to return after a delay.';
  comparison.oninput();
  h.find('#connect-complete').onclick();
  await tick();
  assert.ok(record.completedAt);
  const reload = harness(h.saved, h.location.pathname + h.location.search);
  await reload.render();
  assert.match(reload.html, /Written comparison completed/);
  assert.equal(reload.find('#connect-draft').readOnly, true);
...
test('pause actually saves and preserves records; failed writes retain pending content for recovery', async () => {
  ...
  h.find('#connect-enable').onclick();
  await tick();
  assert.equal(JSON.parse(h.saved.get(KEY)).enabled, false);
  h.find('#connect-enable').onclick();
  await tick();
  h.fail = true;
  h.all('[data-connect-add]')[0].onclick();
  await tick();
  assert.equal(h.state.records.length, 1);
  assert.equal(JSON.parse(h.saved.get(KEY)).records.length, 0);
  assert.equal(h.location.searchParams.get('run'), h.state.records[0].id);
  assert.equal(h.storage.paused, true);
  const before = h.writes;
  h.all('[data-connect-add]')[1].onclick();
  assert.equal(h.writes, before);
  h.fail = false;
  h.storage.paused = false;
  h.saved.set(KEY, JSON.stringify(h.state));
  h.listeners.get('study-storage-recovered')();
  await tick();
  assert.match(h.html, /Explain before checking/);
  h.find('#connect-draft').value = 'Newest pending explanation.';
  h.fail = true;
  h.find('#connect-draft').oninput();
  assert.equal(h.state.records[0].draft, 'Newest pending explanation.');
  assert.equal(JSON.parse(h.saved.get(KEY)).records[0].draft, '');
  h.find('#connect-reveal').onclick();
  assert.equal(h.state.records[0].revealedAt, undefined);
});
test('corrupt saved queue is retained and saving is paused; unavailable catalog can retry', async () => {
  const saved = new Map([[KEY, '{"version":1,"enabled":true,"records":[{}]}']]),
    h = harness(saved);
  await h.render();
  assert.equal(h.invalid, 1);
  assert.equal(h.writes, 0);
  assert.equal(saved.get(KEY), '{"version":1,"enabled":true,"records":[{}]}');
  const unavailable = harness();
  unavailable.fetchOK = false;
  await unavailable.render();
  assert.match(unavailable.html, /could not load/);
  assert.equal(unavailable.reads, 1);
  assert.equal(unavailable.writes, 0);
  unavailable.fetchOK = true;
  await unavailable.find('#connect-retry').onclick();
  assert.match(unavailable.html, /Optional retrieval is off/);
});
