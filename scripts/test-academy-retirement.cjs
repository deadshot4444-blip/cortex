...
test('retiring a course preserves historical plans without scheduling or advertising it', () => {
  const dom = new JSDOM('<!doctype html><div id="app"></div>', {
    url: 'https://cortex.example/academy?view=today',
  ...
  const original = {
    'cs-academy-today-v1': JSON.stringify({
      version: 1,
      budget: 45,
      priority: ['cogpsych', 'mcat'],
      paused: [],
      days: {
  ...
  const saved = new Map(Object.entries(original)),
    writes = [];
  Object.assign(w, {
...
test('public discovery and new offline packs contain no retired-course destination', () => {
  ...
  assert.equal(
    catalog.entries.some(entry => entry.track === 'cogpsych'),
    false
  );
  assert.equal(
    catalog.connections.some(link => link.from.startsWith('cogpsych:') || link.to.startsWith('cogpsych:')),
    false
  );
  assert.equal(
    catalog.cards.some(card => card.links.some(link => link.track === 'cogpsych')),
    false
  );
  assert.equal(
    manifest.packs.some(pack => pack.id === 'cogpsych'),
    false
  );
  assert.equal(
    manifest.packs.some(pack => pack.files.some(file => /cogpsych/.test(file.url))),
    false
  );
});
