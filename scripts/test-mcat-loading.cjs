...
function setup(){
  ...
  const context=vm.createContext({console,window:{addEventListener(){}},document:{addEventListener(){}},
    StudyStorage:{read:(key,fallback)=>fallback,watch(){},write(){throw Error('Unexpected progress write');}},
...
(async()=>{
  ...
  await test('An outage stays retryable and does not refetch already loaded banks',async()=>{
    ...
    assert.equal(s.run('MCAT.loaded'),true);assert.equal(s.run('MCAT.cards.length'),504);assert.deepEqual(s.calls.map(url=>url.split('?')[0]),['data/mcat-cards.json']);
  ...
  await test('Offline, malformed JSON, and malformed bank shapes do not become successful empty loads',async()=>{
    for(const fault of ['network','json','shape']){
      ...
      s.failures.clear();await s.run('loadMCAT()');assert.equal(s.run('MCAT.loaded'),true);assert.equal(s.run('MCAT.sci.length'),37);
    }
  });
  await test('Category alignment updates reports without rewriting historical answers',async()=>{
    const s=setup();await s.run('loadMCAT()');
    const result=JSON.parse(s.run(`
      QLOG = Object.freeze([
    ...
    `));
    assert.equal(result.current[0].category,'6A');assert.equal(result.saved[0].category,'7A');
    assert.equal(result.current[0].ts,123);assert.equal(result.current[0].correct,false);
