...
(() => {
    ...
    (Array.isArray(views) &&
      ...
      views.every(
        view =>
          ...
          view.rows.length > 0 &&
          view.rows.every(row => object(row) && ['from', 'relation', 'to'].every(field => text(row[field])))
      ));
  const validStep = step =>
    ...
    ((step.kind === 'read' &&
      ...
      (step.kind === 'reflect' &&
        text(step.prompt) &&
        text(step.model) &&
        (step.required == null || typeof step.required === 'boolean')));
  function validLesson(lesson) {
    return (
      ...
      Number.isInteger(lesson.revision) &&
      lesson.revision > 0 &&
      Array.isArray(lesson.steps) &&
      lesson.steps.length > 0 &&
      lesson.steps.every(validStep) &&
      ...
      (lesson.connections == null ||
        (Array.isArray(lesson.connections) &&
          lesson.connections.every(
            link =>
              object(link) &&
              ['reference', 'anatomy', 'practice'].includes(link.section) &&
              text(link.label) &&
              object(link.params) &&
              Object.entries(link.params).every(
                ([key, value]) =>
                  ['lesson', 'view', 'tool', 'mode', 'focus'].includes(key) && /^[a-z0-9-]+$/.test(value)
              ) &&
  ...
  function validRecord(record) {
    if (
      !object(record) ||
      !validLesson(record.content) ||
      !object(record.steps) ||
      !Number.isInteger(record.index) ||
      record.index < 0 ||
      record.index >= record.content.steps.length ||
    ...
    return Object.entries(record.steps).every(([id, saved]) => {
      ...
      if (
        !step ||
        !object(saved) ||
        ['draft', 'firstDraft', 'comparison'].some(key => saved[key] != null && typeof saved[key] !== 'string')
      )
        return false;
      if (step.required && saved.revealedAt && !text(saved.firstDraft)) return false;
      if (saved.view != null && (step.kind !== 'read' || !step.views?.some(view => view.id === saved.view)))
        return false;
      if (step.kind !== 'check') return saved.selected == null;
      return (
        Array.isArray(saved.order) &&
        saved.order.length === step.options.length &&
  ...
  async function load(section, file) {
    ...
    if (
      ...
      new Set(data.lessons.map(lesson => lesson.id)).size !== data.lessons.length ||
      (data.groups != null &&
        (!Array.isArray(data.groups) ||
          !data.groups.length ||
          data.groups.some(
            group => !object(group) || !text(group.id) || !text(group.title) || !text(group.description)
          ) ||
    ...
    const key = `cs-academy-${section}-v1`,
      progress = StudyStorage.read(key, { lessons: {} });
    if (
      !object(progress) ||
      !object(progress.lessons) ||
  ...
  function save(course) {
    return StudyStorage.write(course.key, course.progress);
  }
  function ready(step, saved = {}) {
    return step.kind === 'check'
      ? Number.isInteger(saved.selected)
      : step.kind === 'reflect'
        ? !!saved.revealedAt && (!step.required || (text(saved.firstDraft) && text(saved.comparison)))
        : !!saved.seenAt;
  }
  function recordFor(course, lesson) {
    return (course.progress.lessons[lesson.id] ||= {
      content: clone(lesson),
      index: 0,
      steps: {},
      startedAt: Date.now(),
    });
  ...
  function liveMarkup(lesson) {
    const body = lesson.steps
      .map(step =>
        step.kind === 'read'
          ? step.paragraphs.map(paragraph => `<p>${esc(paragraph)}</p>`).join('') +
            (step.sequence ? `<ol>${step.sequence.map(item => `<li>${esc(item)}</li>`).join('')}</ol>` : '') +
            (step.views || [])
              .map(
                view =>
                  `<h3>${esc(view.title)}</h3><p>${esc(view.description)}</p><ul>${view.rows.map(row => `<li>${esc(row.from)}: ${esc(row.relation)} ${esc(row.to)}</li>`).join('')}</ul>`
              )
              .join('') +
            tableMarkup(step.table) +
            (step.trace ? ECGTrace.markup(step.trace) : '')
          : step.kind === 'check'
            ? `<p>${esc(step.prompt)}</p><ul>${step.options.map(option => `<li>${esc(option)}</li>`).join('')}</ul><p>Answer: ${esc(step.options[step.answer])}</p><p>${esc(step.explanation)}</p>`
            : `<p>${esc(step.prompt)}</p><p>${esc(step.model)}</p>`
      )
      .join('<hr>');
  ...
  function connectionsMarkup(lesson) {
    ...
    return `<aside class="academy-connections"><h2>Connect this lesson</h2><ul>${lesson.connections
      .map(link => {
  ...
  function catalog(section) {
    ...
    const list = lessons =>
      `<ol class="academy-lesson-list">${lessons
        .map(lesson => {
  ...
  function bind(root, section, back) {
    ...
    course.back = back;
    root
      .querySelectorAll('[data-academy-lesson]')
      .forEach(button => button.addEventListener('click', () => open(section, button.dataset.academyLesson)));
  }
  ...
  function open(section, id, requested) {
    ...
    const course = courses.get(section),
      current = course?.data.lessons.find(lesson => lesson.id === id);
    if (!current) return;
    const record = recordFor(course, current),
    ...
    index = Math.max(
      0,
      Math.min(index, lesson.steps.length - 1, firstUnfinished < 0 ? lesson.steps.length - 1 : firstUnfinished)
    );
    record.index = index;
    const step = lesson.steps[index],
      saved = (record.steps[step.id] ||= {});
    ...
    const body =
      ...
      ${step.sequence ? `<ol class="academy-text-map">${step.sequence.map(item => `<li>${esc(item)}</li>`).join('')}</ol>` : ''}${viewsMarkup(step, saved)}${tableMarkup(step.table)}${step.trace ? ECGTrace.markup(step.trace) : ''}`
        : step.kind === 'check'
          ? `<h2>${esc(step.prompt)}</h2><div class="academy-check-options">${saved.order
              .map(
                (option, i) => `<button class="btn" data-answer="${option}" ${answered ? 'disabled' : ''}>
        <span aria-hidden="true">${'ABCD'[i] || i + 1}.</span> ${esc(step.options[option])}${answered && saved.selected === option ? ' · Your first answer' : ''}</button>`
              )
              .join('')}</div>
        ...
        ${
          saved.revealedAt
            ? `<div class="academy-answer" role="status" tabindex="-1"><p>${esc(step.model)}</p><p>Your writing is saved, without an automated correctness score.</p></div>
    ...
    root.appendChild(topbar(section));
    const main =
      el(`<main class="panel academy-lesson" id="main"><button class="ghostbtn" id="academy-back">← Course home</button>
    ...
    main.querySelectorAll('[data-academy-view]').forEach(
      button =>
        (button.onclick = () => {
    ...
    main.querySelectorAll('[data-answer]').forEach(
      button =>
        (button.onclick = () => {
          ...
          saved.selected = Number(button.dataset.answer);
          saved.answeredAt = Date.now();
          if (save(course)) reopen(section, id, index, '.academy-answer');
        })
    );
    const input = main.querySelector('#academy-response');
    if (input && !record.completedAt && !(step.required && saved.revealedAt))
      input.oninput = () => {
        saved.draft = input.value;
        save(course);
    ...
    if (comparison && !record.completedAt)
      comparison.oninput = () => {
  ...
  function result(course, record) {
    const lesson = record.content,
      checks = lesson.steps.filter(step => step.kind === 'check');
    ...
    const root = el('<div></div>');
    root.appendChild(topbar(course.section));
    const main =
      el(`<main class="panel academy-lesson"><span class="label">Lesson complete</span><h1>${esc(lesson.title)}</h1>
      ...
      ${lesson.steps
        .filter(step => step.kind === 'reflect')
        .map(
          step => `<h2>Your explanation</h2><p class="academy-written-work">${esc(record.steps[step.id]?.firstDraft || record.steps[step.id]?.draft || 'No written response added.')}</p>
        ${record.steps[step.id]?.comparison ? `<h3>Your comparison</h3><p class="academy-written-work">${esc(record.steps[step.id].comparison)}</p>` : ''}`
        )
        .join('')}
    ...
    const nextButton = main.querySelector('#academy-next-lesson');
    if (nextButton) nextButton.onclick = () => open(course.section, next.id);
    root.appendChild(main);
    setView(root);
  }
