...
(function (root) {
  function rows(outline, course, banks) {
    ...
    return outline.concepts.flatMap(concept => concept.categories.map(category => {
      ...
      return {
        ...category, section: concept.section, lessons,
        ...
        skills: [...new Set([...lessons.flatMap(unit => unit.skills || []), ...practice.map(q => q.skill).filter(Boolean)])],
      };
    }));
  }
  function render() {
    ...
    if (concept && !category.startsWith('CARS-')) return renderCategory(concept.categories.find(cat => cat.id === category), concept, concept.section);
    ...
    const main = el(`<main class="course-page coverage-page">
      ...
      <form class="coverage-filters" aria-label="Filter the coverage map">
        ...
        <label>Section<select name="section"><option value="all">All sections</option>${Object.entries(COURSE_SECTIONS).map(([id, name]) => `<option value="${id}" ${id === section ? 'selected' : ''}>${esc(name)}</option>`).join('')}</select></label>
    ...
    const skillNames = Object.fromEntries([...MCAT.outline.scienceSkills, ...MCAT.outline.carsSkills].map(skill => [skill.id, skill.title]));
    function update() {
      const query = form.elements.query.value.trim().toLowerCase();
      const selected = form.elements.section.value, gaps = form.elements.gaps.checked;
      const shown = data.filter(row => (selected === 'all' || selected === row.section) &&
        (!gaps || row.missingTopics.length) &&
        [row.id, row.title, ...row.topics, ...row.lessons.flatMap(u => [u.title, u.subtitle])].join(' ').toLowerCase().includes(query));
      const url = new URL(location.href);
      for (const [key, value] of [['q', query], ['section', selected === 'all' ? '' : selected], ['gaps', gaps ? '1' : '']]) {
        if (value) url.searchParams.set(key, value); else url.searchParams.delete(key);
      }
      history.replaceState({}, '', url.pathname + url.search);
      main.querySelector('#coverage-count').textContent = `${shown.length} of ${data.length} areas shown. Expand an area for objectives, resources, and gaps.`;
      main.querySelector('#coverage-results').innerHTML = shown.length ? shown.map(row => `<details class="coverage-area">
        ...
        </div></details>`).join('') : '<p class="course-notice">No matching areas. Try a broader topic or clear the filters.</p>';
      main.querySelectorAll('[data-coverage-practice]').forEach(button => button.onclick = () => {
        ...
        renderCategory(concept.categories.find(cat => cat.id === id), concept, concept.section);
      });
    }
