...
self.onmessage = async event => {
  ...
  const output = () => {
    send({ type: 'output', stdout, stderr });
    sinceOutput = 0;
  };
  const write = stream => line => {
    const text = String(line) + '\n';
    if (stdout.length + stderr.length + text.length > 32768)
      throw new Error('Output limit reached. Print a smaller sample and try again.');
  ...
  try {
    ...
    result?.destroy?.();
    output();
    send({ type: 'result', ok: true, stdout, stderr });
  } catch (error) {
