...
function validEkgPattern(item) {
  return ekgObject(item) && ECGTrace.kinds.includes(item.id) && Number.isInteger(item.revision) && item.revision > 0
...
function validEkgRecord(record) {
  return ekgObject(record) && ekgText(record.id) && validEkgPattern(record.content) && ECGTrace.valid(record.trace)
    && Number.isFinite(record.startedAt) && typeof record.draft === 'string' && typeof record.comparison === 'string'
    && Array.isArray(record.options) && record.options.length === 4
    && record.options.every(option => ekgObject(option) && ECGTrace.kinds.includes(option.id) && ekgText(option.name)
      && (option.features == null || ekgText(option.features)))
    && new Set(record.options.map(option => option.id)).size === 4 && record.options.some(option => option.id === record.content.id)
    && (record.selected === null && record.answeredAt == null && record.firstExplanation == null && record.completedAt == null
      || record.options.some(option => option.id === record.selected) && Number.isFinite(record.answeredAt) && ekgText(record.firstExplanation)
        && (record.completedAt == null || Number.isFinite(record.completedAt) && ekgText(record.comparison)));
}
function defaultEkgProg() { return { drill: { correct: 0, total: 0 }, byCat: {}, reviewed: [], records: {}, active: null, category: 'all' }; }
function safeEkgProg(raw) {
  const base = { ...defaultEkgProg(), ...raw };
  const count = value => ekgObject(value) && Number.isInteger(value.correct) && Number.isInteger(value.total)
    && value.correct >= 0 && value.total >= value.correct;
  if (!ekgObject(raw) || !count(base.drill) || !ekgObject(base.byCat) || !Object.values(base.byCat).every(count)
    || !Array.isArray(base.reviewed) || !base.reviewed.every(id => ECGTrace.kinds.includes(id))
    || !ekgObject(base.records) || Object.entries(base.records).some(([id, record]) => id !== record.id || !validEkgRecord(record))
    || base.active !== null && !base.records[base.active] || !ekgText(base.category)) {
    StudyStorage.sessionFailed(); return defaultEkgProg();
  }
  return base;
}
let EKG_PROG = safeEkgProg(StudyStorage.read(EKG_KEY, {}));
StudyStorage.watch(EKG_KEY, () => EKG_PROG);
function saveEkgProg() { return StudyStorage.write(EKG_KEY, EKG_PROG); }
...
function ekgHubStats() {
  const reviewed = ekgReviewedSet().size, total = ECGTrace.kinds.length;
  return { reviewed, total, pct: Math.round(100 * reviewed / total),
...
async function loadEkg() {
  ...
  if (!data || !Array.isArray(data.patterns) || data.patterns.length !== ECGTrace.kinds.length
    || !data.patterns.every(validEkgPattern) || new Set(data.patterns.map(item => item.id)).size !== ECGTrace.kinds.length)
    throw Error('ECG examples are incomplete');
  EKG_DATA = data; return data;
}
function ekgRoute(mode, params = {}) {
  const url = new URL(sectionUrl('reference'), location.origin); url.searchParams.set('tool', 'ecg'); url.searchParams.set('mode', mode);
  for (const [key, value] of Object.entries(params)) if (value) url.searchParams.set(key, value);
  if (url.pathname + url.search !== location.pathname + location.search) history.replaceState({}, '', url.pathname + url.search);
}
function newEkgRecord() {
  ...
  const shuffle = values => {
    const out = values.slice(); for (let i = out.length - 1; i > 0; i--) { const j = Math.floor(Math.random() * (i + 1)); [out[i], out[j]] = [out[j], out[i]]; } return out;
  };
  const pool = EKG_DATA.patterns.filter(item => EKG_PROG.category === 'all' || item.cat === EKG_PROG.category);
  if (!pool.length) { EKG_PROG.category = 'all'; return newEkgRecord(); }
  const item = pool[Math.floor(Math.random() * pool.length)], id = `ecg-${Date.now()}-${Math.random().toString(36).slice(2, 9)}`;
  const record = { id, content: JSON.parse(JSON.stringify(item)), trace: ECGTrace.create(item.id),
    options: shuffle([item, ...shuffle(EKG_DATA.patterns.filter(other => other.id !== item.id)).slice(0, 3)]).map(option => ({ id: option.id, name: option.name, features: option.clue })),
    draft: '', comparison: '', startedAt: Date.now(), selected: null, answeredAt: null, firstExplanation: null, completedAt: null };
  EKG_PROG.records[id] = record; EKG_PROG.active = id;
  ekgRoute('drill', { record: id });
  return saveEkgProg() ? record : null;
}
function answerEkg(record, selected) {
  if (StudyStorage.paused || record.selected !== null || !ekgText(record.draft) || !record.options.some(option => option.id === selected)) return false;
  record.selected = selected; record.firstExplanation = record.draft; record.answeredAt = Date.now();
  const correct = selected === record.content.id, cat = record.content.cat;
...
function completeEkg(record) {
  ...
  record.completedAt ||= Date.now(); return saveEkgProg();
}
...
async function renderEKG(tab = 'library', opts = {}) {
  ...
  try {
    ...
    const stats = ekgHubStats(), root = el('<div></div>'); root.appendChild(topbar('reference'));
    ...
    const target = tab === 'library' && ECGTrace.kinds.includes(focusId)
      ? main.querySelector(`[data-pattern="${focusId}"]`) : opts.feedback ? main.querySelector('#ekgfeedback') : null;
    ...
    const root = el('<div></div>'); root.appendChild(topbar('reference'));
    const main = el(`<main class="panel"><h1>ECG study could not open.</h1><p>${esc(error.message)}</p><p>Your saved exercises have been kept.</p>
      <button class="btn" id="ekgretry">Retry</button><button class="btn" id="ekghome">Medicine home</button></main>`);
    main.querySelector('#ekgretry').onclick = () => renderEKG(tab, opts); main.querySelector('#ekghome').onclick = medicineHome;
    root.appendChild(main); setView(root);
  }
}
function buildEkgLibrary(body, focus) {
  body.appendChild(el('<p class="course-caption">Open a pattern to inspect its trace. “Marked reviewed” records a reading action, not an interpretation assessment.</p>'));
  const list = el('<div class="ekglist"></div>');
  EKG_DATA.patterns.forEach(item => {
    const open = item.id === focus, done = ekgReviewedSet().has(item.id);
    const card = el(`<section class="ekgitem ${open ? 'open' : ''}"><button class="ekghead" data-pattern="${item.id}" aria-expanded="${open}">
      ...
      ${open ? `<div class="ekgdetail">${ECGTrace.markup(ECGTrace.create(item.id))}<p><strong>Modeled timing:</strong> ${esc(item.rate)}${/^(?:About )?\d/.test(item.rate) ? ' per minute' : ''}.</p>
      ...
      ${done ? '' : '<button class="btn" id="ekgmark">Mark this entry reviewed</button>'}</div>` : ''}</section>`);
    card.querySelector('[data-pattern]').onclick = () => renderEKG('library', { focus: open ? null : item.id, returnFocus: item.id });
    const button = card.querySelector('#ekgmark'); if (button) button.onclick = () => { if (markEkgReviewed(item.id)) renderEKG('library', { focus: item.id }); };
    list.appendChild(card);
  });
  body.appendChild(list);
}
function buildEkgDrill(body, record) {
  const answered = record.selected !== null, complete = !!record.completedAt, item = record.content;
  const chosen = record.options.find(option => option.id === record.selected);
  const card = el(`<section class="academy-lesson"><span class="label">${complete ? 'Saved completed exercise' : answered ? 'Compare your reasoning' : 'Observe, measure, explain'}</span>
    ${ECGTrace.markup(record.trace)}
    ${answered ? `<h2 id="ekgfeedback" tabindex="-1">${esc(item.name)}</h2><p><strong>First choice:</strong> ${esc(chosen.name)} · ${record.selected === item.id ? 'Matched the authored pattern' : 'Review the distinction'}</p>
  ...
  const input = card.querySelector('#ekgreason'), buttons = card.querySelectorAll('[data-pick]');
  const updateChoices = () => buttons.forEach(button => { button.disabled = !ekgText(record.draft) || StudyStorage.paused; });
  if (input) { input.oninput = () => { record.draft = input.value; saveEkgProg(); updateChoices(); }; updateChoices(); }
  buttons.forEach(button => button.onclick = () => { if (answerEkg(record, button.dataset.pick)) renderEKG('drill', { record: record.id, feedback: true }); });
  const comparison = card.querySelector('#ekgcomparison'), finish = card.querySelector('#ekgfinish');
  if (finish) {
    finish.disabled = !ekgText(record.comparison);
    comparison.oninput = () => { record.comparison = comparison.value; saveEkgProg(); finish.disabled = !ekgText(record.comparison) || StudyStorage.paused; };
    finish.onclick = () => { if (completeEkg(record)) renderEKG('drill', { record: record.id }); };
  }
  card.querySelector('#ekgcategory').onchange = event => { if (StudyStorage.paused) return; EKG_PROG.category = event.target.value; saveEkgProg(); };
  const next = card.querySelector('#ekgnext'); if (next) next.onclick = () => {
    const created = newEkgRecord(); if (created) renderEKG('drill', { record: created.id });
  };
  body.appendChild(card);
  const records = Object.values(EKG_PROG.records).sort((a, b) => b.startedAt - a.startedAt);
  const history = el(`<details class="ecg-history"><summary>Saved exercises (${records.length}) and earlier drill totals</summary>
  ...
  history.querySelectorAll('[data-record]').forEach(button => button.onclick = () => renderEKG('drill', { record: button.dataset.record }));
