...
(() => {
  'use strict';
  const Core = AcademyCurriculum, KEY = 'cs-academy-connections-v1';
  let catalog = null, loading = null, state = null, generation = 0;
  ...
  function academyUrl(view, values = {}) {
    const url = new URL(sectionUrl('academy'), location.origin); url.searchParams.set('view', view);
    for (const [key, value] of Object.entries(values)) if (value) url.searchParams.set(key, value);
    return url.pathname + url.search;
  }
  function href(entry) {
    const url = Core.destination(entry, current(), back()); if (!url) return url;
    // A link back to the lesson the learner came from must not carry a return link to itself.
    const parsed = new URL(url, location.origin), key = Core.contextKey(url);
    if (key && key === Core.contextKey(parsed.searchParams.get('returnTo'))) { parsed.searchParams.delete('returnTo'); return parsed.pathname + parsed.search + parsed.hash; }
    return url;
  }
  ...
  function shell(title, description, body) {
    ...
    root.appendChild(el(`<main class="panel academy-shell academy-connect"><div class="academy-intro"><span class="label">Connected Academy</span><h1>${esc(title)}</h1><p>${esc(description)}</p><nav aria-label="Academy discovery"><a href="${esc(academyUrl('curriculum'))}" data-connect-view>Find a lesson</a><a href="${esc(academyUrl('queue'))}" data-connect-view>Optional retrieval</a><a href="${esc(sectionUrl('academy'))}" data-connect-view>All courses</a></nav></div>${body}</main>`));
    return root;
  }
  function wireNavigation(root) {
    root.querySelectorAll('[data-connect-view], [data-connect-course]').forEach(link => link.onclick = event => {
      if (!normalClick(event)) return; event.preventDefault();
      history.pushState({}, '', link.getAttribute('href')); openSection(link.dataset.connectCourse || 'academy');
    });
  }
  function show(root) { wireNavigation(root); setView(root); }
  async function load() {
    if (catalog) return catalog;
    if (!loading) loading = fetch('data/academy-curriculum.json?v=8').then(response => {
      if (!response.ok) throw Error('Curriculum could not load.'); return response.json();
    }).then(Core.validate).then(data => catalog = data).finally(() => loading = null);
    return loading;
  }
  function queueState() {
    if (state) return state;
    const saved = StudyStorage.read(KEY, Core.emptyState());
    if (!Core.validState(saved)) { StudyStorage.sessionFailed(); state = Core.emptyState(); }
    else state = saved;
    StudyStorage.watch(KEY, () => state); return state;
  }
  ...
  function discovery() {
    const p = params(), context = catalog.entries.find(e => e.id === p.get('context'));
    const filters = { q: p.get('q') || '', track: p.get('track') || '', level: p.get('level') || '', objective: p.get('objective') || '', drafts: p.get('scope') === 'drafts' };
    const results = Core.search(catalog, filters), matches = new Set(results.map(e => e.id));
    const pages = Math.max(1, Math.ceil(results.length / 24)), page = Math.min(pages, Math.max(1, Number.parseInt(p.get('page'), 10) || 1));
    const pageUrl = number => { const url = new URL(location.href); url.searchParams.set('page', String(number)); return url.pathname + url.search; };
    const connections = context ? catalog.connections.filter(c => c.from === context.id || c.to === context.id).map(c => ({
      entry: catalog.entries.find(e => e.id === (c.from === context.id ? c.to : c.from)), connection: c,
    })).filter((item, index, items) => matches.has(item.entry.id) && items.findIndex(other => other.entry.id === item.entry.id) === index) : [];
    const cards = context ? catalog.cards.filter(c => c.links.some(e => e.id === context.id)) : [];
    const root = shell('Find the next useful connection.', 'Search public lesson descriptions and selected shared objectives. Preparation levels are author suggestions, not calibrated difficulty or a measurement of your ability.', `
      ...
      ${context ? `<section><h2>Connected to: ${esc(context.title)}</h2><p>${esc(context.summary)}</p><p>These optional links explain a teaching connection. They do not add tasks, change a plan or complete either course. ${courseLink(context, 'Stay with this lesson')}</p>
        ...
        ${cards.length ? `<p>Related optional retrieval: ${cards.map(c => `<a data-connect-view href="${esc(academyUrl('queue', { card: c.id }))}">${esc(c.title)}</a>`).join(' · ')}. Opening a prompt does not add it to your queue.</p>` : ''}</section>` : p.get('context') ? '<p role="status">That lesson is not in the current index. Your return link and general search remain available.</p>' : ''}
      <section><h2>${results.length} matching ${results.length === 1 ? 'entry' : 'entries'}</h2>${results.length ? `<p>Page ${page} of ${pages}</p><div class="academy-catalog">${results.slice((page - 1) * 24, page * 24).map(e => entryCard(e)).join('')}</div><nav aria-label="Search result pages">${page > 1 ? `<a data-connect-view href="${esc(pageUrl(page - 1))}">Previous page</a>` : ''}${page < pages ? `<a data-connect-view href="${esc(pageUrl(page + 1))}">Next page</a>` : ''}</nav>` : '<p>No matches. Try fewer words or clear the course, level or objective filters.</p>'}</section>`);
    root.querySelector('#connect-search').onsubmit = event => {
      event.preventDefault(); const form = new FormData(event.currentTarget);
      history.pushState({}, '', academyUrl('curriculum', { context: context?.id, q: String(form.get('q') || '').trim(), track: form.get('track'), level: form.get('level'), objective: form.get('objective'), scope: form.has('drafts') ? 'drafts' : '' })); render();
    };
    show(root);
  }
  ...
  function recordUrl(record) { return academyUrl('queue', { run: record.id }); }
  function queue(unavailable = false) {
    const saved = queueState(), p = params(), record = saved.records.find(r => r.id === p.get('run'));
    const cards = catalog?.cards || [], selected = cards.find(c => c.id === p.get('card'));
    if (record) { player(record); return; }
    const pending = saved.records.filter(r => !r.completedAt && r.status === 'pending');
    const root = shell('Practice a connection, if it helps.', 'Choose original written prompts that connect subjects. Each attempt keeps its own explanation and comparison. Nothing here changes a course score, lesson completion or study plan.', `
      ...
      <section><h2>${unavailable ? 'New prompts unavailable' : selected ? 'Selected prompt' : 'Choose a prompt'}</h2><div class="academy-catalog">${(selected ? [selected] : cards).map(c => {
      ...
      }).join('')}</div>${selected ? `<p><a data-connect-view href="${esc(academyUrl('queue'))}">See all prompts and records</a></p>` : ''}</section>
      <section><h2>Completed, set aside and removed</h2><p>Removing a prompt from the queue retains its writing here. Saved content is kept even if the catalog changes.</p><ul class="connect-records">${saved.records.filter(r => r.completedAt || r.status !== 'pending').slice().reverse().map(r => `<li><a data-connect-view href="${esc(recordUrl(r))}">${esc(r.content.title)}</a> · ${r.completedAt ? 'Written comparison completed' : r.status === 'deferred' ? 'Set aside' : 'Removed from queue'} · ${esc(new Date(r.queuedAt).toLocaleDateString())}</li>`).join('') || '<li>No earlier records.</li>'}</ul></section>`);
    root.querySelector('#connect-enable').onclick = () => write(() => { saved.enabled = !saved.enabled; return true; });
    const retry = root.querySelector('#connect-retry'); if (retry) retry.onclick = render;
    root.querySelectorAll('[data-connect-add]').forEach(button => button.onclick = () => {
      ...
      if (!run) { root.querySelector('#connect-status').textContent = 'This prompt could not be added. Turn retrieval on, or keep your existing records and download a backup if the record limit is reached.'; return; }
    ...
    wireMoves(root); show(root); pauseControls(root);
  }
  function wireMoves(root) {
    root.querySelectorAll('[data-connect-move]').forEach(button => button.onclick = () => {
      if (!write(() => Core.move(state, button.dataset.connectMove, button.dataset.status))) {
        const message = root.querySelector('#connect-status');
        if (message && !StudyStorage.paused) message.textContent = 'Another unfinished attempt for this prompt is already in the queue. Open that attempt first.';
      }
    });
  }
  function player(record) {
    const card = record.content, revealed = record.revealedAt != null;
    const canEdit = state.enabled && record.status === 'pending' && !record.completedAt;
    const root = shell(card.title, 'This original cross-course prompt uses a frozen copy of the content saved when you added it.', `
    ...
    const draft = root.querySelector('#connect-draft'), comparison = root.querySelector('#connect-comparison');
    const input = (node, field) => { if (node && canEdit && !(field === 'draft' && revealed)) node.oninput = () => {
      write(() => Core.edit(state, record.id, field, node.value), () => status.textContent = 'Writing saved in this workspace.');
    }; };
    input(draft, 'draft'); input(comparison, 'comparison');
    const revealButton = root.querySelector('#connect-reveal');
    if (revealButton) revealButton.onclick = () => { if (!draft.value.trim()) { status.textContent = 'Write your own explanation before showing the model.'; draft.focus(); return; } write(() => Core.reveal(state, record.id, Date.now())); };
    const completeButton = root.querySelector('#connect-complete');
    if (completeButton) completeButton.onclick = () => { if (!comparison.value.trim()) { status.textContent = 'Write what matched, changed or still needs work before completing this comparison.'; comparison.focus(); return; } write(() => Core.complete(state, record.id, Date.now())); };
  ...
  async function render() {
    const token = ++generation, address = current();
    ...
    if (!catalog) show(shell('Loading the curriculum…', 'Your course work stays saved while the public catalog loads.', '<p role="status">Loading lesson descriptions.</p>'));
    try {
      await load(); if (token !== generation || address !== current()) return;
      if (params().get('view') === 'queue') queue(); else discovery();
    } catch {
      if (token !== generation || address !== current()) return;
      if (params().get('view') === 'queue') { queue(true); return; }
      const root = shell('The curriculum could not load.', 'Your saved work has not been cleared. Try the catalog again when its course files are available.', '<button class="btn" id="connect-retry">Try again</button>');
      root.querySelector('#connect-retry').onclick = render; show(root);
    }
  }
  function attach(root) {
    const main = root.querySelector('main'); if (!main || root.querySelector('.connect-return')) return;
    const returnTo = back(), context = Core.contextKey(current());
    if (returnTo) {
      const target = new URL(returnTo, location.origin), track = Object.keys(Core.paths).find(key => Core.paths[key] === target.pathname);
      const here = new URL(location.href); here.searchParams.delete('returnTo');
      main.prepend(el(`<aside class="connect-return" aria-label="Return to your starting point"><a href="${esc(returnTo)}">Return to ${esc(trackName(track))}</a><span>Your course records stay separate.</span><a href="${esc(here.pathname + here.search + here.hash)}">End this return link</a></aside>`));
    }
    if (context) {
      ...
      main.appendChild(el(`<aside class="connect-context"><a href="${esc(url.pathname + url.search)}">Explore related lessons</a><p>Optional connections with a return link to your starting point.</p></aside>`));
    }
  }
  window.addEventListener('study-storage-paused', () => { const root = document.querySelector('.academy-connect'); if (root) pauseControls(root); });
  window.addEventListener('study-storage-recovered', () => { if (location.pathname === '/academy' && params().get('view') === 'queue') render(); });
