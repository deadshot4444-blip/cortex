...
async function loadExperimentNotes() {
  ...
  try {
    ...
    /* The notebook remains usable without model notes. */
  }
}
...
function studyLastDay(plan) {
  ...
  Object.values(plan.completed || {})
    .filter(Number.isFinite)
    .forEach(ts => times.push(guideDateKey(new Date(ts))));
  const activity = [
    ...Object.values(courseState.units).flatMap(r => [
      r.startedAt,
      r.completedAt,
      ...(r.attempts || []).map(a => a.ts),
    ]),
    ...QLOG.map(x => x.ts),
    ...Object.values(SRS).map(x => x.last),
    ...Object.values(repairState?.records || {}).map(x => x.reviewedAt),
  ].filter(Number.isFinite);
  if (activity.length) times.push(guideDateKey(new Date(Math.max(...activity))));
  return times.sort().at(-1);
}
function studyTouch(plan) {
  plan.lastStudyDate = guideDateKey();
  saveGuidePlan(plan);
}
function studyCompletedActivities(plan) {
  ...
  for (const session of Object.values(plan?.sessions || {}))
    for (const task of session.tasks || []) {
      const id = guideTaskKey(task.day, task.id),
        ts = plan.completed?.[id];
      if (!Number.isFinite(ts)) continue;
      completed.set(id, {
        id: `daily:${id}`,
        type: task.type,
        key: task.unitId || task.conceptId || task.id,
        title: task.type === 'course' ? task.desc : task.title,
        minutes: task.minutes,
        ts,
        day: guideDateKey(new Date(ts)),
        kind: task.courseKind || 'lesson',
        reason: 'Completed in your daily session.',
      });
    }
...
function studySavedTask(plan) {
  if (plan.active && !guideTaskDone(plan, plan.active.day, plan.active.id)) {
    ...
    if (
      (spec && loadResume(spec.key)) ||
      (a.type === 'repair' && repairState?.active) ||
      (a.type === 'course' && courseUnit(a.unitId)) ||
      (a.type === 'examReview' && courseExamReports().some(r => r.attemptId === a.examId))
    ) {
      ...
      if (original)
        return {
          ...original,
          resume: true,
          minutes: original.minutes || { flash: 10, drill: 15, cars: 20, passage: 25, exam: 65, repair: 5 }[a.type],
        };
    }
  }
  if (repairState?.active && repairState.active.phase !== 'done') {
    const active = repairState.active,
      concept = repairConcept(active.conceptId);
    if (concept)
      return {
        ...(active.guideTask || {}),
        id: active.guideTask?.id || `flex-${guideDateKey()}-repair`,
        day: active.guideTask?.day || guidePlanDay(plan),
        type: 'repair',
        conceptId: concept.id,
        title: 'Continue your concept session',
        desc: concept.title,
        minutes: 5,
        resume: true,
      };
  ...
  if (!best && courseUnit(courseState.activeUnit) && !courseRecord(courseState.activeUnit).completedAt) {
    const unit = courseUnit(courseState.activeUnit),
      record = courseRecord(unit.id);
    return {
      ...(record.guideTask || {}),
      id: record.guideTask?.id || `flex-${guideDateKey()}-course`,
      day: record.guideTask?.day || guidePlanDay(plan),
      type: 'course',
      unitId: unit.id,
  ...
  return {
    ...(best.r.guideTask || {}),
    id: best.r.guideTask?.id || `flex-${guideDateKey()}-resume-${type}`,
    day: best.r.guideTask?.day || guidePlanDay(plan),
    type,
    resume: true,
    resumeKey: best.spec.key,
    title: `Resume ${best.spec.mod}`,
    desc: 'Your unfinished session is saved, including its review.',
    minutes: { flash: 10, drill: 15, cars: 20, passage: 25, exam: 65 }[type],
  };
}
function studyBuildSession(plan, minutes, previous) {
  const date = guideDateKey(),
    day = guidePlanDay(plan);
  ...
  const base = type => ({
    ...
    day,
    type,
    category: focus?.id || null,
    section: focus?.section || 'bioBiochem',
  });
  const examReview =
    courseState.mode === 'exam' && courseExamReports().find(r => !courseState.examReviews[r.attemptId]?.reviewedAt);
  if (!taper && courseState.mode === 'exam' && !examReview && remaining >= 51 && !tasks.some(t => t.type === 'exam'))
    add({
      ...base('exam'),
      minutes: 51,
      title: 'Rehearse a timed CARS set',
      desc: 'Up to 30 questions in whole passage sets, followed by a saved review.',
      section: 'cars',
    });
  const rec =
    repairData &&
    repairState?.active?.phase !== 'done' &&
    McatRepairCore.recommend(repairData.concepts, repairState, QLOG, nowTs());
  if (
    rec &&
    remaining >= (courseState.mode === 'learn' && courseRec() && !taper ? 17 : 5) &&
    !tasks.some(t => t.type === 'repair')
  )
    add({
      ...base('repair'),
      minutes: 5,
      title: rec.kind === 'later' ? 'Due concept check' : 'Five-minute concept repair',
      desc: rec.concept.title,
      conceptId: rec.concept.id,
      repairKind: rec.kind,
    });
  const courseNext = courseRec();
  if (!taper && examReview && remaining >= 10 && !tasks.some(t => t.type === 'examReview'))
    add({
      ...base('examReview'),
      minutes: 10,
      title: 'Review your timed run',
      desc: 'Revisit answers and save one change for next time.',
      examId: examReview.attemptId,
    });
  if (
    !taper &&
    courseNext &&
    remaining >= (['delayed', 'review'].includes(courseNext.kind) ? 5 : 12) &&
    !tasks.some(t => t.type === 'course') &&
    (courseState.mode === 'learn' || ['delayed', 'review'].includes(courseNext.kind))
  )
    add({
      ...base('course'),
      minutes: ['delayed', 'review'].includes(courseNext.kind) ? 5 : 12,
      courseKind: courseNext.kind,
      title:
        courseNext.kind === 'delayed'
          ? 'Later application check'
          : courseNext.kind === 'review'
            ? 'Revisit a foundation'
            : 'Learn one foundation',
      desc: courseNext.unit.title,
      unitId: courseNext.unit.id,
    });
  ...
  if (cardMinutes >= 5 && !tasks.some(t => t.type === 'flash') && MCAT.cards.length)
    add({
      ...base('flash'),
      title: dueCount() ? 'Due cards first' : 'Focused recall',
      desc: 'A small card set. Due cards take priority over new cards.',
      minutes: cardMinutes,
      limitCards: cardMinutes,
      limitDue: cardMinutes,
      newCards: taper ? 0 : Math.min(3, cardMinutes),
    });
  ...
  for (const type of taper ? [] : passageTypes) {
    ...
    if (
      ...
      (type === 'cars' ? MCAT.cars.length : MCAT.sci.length)
    )
      add({
        ...base(type),
        title: type === 'cars' ? 'CARS + blind review' : 'Science + experiment notebook',
        desc:
          type === 'cars'
            ? 'Answer, revisit your reasoning, then see the key.'
            : 'Work the data, map the experiment, then compare explanations.',
        minutes: cost,
      });
  }
  if (remaining >= 4 && !tasks.some(t => t.type === 'drill') && MCAT.questions.length) {
    ...
    add({
      ...base('drill'),
      title: 'Focused question practice',
      desc: focus ? `${focus.id} · ${focus.title}` : 'Practice and review a few questions.',
      minutes: questions * 2,
      questions,
    });
  }
...
function studyDailySession(plan, minutes) {
  ...
  const unavailable = task =>
    task.type === 'course'
      ? !courseUnit(task.unitId)
      : task.type === 'repair'
        ? !repairData
        : task.type === 'flash'
          ? !MCAT.cards.length
          : task.type === 'drill'
            ? !MCAT.questions.length
            : task.type === 'cars'
              ? !MCAT.cars.length
              : task.type === 'passage'
                ? !MCAT.sci.length
                : task.type === 'exam'
                  ? !MCAT.questions.length || !MCAT.cars.length || !MCAT.sci.length
                  : false;
  ...
  const courseDueChanged =
    !saved &&
    courseDue?.kind === 'delayed' &&
    !old?.tasks.some(t => t.type === 'course' && t.unitId === courseDue.unit.id && !guideTaskDone(plan, t.day, t.id));
  const dueChanged =
    !saved &&
    rec?.kind === 'later' &&
    !rec.resume &&
    !old?.tasks.some(t => t.type === 'repair' && t.conceptId === rec.concept.id && t.repairKind === 'later');
  if (
    !old ||
    old.courseVersion !== 2 ||
    old.mode !== courseState.mode ||
    minutes ||
    dueChanged ||
    courseDueChanged ||
    (saved && (first?.id !== saved.id || first?.day !== saved.day))
  ) {
...
function studyLaunchTask(task, plan) {
  if (task.type === 'course') {
    const record = courseRecord(task.unitId);
    record.guideTask = task;
    plan.active = task;
    studyTouch(plan);
    saveCourse();
    renderCourseUnit(task.unitId, task.courseKind === 'review' ? 'learn' : undefined);
    return;
  }
  if (task.type === 'examReview') {
    plan.active = task;
    studyTouch(plan);
    courseOpenExam(task.examId);
    return;
  }
  if (task.type === 'repair') {
    ...
    if (!active && !McatRepairCore.begin(concept, repairState, task.repairKind || 'repair', nowTs())) {
      ...
      const today = guideDateKey();
      const checkedToday = (repairState.records[concept.id]?.attempts || []).some(
        a => a.mode !== 'diagnose' && guideDateKey(new Date(a.at)) === today
      );
      if (checkedToday) studyCompleteRepair({ guideTask: task });
      else studyDailySession(plan, plan.dailyMinutes || 30);
      return renderGuide();
    }
  ...
  const spec = guideResumeSpec(task.type),
    saved = spec && loadResume(spec.key);
  ...
  if (saved && (task.resume || spec.progressOf(saved) > 0 || saved.guideTask)) {
    plan.active = saved.guideTask || task;
    saved.guideTask = plan.active;
    studyTouch(plan);
    spec.resume(saved);
    return;
  }
...
function mountDailySession(host, plan) {
  const session = studyDailySession(plan),
    tasks = session.tasks;
  ...
  host.innerHTML = `<section class="study-session" aria-label="Flexible daily session">
    ...
    <div class="study-task-list">${tasks
      .map((t, i) => {
        const complete = guideTaskDone(plan, t.day, t.id);
        return `<button class="study-task ${complete ? 'done' : ''}" data-study-task="${i}" ${complete ? 'disabled' : ''}><span class="study-task-index">${complete ? '✓' : String(i + 1).padStart(2, '0')}</span><span><strong>${esc(t.title)}</strong><small>${esc(t.desc || '')}${t.resume ? ' · Saved session' : ''}</small></span><span class="study-task-time">~${t.minutes}m</span></button>`;
      })
      .join('')}</div>
  ...
  host.querySelectorAll('[data-study-minutes]').forEach(
    b =>
      (b.onclick = () => {
        studyDailySession(guidePlan(), Number(b.dataset.studyMinutes));
        renderGuide();
      })
  );
  host
    .querySelectorAll('[data-study-task]')
    .forEach(b => (b.onclick = () => studyLaunchTask(tasks[Number(b.dataset.studyTask)], guidePlan())));
...
function studyFinishAttempt(run) {
  ...
  run.p.questions.forEach((q, i) => {
    if (!run.results[i])
      run.results[i] = {
        q,
        chosen: null,
        correct: false,
        conf: run.confidence?.[q.id] || 'unsure',
        flagged: !!run.flags?.[q.id],
        unanswered: true,
      };
  });
}
function studyLogAttempt(run, kind) {
  ...
  run.results.forEach(r => {
    ...
    QLOG.push({
      qId: r.q.id,
      section: kind === 'cars' ? 'cars' : run.p.section,
      category: kind === 'cars' ? 'CARS-' + r.q.skill.split('-')[1] : r.q.category,
      passage: run.p.id,
      correct: r.correct,
      conf: r.conf || 'unsure',
    ...
    QHIST[r.q.id] = {
      n: (old?.n || 0) + (old?.lastAttemptId === run.attemptId ? 0 : 1),
...
function studySaveReport(kind, run) {
  ...
  const copy = {
    ...run,
    passageId: run.p.id,
    content: JSON.parse(JSON.stringify(run.p)),
    results: run.results.map(r => ({ ...r, q: r.q.id })),
  };
  delete copy.p;
  delete copy.timerId;
...
function studyReportButton(kind, main) {
  const reports = Object.entries(StudyStorage.read('cs-mcat-passage-reviews', {})).filter(([key]) =>
    key.startsWith(kind + ':')
  );
  ...
  const open = (saved, passage) => {
    const run = {
      ...
      p: passage,
      archived: true,
      sourceSnapshotMissing: !!saved.sourceSnapshotMissing || !saved.content,
      results: saved.results.map(r => ({ ...r, q: passage.questions.find(q => q.id === r.q) })),
    };
    if (kind === 'cars') {
      cars = run;
      renderCarsReviewResult();
    } else {
      plab = run;
      finishPassage();
    }
  ...
  const history = el(
    '<details class="guide-schedule"><summary><span><strong>Saved passage reviews</strong><small>Your most recent review for each passage</small></span><i>Open</i></summary><div class="rows"></div></details>'
  );
  reports
    .map(([, r]) => r)
    .sort((a, b) => b.attemptEndedAt - a.attemptEndedAt)
    .forEach(r => {
      const passage = r.content || (kind === 'cars' ? MCAT.cars : MCAT.sci).find(p => p.id === r.passageId);
      if (!passage) return;
      const row = el(
        `<button class="row"><span class="row-main"><span class="row-title">${esc(passage.title)}</span><span class="row-when">${new Date(r.attemptEndedAt).toLocaleDateString()}</span></span><span class="row-right">Review →</span></button>`
      );
      row.onclick = () => open(r, passage);
      history.querySelector('.rows').appendChild(row);
    });
  main.appendChild(history);
}
function studyLegacyReviewNotice(main, run) {
  if (run.sourceSnapshotMissing)
    main.prepend(
      el(
        '<p class="course-notice">This older report kept answer choices but not its original question text. Current wording is shown and may have changed. The saved results are unchanged.</p>'
      )
    );
}
...
function resumeCarsSession(r) {
  ...
  if (!cars.phase || cars.phase === 'attempt') {
    if (cars.timed) {
      cars.deadline = nowTs() + (r._remain ?? 600000);
      cars.timerId = setInterval(carsTick, 500);
    }
    renderCarsRunner();
...
function resumePassageSession(r) {
  ...
  if (!plab.phase || plab.phase === 'attempt') {
    if (plab.timed) {
      plab.deadline = nowTs() + (r._remain ?? 600000);
      plab.timerId = setInterval(plabTick, 500);
    }
    renderPassageRunner();
...
function studyWirePassageExit(root, kind, run, fallback) {
  const leave = () => {
    studyPausePassage(kind, run);
    guideLeaveActive(kind === 'plab' ? 'passage' : kind, fallback);
  };
  root.querySelector('#exit').onclick = leave;
  root.querySelector('#crumbmcat').onclick = () => {
    studyPausePassage(kind, run);
    renderMCATEntry();
  };
}
function studyLockPassageList(kind, main) {
  const saved = loadResume(kind),
    spec = RESUME_SPECS.find(s => s.key === kind);
  if (!saved || !spec || spec.progressOf(saved) <= 0) return;
  main.querySelectorAll('#plist .row, #rand').forEach(b => {
    b.disabled = true;
  });
  main
    .querySelector('#plist')
    .before(
      el(
        '<p class="study-instruction">You have a passage in progress. Resume it before starting another; your answers and review notes are saved.</p>'
      )
    );
}
...
function beginCarsBlindReview() {
  ...
  cars.phase = 'blind';
  cars.reviews ||= {};
  cars.reviewOrder ||= cars.results
    .map((r, i) => i)
    .sort(
      (a, b) =>
        Number(cars.results[b].flagged || cars.results[b].conf !== 'sure') -
        Number(cars.results[a].flagged || cars.results[a].conf !== 'sure')
    );
  cars.reviewIdx ||= 0;
  saveResume('cars', cars);
  renderCarsBlindReview();
}
function renderCarsBlindReview() {
  if (
    window.McatRehearsal &&
    !window.McatRehearsal.allow({ passageId: cars.p.id }, renderCarsBlindReview, renderCarsHome)
  )
    return;
  const order = cars.reviewOrder,
    index = order[cars.reviewIdx];
  if (index == null) {
    cars.phase = 'done';
    saveResume('cars', cars);
    renderCarsReviewResult();
  ...
  const r = cars.results[index],
    q = r.q;
  const draft = (cars.reviews[q.id] ||= { chosen: r.chosen, evidence: [], rationale: '' });
  let sentenceIndex = 0;
  const passage = carsSentences(cars.p)
    .map(
      par =>
        `<p>${par
          .map(sentence => {
            const i = sentenceIndex++;
            return `<button class="cars-evidence ${draft.evidence.includes(i) ? 'selected' : ''}" data-sentence="${i}" aria-pressed="${draft.evidence.includes(i)}">${esc(sentence)}</button>`;
          })
          .join(' ')}</p>`
    )
    .join('');
  const root =
    el(`<div>${mcatTaskHeader(['CARS', 'Blind review'], `<span class="topstat">${cars.reviewIdx + 1}/${order.length}</span>`)}
    ...
    <p class="q">${esc(q.stem)}</p><div class="opts">${McatV2Core.optionOrder(q.options, q.displayOrder)
      .map(
        (i, position) =>
          `<button class="opt ${draft.chosen === i ? 'study-selected' : ''}" data-review-answer="${i}" aria-pressed="${draft.chosen === i}"><span class="key">${'ABCD'[position]}</span><span>${esc(q.options[i])}</span></button>`
      )
      .join('')}</div>
  ...
  root.querySelectorAll('[data-review-answer]').forEach(
    b =>
      (b.onclick = () => {
        draft.chosen = Number(b.dataset.reviewAnswer);
        root.querySelectorAll('[data-review-answer]').forEach(option => {
          option.classList.toggle('study-selected', option === b);
          option.setAttribute('aria-pressed', option === b);
        });
        update();
      })
  );
  root.querySelectorAll('[data-sentence]').forEach(
    b =>
      (b.onclick = () => {
        const i = Number(b.dataset.sentence);
        draft.evidence = draft.evidence.includes(i) ? draft.evidence.filter(v => v !== i) : [...draft.evidence, i];
        b.classList.toggle('selected', draft.evidence.includes(i));
        b.setAttribute('aria-pressed', draft.evidence.includes(i));
        root.querySelector('#cars-evidence-count').textContent =
          `${draft.evidence.length} supporting sentences selected.`;
        update();
      })
  );
  root.querySelector('#cars-rationale').oninput = e => {
    draft.rationale = e.target.value;
    update();
  };
  next.onclick = () => {
    draft.reviewed = true;
  ...
  root.querySelector('#cars-review-skip').onclick = () => {
...
function renderCarsReviewResult() {
  if (
    window.McatRehearsal &&
    !window.McatRehearsal.allow({ passageId: cars.p.id }, renderCarsReviewResult, renderCarsHome)
  )
    return;
  if (!cars.archived) {
    studyLogAttempt(cars, 'cars');
    cars.guided ||= guideCompleteActiveTask('cars');
    studySaveReport('cars', cars);
    clearResume('cars');
  }
  const total = cars.p.questions.length,
    original = cars.results.filter(r => r.correct).length;
  const reviewed = cars.results.filter(r => cars.reviews?.[r.q.id]?.reviewed);
  const revised = cars.results.filter(
    r => (cars.reviews?.[r.q.id]?.reviewed ? cars.reviews[r.q.id].chosen : r.chosen) === r.q.answer
  ).length;
  ...
  const main =
    el(`<main class="panel study-result"><span class="label">CARS · ${esc(cars.p.title)}</span><h1>What changed on review?</h1>
  ...
  main.querySelector('#review-plan').onclick = renderMCATEntry;
  main.querySelector('#review-passages').onclick = renderCarsHome;
...
const EXPERIMENT_FIELDS = [
  ...
  [
    ...
    'What was changed or compared? For observational work, identify the measured exposure.',
  ],
  ...
  [
    ...
    'Use a specific result. Separate what the data show from a proposed mechanism.',
  ],
...
function experimentGraph(p, model = experimentNotes?.[p.id]) {
  ...
  const rows = p.table.rows,
    xs = rows.map(r => Number(r[graph.xColumn]));
  const series = graph.series.map(s => ({ ...s, ys: rows.map(r => Number(r[s.column])) }));
  const maxX = Math.max(...xs),
    minX = Math.min(0, ...xs),
    maxY = Math.max(...series.flatMap(s => s.ys)) * 1.1;
  const x = v => 52 + (330 * (v - minX)) / (maxX - minX || 1),
    y = v => 205 - (160 * v) / (maxY || 1);
  const colors = ['#0b5cad', '#263442'];
  return `<figure class="study-graph"><svg viewBox="0 0 430 265" role="img" aria-label="${esc(graph.description)}"><title>${esc(graph.description)}</title><path d="M52 35V205H390" fill="none" stroke="#677786"/>${[0, 0.5, 1].map(f => `<text x="46" y="${y(maxY * f) + 4}" text-anchor="end" font-size="11">${Number((maxY * f).toFixed(1))}</text><path d="M52 ${y(maxY * f)}H390" stroke="#dae2eb"/>`).join('')}${xs.map(v => `<text x="${x(v)}" y="223" text-anchor="middle" font-size="11">${v}</text>`).join('')}${series.map((s, i) => `<path d="${s.ys.map((v, j) => `${j ? 'L' : 'M'}${x(xs[j])},${y(v)}`).join(' ')}" fill="none" stroke="${colors[i]}" stroke-width="2.5" ${i ? 'stroke-dasharray="5 4"' : ''}/>${s.ys.map((v, j) => `<circle cx="${x(xs[j])}" cy="${y(v)}" r="3" fill="${colors[i]}"/>`).join('')}`).join('')}<text x="52" y="18" font-size="12">${esc(graph.yLabel)}</text><text x="220" y="247" text-anchor="middle" font-size="12">${esc(graph.xLabel)}</text></svg><figcaption>${series.map((s, i) => `${i ? 'Dashed' : 'Solid'}: ${esc(s.label)}`).join(' · ')}. Points come directly from the passage table; connecting lines are guides.</figcaption></figure>`;
}
function renderExperimentNotebook() {
  if (
    window.McatRehearsal &&
    !window.McatRehearsal.allow({ passageId: plab.p.id }, renderExperimentNotebook, renderPassageHome)
  )
    return;
  plab.phase = 'analysis';
  plab.analysis ||= {};
  const p = plab.p,
    model = experimentModel(plab);
  const root =
    el(`<div>${mcatTaskHeader(['Science', 'Experiment notebook'], '<span class="topstat">Untimed</span>')}<main class="cars-stage study-review-stage">
  ...
  const save = () => {
    next.disabled =
      !EXPERIMENT_FIELDS.every(([id]) => plab.analysis[id]?.trim()) || !!(model?.graph && !plab.analysis.graph?.trim());
    saveResume('plab', plab);
  };
  root.querySelectorAll('[data-experiment]').forEach(
    input =>
      (input.oninput = () => {
        plab.analysis[input.dataset.experiment] = input.value;
        save();
      })
  );
  next.onclick = () => {
    if (StudyStorage.paused || next.disabled || plab.phase === 'done') return;
    plab.analysisSkipped = false;
    plab.phase = 'done';
    saveResume('plab', plab);
    finishPassage();
  };
  root.querySelector('#experiment-skip').onclick = () => {
    if (StudyStorage.paused || plab.phase === 'done') return;
    plab.analysisSkipped = true;
    plab.phase = 'done';
    saveResume('plab', plab);
    finishPassage();
  };
