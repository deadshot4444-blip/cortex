...
(function (root, factory) {
  ...
  const FORMAT = 'cortex-study-backup',
    VERSION = 1,
  ...
  const keys = new Set([
    ...
    'cs-mcat-course-v1',
    'cs-mcat-coursework',
    'cs-mcat-passage-reviews',
    'cs-mcat-rehearsal-v1',
    'cs-mcat-item-reports-v1',
    'cs-mcat-plan',
    'cs-mcat-q',
    'cs-mcat-repairs-v1',
    'cs-mcat-srs',
  ...
  function inspect(data, checkMarkup, action = 'restore') {
    if (!object(data) || Object.keys(data).length > 100)
      throw Error('The file does not contain a supported study workspace.');
    let count = 0;
    function walk(value, depth = 0, field = '', plainText = false, rootKey = '') {
      ...
      if (typeof value === 'string') {
        if (/(?:^id$|Id$|^key$)/.test(field) && value && !/^[\w:.|/@+ -]{1,250}$/.test(value))
          throw Error('The backup contains an invalid content identifier.');
        if (/(?:url|href)$/i.test(field) && value) {
          ...
          let url;
          try {
            url = new URL(value, 'https://cortexmedical.academy/');
          } catch {
            throw Error('The backup contains an invalid source link.');
          }
          if (!['https:', 'http:'].includes(url.protocol))
            throw Error('The backup contains an unsupported source link.');
        }
        if (!plainText && /<[a-z/!]/i.test(value)) {
          if (!checkMarkup || !checkMarkup(value))
            throw Error(
              action === 'export'
                ? 'The saved record ' +
                    rootKey +
                    ' contains HTML that a portable backup cannot carry, so no backup was prepared. Download recovery copies instead.'
                : 'The backup contains unsupported HTML. Keep it as a recovery copy; it has not been restored.'
            );
        }
      } else if (value && typeof value === 'object') {
        for (const [key, item] of Object.entries(value)) {
          if (['__proto__', 'prototype', 'constructor'].includes(key) || /[<>"'`&]/.test(key))
            throw Error('The backup contains an unsafe record key.');
          ...
          walk(
            ...
            plainText ||
              learnerText.has(key) ||
              (rootKey === 'cs-clinical-longitudinal-v1' && longitudinalText.has(key)) ||
              (rootKey === 'cs-neuro' && depth === 0 && key === 'projects') ||
              (rootKey === 'cs-cogpsych' && depth === 0 && key === 'practice') ||
              (rootKey === 'cs-mcat-course-v1' && depth === 2 && key === 'help'),
            rootKey
          );
        }
      }
    ...
    for (const [key, raw] of Object.entries(data)) {
      ...
      try {
        ...
        throw Error('The saved record ' + key + ' is damaged.');
      }
  ...
  function payload(file) {
    return {
      ...
      data: Object.fromEntries(Object.entries(file.data).sort(([a], [b]) => a.localeCompare(b))),
    };
  }
  ...
  async function create(data, appVersion, options = {}) {
    const file = payload({
      ...
      data: inspect(data, options.checkMarkup, 'export'),
    });
    const text = JSON.stringify({ ...file, sha256: await digest(file, options.crypto) }, null, 2);
    if (bytes(text) > MAX_BYTES)
      throw Error('This workspace exceeds the 16 MB portable backup limit. Download recovery copies instead.');
    return text;
  }
  async function parse(text, options = {}) {
    ...
    if (!object(file) || file.format !== FORMAT || file.version !== VERSION || file.scope !== 'active-workspace')
      throw Error('This is not a supported Cortex study backup. Recovery downloads use a different format.');
    if (
      typeof file.appVersion !== 'string' ||
      file.appVersion.length > 80 ||
      typeof file.createdAt !== 'string' ||
      !Number.isFinite(Date.parse(file.createdAt)) ||
      !/^[a-f0-9]{64}$/.test(file.sha256)
    )
      throw Error('The backup metadata is incomplete.');
    const data = inspect(file.data, options.checkMarkup),
      normalized = payload({ ...file, data });
    if ((await digest(normalized, options.crypto)) !== file.sha256)
      throw Error('The integrity check failed. This file is incomplete or has changed.');
    return normalized;
  }
  function safeMarkup(text, documentAPI = globalThis.document) {
    ...
    return [...template.content.querySelectorAll('*')].every(
      node => allowed.has(node.tagName) && [...node.attributes].every(a => a.name === 'aria-label')
    );
  }
